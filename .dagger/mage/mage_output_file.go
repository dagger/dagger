...
import (
	...
	"time"
	mage_mageimport "github.com/dagger/dagger/.dagger/mage"
 ...
 `[1:], _filepath.Base(os.Args[0]))
	...
	list := func() error {
		...
		targets := map[string]string{
			"engine:dev": "builds and starts an Engine & CLI from local source code",
			"engine:devEnv": "Get environment variable updates for running dagger",
		}
	...
	if args.Help {
		...
		switch _strings.ToLower(args.Args[0]) {
			case "engine:dev":
				_fmt.Println("Dev builds and starts an Engine & CLI from local source code")
				_fmt.Println()
				...
				return
				case "engine:devenv":
				_fmt.Println("Get environment variable updates for running dagger")
				_fmt.Println()
		...
		switch _strings.ToLower(target) {
				...
				case "engine:dev":
					...
					if expected > len(args.Args) {
						...
						logger.Printf("not enough arguments for target \"Engine:Dev\", expected %v, got %v\n", expected-1, len(args.Args)-1)
						os.Exit(2)
					}
					if args.Verbose {
						logger.Println("Running target:", "Engine:Dev")
					}
					
				wrapFn := func(ctx context.Context) error {
					return mage_mageimport.Engine{}.Dev(ctx)
				}
				...
				case "engine:devenv":
					...
					if expected > len(args.Args) {
						// note that expected and args at this point include the arg for the target itself
						// so we subtract 1 here to show the number of args without the target.
						logger.Printf("not enough arguments for target \"Engine:DevEnv\", expected %v, got %v\n", expected-1, len(args.Args)-1)
						os.Exit(2)
					}
					if args.Verbose {
						logger.Println("Running target:", "Engine:DevEnv")
					}
					
				wrapFn := func(ctx context.Context) error {
					mage_mageimport.Engine{}.DevEnv(ctx)
					return nil
				}
				ret := runTarget(logger, wrapFn)
					handleError(logger, ret)
