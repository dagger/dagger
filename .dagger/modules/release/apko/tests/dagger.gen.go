...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Tests":
		...
		case "Alpine":
			...
			return nil, (*Tests).Alpine(&parent, ctx)
		case "Build":
			var parent Tests
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return nil, (*Tests).Build(&parent, ctx)
		case "Publish":
			var parent Tests
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return nil, (*Tests).Publish(&parent, ctx)
		case "Wolfi":
			var parent Tests
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return nil, (*Tests).Wolfi(&parent, ctx)
		default:
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Tests", dagger.TypeDefWithObjectOpts{SourceMap: dag.SourceMap("main.go", 11, 6)}).
					WithFunction(
						dag.Function("All",
					...
					WithFunction(
						dag.Function("Wolfi",
							dag.TypeDef().WithKind(dagger.TypeDefKindVoidKind).WithOptional(true)).
							WithSourceMap(dag.SourceMap("main.go", 47, 1)))), nil
	default:
