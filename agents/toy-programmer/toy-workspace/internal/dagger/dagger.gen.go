...
func (r *Llm) GitModuleSource() *GitModuleSource {
	q := r.query.Select("GitModuleSource")
...
func (r *Llm) LocalModuleSource() *LocalModuleSource {
	q := r.query.Select("LocalModuleSource")
...
func (r *Llm) ModuleDependency() *ModuleDependency {
...
func (r *Llm) ModuleSourceView() *ModuleSourceView {
	q := r.query.Select("ModuleSourceView")
...
// send a single prompt to the llm, and return its reply as a string
func (r *Llm) Ask(ctx context.Context, prompt string) (string, error) {
	if r.ask != nil {
		return *r.ask, nil
	}
	q := r.query.Select("ask")
...
// LlmSyncOpts contains options for Llm.Sync
type LlmSyncOpts struct {
	// The maximum number of loops to allow.
	MaxLoops int
}

// synchronize the llm state: send outstanding prompts, process replies and tool calls
func (r *Llm) Sync(opts ...LlmSyncOpts) *Llm {
	q := r.query.Select("sync")
	for i := len(opts) - 1; i >= 0; i-- {
		// `maxLoops` optional argument
		if !querybuilder.IsZeroValue(opts[i].MaxLoops) {
			q = q.Arg("maxLoops", opts[i].MaxLoops)
		}
	}
...
func (r *Llm) WithGitModuleSource(value *GitModuleSource) *Llm {
	assertNotNil("value", value)
	q := r.query.Select("withGitModuleSource")
	q = q.Arg("value", value)
...
func (r *Llm) WithLocalModuleSource(value *LocalModuleSource) *Llm {
	assertNotNil("value", value)
	q := r.query.Select("withLocalModuleSource")
	q = q.Arg("value", value)
...
func (r *Llm) WithModuleDependency(value *ModuleDependency) *Llm {
	assertNotNil("value", value)
	q := r.query.Select("withModuleDependency")
	q = q.Arg("value", value)
...
func (r *Llm) WithModuleSourceView(value *ModuleSourceView) *Llm {
	assertNotNil("value", value)
	q := r.query.Select("withModuleSourceView")
	q = q.Arg("value", value)
...
type LlmWithPromptOpts struct {
	// Buffer the prompt locally without sending
	Lazy bool
}
...
// LlmWithPromptFileOpts contains options for Llm.WithPromptFile
type LlmWithPromptFileOpts struct {
	// Buffer the prompt locally without sending
	Lazy bool
}

// append the contents of a file to the llm context
func (r *Llm) WithPromptFile(file *File, opts ...LlmWithPromptFileOpts) *Llm {
...
func (r *Span) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = *dag.LoadSpanFromID(SpanID(id))
	return nil
}
