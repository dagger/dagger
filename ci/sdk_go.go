...
func (t GoSDK) Bump(ctx context.Context, version string) (*Directory, error) {
	return bumpSDK("go", version, t.Dagger.Source), nil
}
...
func bumpSDK(sdk string, version string, src *Directory) *Directory {
	...
	result := dag.Container().
		...
		WithWorkdir("/src").
		WithExec([]string{"python", "/opt/version-update.py", "--sdk", sdk, "generate", version}).
		Directory("/src")
	return result.Diff(src)
}
