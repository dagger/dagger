...
func (repo *Builder) Engine(
	...
	container := dag.
		...
		WithFile(EngineServerPath, repo.engineBinary(platform)).
		WithFile(engineShimPath, repo.shimBinary(platform)).
		WithFile("/usr/bin/dialstdio", repo.dialstdioBinary(platform)).
		WithExec([]string{"ln", "-s", "/usr/bin/dialstdio", "/usr/bin/buildctl"}).
		WithFile("/opt/cni/bin/dnsname", repo.dnsnameBinary(platform)).
		...
		With(repo.goSDKContent(ctx, platform)).
		With(repo.pythonSDKContent(ctx, platform)).
		With(repo.typescriptSDKContent(ctx, platform)).
		WithDirectory("/", cniPlugins(platform, false)).
		WithDirectory(distconsts.EngineDefaultStateDir, dag.Directory())
	return container, nil
}

func (repo *Builder) codegenBinary(platform dagger.Platform) *File {
	return repo.binary("./cmd/codegen", platform)
}

func (repo *Builder) engineBinary(platform dagger.Platform) *File {
	return repo.binary("./cmd/engine", platform)
}

func (repo *Builder) shimBinary(platform dagger.Platform) *File {
	return repo.binary("./cmd/shim", platform)
}

func (repo *Builder) dnsnameBinary(platform dagger.Platform) *File {
	return repo.binary("./cmd/dnsname", platform)
}

func (repo *Builder) dialstdioBinary(platform dagger.Platform) *File {
	return repo.binary("./cmd/dialstdio", platform)
}

func (repo *Builder) binary(
	pkg string,
	platform dagger.Platform,
) *File {
	base := GoBase(repo.Source)
	...
	ldflags = append(ldflags, "-X", "github.com/dagger/dagger/engine.Version="+repo.Version.EngineVersion())
