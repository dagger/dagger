...
func (r *Repository) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Directory    *Directory
	...
	r.GitDirectory = concrete.GitDirectory
	return nil
}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Util":
		...
		case "Repository":
			...
			return (*Util).Repository(&parent, source), nil
		default:
	...
	case "Repository":
		...
		case "DirectoryForGo":
			...
			return (*Repository).DirectoryForGo(&parent), nil
		case "GoBase":
			var parent Repository
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Repository).GoBase(&parent), nil
		case "DevelVersionInfo":
			var parent Repository
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return (*Repository).DevelVersionInfo(&parent, ctx)
		case "DaggerBinary":
			var parent Repository
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			var goarm string
			if inputArgs["goarm"] != nil {
				err = json.Unmarshal([]byte(inputArgs["goarm"]), &goarm)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg goarm", err))
				}
			}
			return (*Repository).DaggerBinary(&parent, ctx, goos, goarch, goarm)
		case "DaggerEngine":
			var parent Repository
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			if inputArgs["entrypointArgs"] != nil {
				err = json.Unmarshal([]byte(inputArgs["entrypointArgs"]), &entrypointArgs)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg entrypointArgs", err))
				}
			}
			var configEntries []string
			if inputArgs["configEntries"] != nil {
				err = json.Unmarshal([]byte(inputArgs["configEntries"]), &configEntries)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg configEntries", err))
				}
			}
			return (*Repository).DaggerEngine(&parent, ctx, goarch, entrypointArgs, configEntries)
		case "DaggerEngineService":
			var parent Repository
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			...
			return (*Repository).DaggerEngineService(&parent, ctx, name, cloudToken, cloudUrl, goarch, entrypointArgs, configEntries)
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "VersionInfo":
		switch fnName {
		case "EngineVersion":
			var parent VersionInfo
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return (*VersionInfo).EngineVersion(&parent), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Util").
					WithFunction(
						dag.Function("Repository",
							dag.TypeDef().WithObject("Repository")).
							WithArg("source", dag.TypeDef().WithObject("Directory")))).
			WithObject(
				dag.TypeDef().WithObject("Repository").
					WithFunction(
						dag.Function("DirectoryForGo",
							dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("GoBase",
							dag.TypeDef().WithObject("Container"))).
					WithFunction(
						dag.Function("DevelVersionInfo",
							dag.TypeDef().WithObject("VersionInfo"))).
					WithFunction(
						dag.Function("DaggerBinary",
							dag.TypeDef().WithObject("File")).
							WithArg("goos", dag.TypeDef().WithKind(StringKind).WithOptional(true)).
							WithArg("goarch", dag.TypeDef().WithKind(StringKind).WithOptional(true)).
							WithArg("goarm", dag.TypeDef().WithKind(StringKind).WithOptional(true))).
					WithFunction(
						dag.Function("DaggerEngine",
							dag.TypeDef().WithObject("Container")).
							WithArg("goarch", dag.TypeDef().WithKind(StringKind).WithOptional(true)).
							WithArg("entrypointArgs", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true)).
							WithArg("configEntries", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true))).
					WithFunction(
						dag.Function("DaggerEngineService",
							dag.TypeDef().WithObject("Service")).
							WithArg("name", dag.TypeDef().WithKind(StringKind)).
							WithArg("cloudToken", dag.TypeDef().WithObject("Secret").WithOptional(true)).
							WithArg("cloudURL", dag.TypeDef().WithObject("Secret").WithOptional(true)).
							WithArg("goarch", dag.TypeDef().WithKind(StringKind).WithOptional(true)).
							WithArg("entrypointArgs", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true)).
							WithArg("configEntries", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true))).
					WithField("Directory", dag.TypeDef().WithObject("Directory")).
					WithField("GitDirectory", dag.TypeDef().WithObject("Directory"))).
			WithObject(
				dag.TypeDef().WithObject("VersionInfo").
					WithFunction(
						dag.Function("EngineVersion",
							dag.TypeDef().WithKind(StringKind))).
					WithField("Tag", dag.TypeDef().WithKind(StringKind)).
					WithField("Commit", dag.TypeDef().WithKind(StringKind)).
					WithField("TreeHash", dag.TypeDef().WithKind(StringKind))), nil
	default:
