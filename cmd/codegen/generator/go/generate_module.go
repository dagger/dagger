...
func (g *GoGenerator) GenerateModule(ctx context.Context, schema *introspection.Schema, schemaVersion string) (*generator.GeneratedState, error) {
	...
	internalDir := internalDir(moduleConfig.PortableDaggerAPI)
	var overlay fs.FS = layerfs.New(
		mfs,
		&MountedFS{FS: dagger.QueryBuilder, Name: filepath.Join(outDir, internalDir)},
		&MountedFS{FS: dagger.Telemetry, Name: filepath.Join(outDir, internalDir)},
	)
