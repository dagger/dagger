...
{{- "" }}},
    ...
    {{ if $convertID -}}
    return new {{ $promiseRetType | FormatProtected }}(ctx.copy().selectNode(response, "{{ $promiseRetType | FormatProtected }}"))
    {{- else if not .TypeRef.IsVoid -}}
        {{- if and .TypeRef.IsList (IsListOfObject .TypeRef) }}
    return response.map((r) => new {{ . | FormatReturnType | ToSingleType | FormatProtected }}(ctx.copy().selectNode(r.id, "{{ . | FormatReturnType | ToSingleType | FormatProtected }}")))
