...
// which is what the previous implementation, `graphql-json-to-sdl`, was using.
package sdl
...
// Format renders the schema as GraphQL SDL. GraphQL builtins (the primitive
// scalars, the deprecated directive definition, and the introspection
// meta-types) are omitted.
func Format(w io.Writer, schema *introspection.Schema) {
	for _, tp := range schema.Directives {
		if slices.Contains([]string{"deprecated"}, tp.Name) {
			// builtin graphql directives - these need to be special cased
			continue
		}
		formatDirective(w, tp)
		fmt.Fprintln(w)
	}
	for _, tp := range schema.Types {
		if slices.Contains([]string{"String", "Int", "Float", "Boolean", "ID"}, tp.Name) {
			// builtin graphql types - these need to be special cased
			continue
		}
		if strings.HasPrefix(tp.Name, "_") {
