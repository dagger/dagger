...
var (
	...
	generateForceApply bool
)

func init() {
	generateCmd.Flags().BoolVarP(&generateListMode, "list", "l", false, "List available generators")
	generateCmd.Flags().BoolVarP(&generateForceApply, "force", "f", false, "Force apply of changes even if conflicts exist")
}
	...
	cs := generatorGroup.
		...
		Changes(
			dagger.GeneratorGroupChangesOpts{
				ContinueOnConflicts: generateForceApply,
			},
		)
