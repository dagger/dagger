...
func codexHarnessAuthOffer(router *LLMRouter) *LLMHarnessAuth {
	...
	if router.reloadCodexAuthToken != nil && extractChatGPTAccountID(router.OpenAICodexAuthToken) != "" {
		normal := router.reloadCodexAuthToken
		var authMu sync.Mutex
		lastToken := router.OpenAICodexAuthToken
		return &LLMHarnessAuth{Kind: LLMHarnessAuthOAuth, Resolve: func(resolveCtx context.Context, force bool) (LLMHarnessAuthState, error) {
			...
			defer authMu.Unlock()
			if force {
				resolveCtx = secretprovider.ContextWithRejectedEnvValue(resolveCtx, fmt.Sprintf("%x", sha256.Sum256([]byte(lastToken))))
			}
			credential, err := normal(resolveCtx)
			if err != nil {
