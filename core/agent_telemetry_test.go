...
func TestRehydrateDoesNotPublishWhenLeaseAcquisitionFails(t *testing.T) {
	...
	runtimes := NewAgentRuntimes()
	_, err = runtimes.Create(ctx, agent, AgentStateIdle, "", true)
...
func TestRehydratePublishesStateAndParent(t *testing.T) {
	...
	runtimes := NewAgentRuntimes()
	runtime, err := runtimes.Create(ctx, agent, AgentStatePaused, "", true, "agent-parent")
	require.NoError(t, err)
...
func TestAgentTombstoneTelemetryContextIsCompact(t *testing.T) {
	...
	// Host operations may rebind effective metadata to an ancestor. Attribution
	// must still belong to the detached agent scope.
	ctx = engine.ContextWithClientMetadata(ctx, &engine.ClientMetadata{SessionID: "session", ClientID: "ancestor"})
