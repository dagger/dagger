...
// on their own: added directories remain explicit, while a removed directory
// is suppressed whenever the removed files
...
func TestChangesetAbsentFromBeforeShapes(t *testing.T) {
	t.Run("path simply not included in the before tree", func(t *testing.T) {
		...
		require.Equal(t, []string{"ADDED sub/ +0 -0", "ADDED sub/tracked.txt +2 -0"},
