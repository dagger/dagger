...
func TestContainerShellDefaults(t *testing.T) {
	ctr := &Container{}
	require.Equal(t, []string{"sh"}, ctr.Shell(false, false).Args)
	require.Equal(t, []string{"sh", "-c"}, ctr.Shell(true, false).Args)

	require.False(t, ctr.Shell(false, false).PrivilegedNesting)
	require.True(t, ctr.Shell(false, true).PrivilegedNesting)
	...
	require.Equal(t, []string{"python", "-c"}, ctr.Shell(true, false).Args)
	require.True(t, ctr.Shell(true, false).PrivilegedNesting)
	require.True(t, ctr.Shell(true, false).InsecureRootCapabilities)

	ctr.DefaultTerminalCmd.Batch = []string{"python", "-I", "-c"}
	shell := ctr.Shell(true, false)
	require.Equal(t, []string{"python", "-I", "-c"}, shell.Args)
	shell.Args[0] = "changed"
	require.Equal(t, []string{"python", "-I", "-c"}, ctr.Shell(true, false).Args)
	require.Equal(t, []string{"python"}, ctr.Shell(false, false).Args)
