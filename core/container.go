...
func (container *Container) Exists(ctx context.Context, srv *dagql.Server, targetPath string, targetType ExistsType) (bool, error) {
	dir, targetPath := filepath.Split(filepath.Clean(targetPath))
	exists := false
	err := container.runUnderPath(dir, func(dir *Directory) error {
		var err error
		exists, err = dir.Exists(ctx, srv, targetPath, targetType)
		return err
	})
	return exists, err
}
