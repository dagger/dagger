...
func NewContainerDagOp(
	...
	ctr = ctr.Clone()
	err = dagop.setAllContainerMounts(ctx, ctr, sts, parent)
	if err != nil {
		return nil, err
	}
...
func (op *ContainerDagOp) setAllContainerMounts(
	...
	for mountIdx, mount := range op.Mounts {
		...
		case 0:
			...
			var rootfsDir *Directory
			if container.FS.Self() != nil {
				rootfsDir = &Directory{
					LLB:      def.ToPB(),
					Dir:      container.FS.Self().Dir,
					Platform: container.FS.Self().Platform,
					Services: container.FS.Self().Services,
				}
			} else {
				rootfsDir, err = NewScratchDirectory(ctx, container.Platform)
				if err != nil {
					return fmt.Errorf("failed to create scratch directory for rootfs: %w", err)
				}
			}
