...
func (dir *Directory) FindupFile(ctx context.Context, name string, start string) (*File, error) {
	result, err := dir.Findup(ctx, name, start)
	if err != nil {
		return nil, err
	}
	return dir.File(ctx, result)
}

func (dir *Directory) FindupDirectory(ctx context.Context, name string, start string) (*Directory, error) {
	result, err := dir.Findup(ctx, name, start)
	if err != nil {
		return nil, err
	}
	return dir.Directory(ctx, result)
}
...
func (dir *Directory) Findup(ctx context.Context, name string, start string) (string, error) {
	...
	return "", fmt.Errorf("file %q not found in %q or any parent directories", name, start)
}
