...
1. A file `/inputs/dagger.json` will be mounted as read-only into the ExecOp, with the following (json-encoded) contents:
   ...
   * `args` - The args provided to the GraphQL resolver, as described in the [Apollo Server resolver arguments documentation](https://www.apollographql.com/docs/apollo-server/data/resolvers/#resolver-arguments).
   * `parent` - The result of the parent resolver to this field in the GraphQL query (if any), as described in the [Apollo Server resolver arguments documentation](https://www.apollographql.com/docs/apollo-server/data/resolvers/#resolver-arguments).
