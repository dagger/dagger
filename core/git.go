...
func (ref *LocalGitRef) Tree(ctx context.Context, discardGitDir bool) (*Directory, error) {
	...
	pbDef, _, err := bk.EngineContainerLocalImport(ctx, platforms.DefaultSpec(), tmpDir, nil, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to import container tarball from engine container filesystem: %w", err)
	}
	return NewDirectory(ref.Query, pbDef, "", Platform(platforms.DefaultSpec()), nil), nil
}
