...
func (ArtifactsSuite) TestAbsoluteURI(ctx context.Context, t *testctx.T) {
  ...
  artifacts { filterPath(path: ["base"]) { one { uri(absolute: true) } } }
	...
	out, err = base.With(workspaceSelectionDaggerExec("list", "-f=link", "dag://"+ref+":base")).Stdout(ctx)
	require.NoError(t, err)
	...
	require.Equal(t, "--provider --check=verify\n", out)
	_, err = base.With(workspaceSelectionDaggerExec("check", "dag://"+ref+":verify")).Sync(ctx)
	require.NoError(t, err)
	for _, typ := range []string{"Expertise", "expertise"} {
		out, err := base.With(workspaceSelectionDaggerExec("-W", ref, "list", "-f=link", "--type", typ)).Stdout(ctx)
		require.NoError(t, err)
	...
	for _, tc := range []struct {
		...
		{[]string{"list", "-f=link", "base"}, []string{"base"}},
  ...
  artifacts { filterUri(uri: "dag://github.com/dagger/dagger@main:base") { items { uri } } }
	...
	for _, address := range []string{uri, "dag://github.com/acme/other@main:base"} {
		out, err := base.With(workspaceSelectionDaggerQuery(fmt.Sprintf(`{ currentWorkspace {
  artifacts { filterTypes(types: []) {
    filterUri(uri: %q) { items { uri } }
    withoutUri(uri: %q) { items { uri } }
  } }
} }`, address, address), "-W", ref, "-m", "core")).Stdout(ctx)
		require.NoError(t, err)
		require.JSONEq(t, `{"currentWorkspace":{"artifacts":{"filterTypes":{
  "filterUri":{"items":[]},"withoutUri":{"items":[]}
}}}}`, out)
	}
}
...
func (ArtifactsSuite) TestModuleObjects(ctx context.Context, t *testctx.T) {
	...
	provider, err := source.File("provider/main.dang").Contents(ctx)
	require.NoError(t, err)
	source = source.WithNewFile("provider/main.dang", strings.Replace(provider, "pub label:", "pub instance: Provider! { self }\n  pub label:", 1))
	ws := source.AsWorkspace()
	for _, module := range []string{"provider", "consumer"} {
		items, err := ws.Artifacts().FilterPath([]string{module}).Items(ctx)
		require.NoError(t, err)
	...
	for _, fixture := range []struct {
		...
		t.Run(fixture.name, func(ctx context.Context, t *testctx.T) {
			...
			t.Run("shortcut keeps discovery error", func(ctx context.Context, t *testctx.T) {
				...
				require.NotContains(t, out, "Error: exit code")
			})
		})
	}
}

func (ArtifactsSuite) TestHelpProgress(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	base := nativeWorkspaceBase(t, c).WithDirectory(".", artifactSource(c)).
	...
	for _, command := range []string{"check", "generate", "up", "shell", "agent", "list"} {
		t.Run(command, func(ctx context.Context, t *testctx.T) {
			...
			if command == "list" {
				action = "Load available types"
			}
			for _, mode := range []string{"default", "verbose", "silent"} {
				...
				case "silent":
					if command == "list" {
						ctr = ctr.WithEnvVariable("DAGGER_SILENT", "true")
					} else {
						args = append([]string{"--silent"}, args...)
					}
				}
				result := ctr.With(daggerExec(args...))
				...
				require.NoError(t, err)
				if mode == "verbose" {
					require.Contains(t, progress, action)
				} else {
					require.NotContains(t, progress, action)
				}
			}
		})
	}
	t.Run("completion stays silent", func(ctx context.Context, t *testctx.T) {
		result := base.With(daggerExec("-v", "__complete", "check", ""))
		out, err := result.CombinedOutput(ctx)
		require.NoError(t, err)
		require.NotContains(t, out, "Load check filters")
	})
	t.Run("execution after discovery", func(ctx context.Context, t *testctx.T) {
		...
		require.NotContains(t, progress, "exporter is shutdown")
	})
}
...
func (ArtifactsSuite) TestArtifactsCLI(ctx context.Context, t *testctx.T) {
	...
	t.Run("list descriptions", func(ctx context.Context, t *testctx.T) {
		...
		out, err := base.WithNewFile("/work/selected/provider/main.dang", source).
			With(workspaceSelectionDaggerExec("-W", "/work/selected", "list", "broken", "broken", "-f=cli")).Stdout(ctx)
		require.NoError(t, err)
		require.Equal(t, "--provider --container=broken   # A container that must not run.\n", out)
	})
	...
	for _, tc := range []struct {
		...
		{[]string{"list", "--type", "Container"}, "dag+container://base\ndag+container://broken\ndag+container://consumer/base\n"},
		{[]string{"list", "--type", "container"}, "dag+container://base\ndag+container://broken\ndag+container://consumer/base\n"},
		{[]string{"list", "--type", "ProviderDocs"}, "dag+provider-docs://docs\ndag+provider-docs://docs/again\ndag+provider-docs://other-docs\ndag+provider-docs://other-docs/again\n"},
		{[]string{"list", "--type", "provider-docs"}, "dag+provider-docs://docs\ndag+provider-docs://docs/again\ndag+provider-docs://other-docs\ndag+provider-docs://other-docs/again\n"},
		{[]string{"list", "--type="}, ""},
		{[]string{"list", "--type=", "--type=container"}, "dag+container://base\ndag+container://broken\ndag+container://consumer/base\n"},
		{[]string{"list", "consumer", "--type", "Container", "--type", "File"}, "dag+container://consumer/base\ndag+file://consumer/input\n"},
		{[]string{"list", "consumer", "--type", "container", "--type", "File"}, "dag+container://consumer/base\ndag+file://consumer/input\n"},
		{[]string{"list", "docs"}, "dag+directory://docs/source\ndag+provider-docs://docs\ndag+provider-docs://docs/again\n"},
		{[]string{"list", "dag://docs"}, "dag+directory://docs/source\ndag+provider-docs://docs\ndag+provider-docs://docs/again\n"},
		{[]string{"list", "provider/docs"}, "dag+directory://docs/source\ndag+provider-docs://docs\ndag+provider-docs://docs/again\n"},
		{[]string{"list", "provider:docs"}, "dag+directory://docs/source\ndag+provider-docs://docs\ndag+provider-docs://docs/again\n"},
		{[]string{"list", "**/source"}, "dag+directory://docs/source\ndag+directory://other-docs/source\n"},
		{[]string{"list", "base", "consumer/base"}, "dag+container://base\ndag+container://consumer/base\n"},
		{[]string{"list", "dag+container://consumer"}, "dag+container://consumer/base\n"},
		{[]string{"list", "dag://consumer?missing=anything"}, ""},
		{[]string{"list", "dag://?go-module=sdk/go"}, ""},
	} {
...
`).
  ...
  pub types: Directory! { directory }
	...
	out, err := reserved.With(workspaceSelectionDaggerExec("-W", "/work/selected", "list", "types", "-f=link")).Stdout(ctx)
	require.NoError(t, err)
...
func (ArtifactsSuite) TestCLIAddressSelections(ctx context.Context, t *testctx.T) {
	...
	for _, tc := range []struct {
		...
		{[]string{"list", "dag+directory://docs", "dag+container://consumer"}, "dag+container://consumer/base\ndag+directory://docs/source\n"},
		{[]string{"list", "dag://docs?missing=value", "consumer/base"}, "dag+container://consumer/base\n"},
		{[]string{"list", "docs", "docs/source"}, "dag+directory://docs/source\ndag+provider-docs://docs\ndag+provider-docs://docs/again\n"},
		{[]string{"list", "dag+directory://docs", "dag+container://consumer", "--type", "Directory"}, "dag+directory://docs/source\n"},
	} {
		t.Run(strings.Join(tc.args, " "), func(ctx context.Context, t *testctx.T) {
			args := append([]string{"-W", "/work/selected"}, tc.args...)
			args = append(args, "-f=link")
...
`)
	...
	for _, tc := range []struct {
		...
		{[]string{"list", "provider/docs", "--type", "Directory"}, "dag+directory://provider/docs/source\n"},
	} {
		t.Run(strings.Join(tc.args, " "), func(ctx context.Context, t *testctx.T) {
			args := append([]string{"-W", "/work/selected"}, tc.args...)
			args = append(args, "-f=link")
	...
	out, err := base.With(workspaceSelectionDaggerExec("__complete", "-W", "/work/selected", "list", "provider/docs", "--type", "Dir")).Stdout(ctx)
	require.NoError(t, err)
...
`
 ...
 pub scana: Void @check { raise "must not run" }
 pub scanb: Void @check { raise "must not run" }
 ...
 rawServices: filterDirectives(directives: ["up"]) { items { uri } }
	...
	for _, tc := range []struct{ command, flag, expected, scheme string }{
		...
		t.Run(tc.command+tc.flag, func(ctx context.Context, t *testctx.T) {
			args := []string{tc.command, "-l", "-f=link"}
			if tc.flag != "" {
			...
			require.NoError(t, err)
			var want []string
			for _, uri := range expected[tc.expected] {
				want = append(want, strings.Replace(uri, "dag://", "dag+"+tc.scheme+"://", 1))
			}
			require.Equal(t, strings.Join(want, "\n")+"\n", out)
		})
	}
...
func composeArtifactAgents(ctx context.Context, c *dagger.Client, ws *dagger.Workspace, artifacts *dagger.Artifacts, base ...*dagger.LLM) (*dagger.LLM, error) {
	...
	agents, err := artifacts.FilterTypes([]string{"Expertise"}).AsExpertise(ctx)
	if err != nil {
		return nil, err
	}
...
func (ArtifactsSuite) TestUnknownEnvironment(ctx context.Context, t *testctx.T) {
	...
	for _, command := range [][]string{
		{"list", "--type=Check"}, {"check", "-l"}, {"generate", "-l"},
