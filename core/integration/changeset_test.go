...
func (ChangesetSuite) TestEmpty(ctx context.Context, t *testctx.T) {
	...
	// empty
	empty, err := baseDir.Changes(baseDir).Empty(ctx)
	require.NoError(t, err)
	...
	empty, err = addedAndRemovedDir.Changes(baseDir).Empty(ctx)
	require.NoError(t, err)
	require.True(t, empty)

	// not empty - modified
	modifiedDir := baseDir.WithNewFile("file.txt", "modified")
	empty, err = modifiedDir.Changes(baseDir).Empty(ctx)
	require.NoError(t, err)
	require.False(t, empty)

	// not empty - added
	addedDir := baseDir.WithNewFile("newfile.txt", "new")
	empty, err = addedDir.Changes(baseDir).Empty(ctx)
	require.NoError(t, err)
	require.False(t, empty)

	// not empty - removed
	removedDir := baseDir.WithoutFile("file.txt")
	empty, err = removedDir.Changes(baseDir).Empty(ctx)
	require.NoError(t, err)
	require.False(t, empty)
}
