...
func (DirectorySuite) TestFilter(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	...
	t.Run("exclude", func(ctx context.Context, t *testctx.T) {
		entries, err := dir.Filter(ctx, dagger.DirectoryFilterOpts{
			Exclude: []string{"*.rar"},
		}).Entries(ctx)
		require.NoError(t, err)
		require.Equal(t, []string{"a.txt", "b.txt", "subdir"}, entries)
	...
	t.Run("exclude respects subdir", func(ctx context.Context, t *testctx.T) {
		entries, err := subdir.Filter(ctx, dagger.DirectoryFilterOpts{
			Exclude: []string{"*.rar"},
		}).Entries(ctx)
		require.NoError(t, err)
		require.Equal(t, []string{"d.txt", "e.txt"}, entries)
