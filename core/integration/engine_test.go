...
func (EngineSuite) TestSessionQueryOutlivesLegacyHeaderDeadline(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)

	out, err := c.Container().
		From(alpineImage).
		WithExec([]string{"sleep", "35"}).
		File("/etc/alpine-release").
		Contents(ctx)
	require.NoError(t, err)
	require.Contains(t, out, distconsts.AlpineVersion)
}
