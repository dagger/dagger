...
`)
	...
	added := removed.With(daggerNonNestedExec("module", "client", "add", "--sdk=go", "target-alias", "-y"))
	...
	clientUpdated := added.
		...
		With(daggerNonNestedExec("module", "client", "update", "target-alias", "-y"))
...
`)
	...
	t.Run("installed module alias", func(ctx context.Context, t *testctx.T) {
		added := base.
			...
			With(daggerNonNestedExec("--auto-apply", "module", "client", "add", "--sdk=go", "dep-alias"))
...
`).
			...
			With(daggerNonNestedExec("module", "client", "add", "--sdk=go", "target-alias", "-y"))
		out, err := added.CombinedOutput(ctx)
		require.NoError(t, err, out)
