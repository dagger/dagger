...
func (m *Consumer) SyncGenerators(ctx context.Context, workspace *dagger.Workspace) (string, error) {
	items, err := workspace.Artifacts().FilterGenerateCommand().AsGenerators(ctx)
...
`)
	...
	removed := generated.With(daggerNonNestedExec("module", "client", "rm", "./target", "-y"))
	exists, err = removed.Exists(ctx, "internal/dagger/clients/target.gen.go")
	require.NoError(t, err)
	require.False(t, exists)

	added := removed.With(daggerNonNestedExec("module", "client", "add", "--sdk=go", "./target", "-y"))
	config, err := added.File("dagger.toml").Contents(ctx)
	require.NoError(t, err)
	addedCfg, err := workspacecfg.ParseConfig([]byte(config))
	require.NoError(t, err)
	require.Equal(t, []string{"./target"}, addedCfg.SDKs["go"].Scopes["."].Clients)
...
`)
	...
	t.Run("workspace root scope", func(ctx context.Context, t *testctx.T) {
		added := base.
			...
			With(daggerNonNestedExec("--auto-apply", "module", "client", "add", "--sdk=go", "./dep"))
...
`).
	...
	t.Run("lists installed SDKs", func(ctx context.Context, t *testctx.T) {
		...
		require.NoError(t, err, out)
		require.NotContains(t, out, "AVAILABLE COMMANDS")
		require.Contains(t, out, "--sdk SDK")
		require.Contains(t, out, "available: go")
		require.NotContains(t, out, "EXAMPLES")
	})
...
`).
			...
			With(daggerNonNestedExec("module", "client", "add", "--sdk=go", "../target", "-y"))
		out, err := added.CombinedOutput(ctx)
		require.NoError(t, err, out)
		...
		require.Equal(t, []string{"./target"}, cfg.SDKs["go"].Scopes["internal"].Clients)
	})
...
func (GeneratorsSuite) TestGeneratorArtifactEvaluation(ctx context.Context, t *testctx.T) {
 ...
 currentWorkspace { artifacts(include: ["generate-files"]) { filterGenerateCommand {
  values { error { message } value { ... on Generator { changeset { isEmpty } } } }
	...
	require.NoError(t, err)
	require.JSONEq(t, `{"currentWorkspace":{"artifacts":{"filterGenerateCommand":{"values":[{"error":null,"value":{"changeset":{"isEmpty":false}}}]}}}}`, out)
}
