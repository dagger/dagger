...
func (GitSuite) TestGitSchemeless(ctx context.Context, t *testctx.T) {
	...
	t.Run("private ssh explicit scheme no user", func(ctx context.Context, t *testctx.T) {
		sockPath, cleanup := setupPrivateRepoSSHAgent(t)
		defer cleanup()

		// Regression: explicit ssh:// URL with no user must still resolve as git@...
		// even when sshAuthSocket is already provided on the Git object.
		repo := c.Git("ssh://gitlab.com/dagger-modules/private/test/more/dagger-test-modules-private.git", dagger.GitOpts{
			SSHAuthSocket: c.Host().UnixSocket(sockPath),
		})
		require.NoError(t, checkAccess(ctx, repo))
...
// pushToGitService creates a new commit on the test remote.
...
// TestGitFunctionCacheInvalidation verifies cross-session invalidation for GitRepository args.
...
// TestGitLocalRepoFunctionCacheAcrossSessions verifies that local git args
// keep stable cross-session cache identity once repo.__resolve strips CachePerClient noise.
func (GitSuite) TestGitLocalRepoFunctionCacheAcrossSessions(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)

	devEngine := devEngineContainerAsService(devEngineContainer(c))

	baseMod := goGitBase(t, c).
		With(daggerExec("init", "--source=.", "--name=test", "--sdk=go")).
		WithNewFile("main.go", `package main
...
`).
	...
	initialModDir := baseMod.Directory("/work")
	updatedModDir := baseMod.
		WithNewFile("bump.txt", identity.NewID()).
		WithExec([]string{"git", "add", "bump.txt"}).
		WithExec([]string{"git", "commit", "-m", "bump"}).
		Directory("/work")

	callFn := func(modDir *dagger.Directory) string {
		out, err := engineClientContainer(ctx, t, c, devEngine).
			WithDirectory("/work", modDir).
			WithWorkdir("/work").
			WithEnvVariable("CACHEBUSTER", identity.NewID()).
			With(daggerNonNestedExec("call", "get-time", "--repo", ".git")).
			Stdout(ctx)
		require.NoError(t, err)
		return strings.TrimSpace(out)
	}

	firstTime := callFn(initialModDir)
	require.NotEmpty(t, firstTime)

	cachedTime := callFn(initialModDir)
	require.Equal(t, firstTime, cachedTime, "same local git state across sessions should hit DagOp cache")

	updatedTime := callFn(updatedModDir)
	require.NotEqual(t, cachedTime, updatedTime, "local HEAD changes should bust cache")
}

// TestGitRefFunctionCacheInvalidation verifies GitRef arg cache keys include ls-remote state.
...
// TestGitTreeCacheAcrossProtocols verifies .git-preserving trees stay transport-sensitive,
// while content trees (discarded .git) share identity across equivalent protocols.
