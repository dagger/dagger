...
// Sync after saving or external edits supplies a new baseline without changing
// workspaces already bound to an LLM or invalidating ambient read caches.
func (LLMSuite) TestWorkspaceSyncRefreshesBinding(ctx context.Context, t *testctx.T) {
	for _, export := range []bool{false, true} {
		t.Run(fmt.Sprintf("export=%t", export), func(ctx context.Context, t *testctx.T) {
			...
			original := c.LLM().WithWorkspace(syncWorkspace(ctx, t, c.CurrentWorkspace()))
			if export {
			...
			fresh := syncWorkspace(ctx, t, c.CurrentWorkspace())
