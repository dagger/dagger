...
func (LocalDefaultsSuite) TestRemoteFile(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
...
func (LocalDefaultsSuite) TestLocalFile(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	output, err := nestedDaggerContainer(t, c, "go", "defaults").
...
func (LocalDefaultsSuite) TestLocalDirectory(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	output, err := nestedDaggerContainer(t, c, "go", "defaults").
...
func (LocalDefaultsSuite) TestRemoteDirectory(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	output, err := nestedDaggerContainer(t, c, "go", "defaults").
		WithWorkdir("defaults").
...
func (LocalDefaultsSuite) TestLocalBlueprint(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
...
func (LocalDefaultsSuite) TestOuterEnvFile(ctx context.Context, t *testctx.T) {
	...
	t.Run("ModuleSource LocalDefaults", func(ctx context.Context, t *testctx.T) {
		greeting, err := src.LocalDefaults().Get(ctx, "GREETING")
		require.NoError(t, err)
		require.Equal(t, "salutations", greeting)
		messageName, err := src.LocalDefaults().Get(ctx, "MESSAGE_NAME")
		require.NoError(t, err)
		require.Equal(t, "tout le monde", messageName)
		unrelated, err := src.LocalDefaults().Get(ctx, "UNRELATED")
		require.NoError(t, err)
	...
	t.Run("Module LocalDefaults", func(ctx context.Context, t *testctx.T) {
		mod := src.AsModule()
		greeting, err := mod.LocalDefaults().Get(ctx, "GREETING")
		require.NoError(t, err)
		require.Equal(t, "salutations", greeting)
		messageName, err := mod.LocalDefaults().Get(ctx, "MESSAGE_NAME")
		require.NoError(t, err)
		require.Equal(t, "tout le monde", messageName)
		unrelated, err := mod.LocalDefaults().Get(ctx, "UNRELATED")
		require.NoError(t, err)
		require.Equal(t, "", unrelated)
	})
}

func (LocalDefaultsSuite) TestSystemVariables(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	output, err := nestedDaggerContainer(t, c, "go", "defaults").
		WithWorkdir("defaults").
		...
		Stdout(ctx)
	require.NoError(t, err)
	require.Equal(t, "live long and prosper, world!", output, "output should include the system env variable")
}

func (LocalDefaultsSuite) TestRequiredDirectory(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	output, err := nestedDaggerContainer(t, c, "go", "defaults").
		WithWorkdir("defaults").
		...
		Stdout(ctx)
	require.NoError(t, err, "local default should successfully apply to required argument")
	require.Equal(t, "hello.txt\n", output, "local default should successfully apply to required argument")
}

func (LocalDefaultsSuite) TestRequiredString(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	output, err := nestedDaggerContainer(t, c, "go", "defaults").
		...
		Stdout(ctx)
	require.NoError(t, err, "local default should successfully apply to required argument")
	require.Equal(t, "HELLO WORLD", output, "local default should successfully apply to required argument")
}

func (LocalDefaultsSuite) TestArgName(ctx context.Context, t *testctx.T) {
	t.Skip(`FIXME: test conversion between arg name and env var name (eg."FOO_BAR=hello" -> "fooBar=hello"`)
}

func (LocalDefaultsSuite) TestDependencies(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	output, err := nestedDaggerContainer(t, c, "go", "defaults").
		...
		Stdout(ctx)
	require.NoError(t, err)
	require.Equal(t, "hello, world!!!!", output, "Local defaults should apply to nested dependencies")
}

var nestedExec = dagger.ContainerWithExecOpts{ExperimentalPrivilegedNesting: true}

func (LocalDefaultsSuite) TestCustomEnvFile(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
...
func (LocalDefaultsSuite) TestConstructorOptional(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	base := nestedDaggerContainer(t, c, "go", "defaults").
...
func (LocalDefaultsSuite) TestConstructorRequired(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name+" call", func(ctx context.Context, t *testctx.T) {
			out, err := tc.ctr.
				...
				Stdout(ctx)
			require.NoError(t, err)
			require.Equal(t, "yay", out, "local default should be applied")
			out, err = tc.ctr.
				WithExec([]string{"dagger", "call", "count"}, nestedExec).
				Stdout(ctx)
			require.NoError(t, err)
			require.Equal(t, "42", out, "local default should be applied")
			out, err = tc.ctr.
				WithExec([]string{"dagger", "call", "file", "contents"}, nestedExec).
				Stdout(ctx)
			require.NoError(t, err)
			require.Equal(t, "hello there!", out, "local default should be applied")
			out, err = tc.ctr.
				WithExec([]string{"dagger", "call", "dir", "entries"}, nestedExec).
				Stdout(ctx)
			require.NoError(t, err)
			require.Equal(t, "hello.txt\n", out, "local default should be applied")
		})
	}
}

func (LocalDefaultsSuite) TestCaching(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
	ctr := nestedDaggerContainer(t, c, "go", "defaults")
	// First run
	output1, err := ctr.
