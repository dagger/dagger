...
func (EngineSuite) TestLocalCacheGC(ctx context.Context, t *testctx.T) {
	...
	for _, tc := range []struct {
		...
		f := func(ctx context.Context, t *testctx.T, engine *dagger.Container, automaticGCEnabled bool) {
			...
			c3, err := daggerio.Connect(ctx, dagger.WithRunnerHost(endpoint), dagger.WithLogOutput(NewTWriter(t)))
			require.NoError(t, err)
			...
			c4, err := daggerio.Connect(ctx, dagger.WithRunnerHost(endpoint), dagger.WithLogOutput(NewTWriter(t)))
			require.NoError(t, err)
