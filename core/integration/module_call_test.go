...
func (m *Test) FnRef(ref *dagger.GitRef) *dagger.Directory {
	return ref.Tree()
		...
		t.Run("local dir", func(ctx context.Context, t *testctx.T) {
			...
			currentBranch, err := modGen.WithExec([]string{"git", "rev-parse", "--abbrev-ref", "HEAD"}).Stdout(ctx)
			require.NoError(t, err)
			expectedHeadRef := "ref: refs/heads/" + strings.TrimSpace(currentBranch)

			out, err := modGen.With(daggerCall("fn-repo", "--repo", ".git", "file", "--path=.git/HEAD", "contents")).Stdout(ctx)
			require.NoError(t, err)
			require.Equal(t, expectedHeadRef, strings.TrimSpace(out))
