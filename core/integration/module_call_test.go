...
`,
	...
	t.Run("list args", func(ctx context.Context, t *testctx.T) {
		...
		modGen := c.Container().From(golangImage).
			WithMountedFile(testCLIBinPath, daggerCliFile(t, c)).
			WithWorkdir("/work").
			With(daggerExec("init", "--source=.", "--sdk=go", "minimal")).
...
`), 0o644)
		...
		_, err = hostDaggerExec(ctx, t, depModDir, "init", "--source=.", "--sdk=go", "dep")
		require.NoError(t, err)
...
`), 0o644)
		...
		_, err = hostDaggerExec(ctx, t, idModDir, "init", "--source=.", "--sdk=go", "test")
		require.NoError(t, err)
...
func (CallSuite) TestByName(ctx context.Context, t *testctx.T) {
	t.Run("local", func(ctx context.Context, t *testctx.T) {
		...
		ctr := c.Container().From(golangImage).
			...
			With(daggerExec("init", "--source=.", "--sdk=go", "mod-a")).
			...
			With(daggerExec("init", "--source=.", "--sdk=go", "mod-b")).
	...
	t.Run("local ref with @", func(ctx context.Context, t *testctx.T) {
		...
		ctr := c.Container().From(golangImage).
			WithMountedFile(testCLIBinPath, daggerCliFile(t, c)).
			WithWorkdir("/work/").
			With(daggerExec("init", "--source=test@test", "--sdk=go", "mod-a", "test@test")).
