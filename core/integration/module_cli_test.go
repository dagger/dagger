...
`
	...
	// t.Run("no-sdk module can load transitive module without sdk", func(ctx context.Context, t *testctx.T) {
	// 	out, err := testCtr.With(daggerShell("nosdk | hello | hello")).Stdout(ctx)
	// 	require.NoError(t, err)
	// 	require.Equal(t, "Hi\n", out)
	// })
}
