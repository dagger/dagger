...
func (CLISuite) TestModuleDependencyInstall(ctx context.Context, t *testctx.T) {
	t.Run("local", func(ctx context.Context, t *testctx.T) {
		...
		base := goGitBase(t, c).
			...
			WithWorkdir("/work").
			With(daggerExec("module", "init", "--source=test", "--sdk=go", "test", "test")).
			With(daggerExec("module", "install", "-m=test", "./subdir/dep")).
		...
		t.Run("install with absolute paths", func(ctx context.Context, t *testctx.T) {
			ctr := base.
				WithWorkdir("/").
				With(daggerExec("module", "init", "--source=/work/test2", "--sdk=go", "test2", "/work/test2")).
				With(daggerExec("module", "install", "-m=/work/test2", "/work/subdir/dep")).
	...
	t.Run("installing a dependency with duplicate name is not allowed", func(ctx context.Context, t *testctx.T) {
		...
		ctr := c.Container().
			...
			With(daggerExec("module", "init", "--sdk=go", "--source=.", "dep2")).
			WithWorkdir("/work").
		...
		_, err = ctr.
			With(daggerExec("module", "install", "./dep2", "--name=dep")).
			Sync(ctx)
	...
	t.Run("installing a dependency with implicit duplicate name is not allowed", func(ctx context.Context, t *testctx.T) {
		...
		_, err = ctr.
			With(daggerExec("module", "install", "./dep2")).
			Sync(ctx)
		requireErrOut(t, err, fmt.Sprintf("duplicate dependency name %q", "dep"))
	})
...
`,
			...
			WithWorkdir("/work").
			With(daggerExec("module", "init", "--sdk=go", "--source=.", "foo")).
			With(daggerExec("module", "install", "--eager-runtime", "./dep")).
			Sync(ctx)
	...
	t.Run("install dep from various places", func(ctx context.Context, t *testctx.T) {
		...
		base := goGitBase(t, c).
			WithMountedFile(testCLIBinPath, daggerCliFile(t, c)).
			WithWorkdir("/work").
			With(daggerExec("module", "init", "--source=subdir/dep", "--sdk=go", "dep", "subdir/dep")).
			WithNewFile("/work/subdir/dep/main.go", `package main
	...
	t.Run("install out of tree dep fails", func(ctx context.Context, t *testctx.T) {
		...
		t.Run("from src dir", func(ctx context.Context, t *testctx.T) {
			_, err := base.
				WithWorkdir("/work/test").
				With(daggerExec("module", "install", "../../play/dep")).
				Sync(ctx)
		...
		t.Run("from dep dir", func(ctx context.Context, t *testctx.T) {
			_, err := base.
				WithWorkdir("/play/dep").
				With(daggerExec("module", "install", "-m=../../work/test", ".")).
				Sync(ctx)
			requireErrOut(t, err, `local module dependency context directory "/play/dep" is not in parent context directory "/work"`)
		})
		...
		t.Run("from root", func(ctx context.Context, t *testctx.T) {
			_, err := base.
				WithWorkdir("/").
				With(daggerExec("module", "install", "-m=work/test", "play/dep")).
				Sync(ctx)
			requireErrOut(t, err, `local module dependency context directory "/play/dep" is not in parent context directory "/work"`)
		})
...
`,
			...
			t.Run("sad", func(ctx context.Context, t *testctx.T) {
				...
				_, err := goGitBase(t, c).
					...
					WithWorkdir("/work").
					With(daggerExec("module", "init", "--sdk=go", "--source=.", "test")).
					With(daggerExec("module", "install", testGitModuleRef(tc, "../../"))).
					Sync(ctx)
				...
				_, err = goGitBase(t, c).
					...
					WithWorkdir("/work").
					With(daggerExec("module", "init", "--sdk=go", "--source=.", "test")).
					With(daggerExec("module", "install", testGitModuleRef(tc, "this/just/does/not/exist"))).
					Sync(ctx)
			...
			t.Run("unpinned gets pinned", func(ctx context.Context, t *testctx.T) {
				...
				out, err := goGitBase(t, c).
					...
					WithWorkdir("/work").
					With(daggerExec("module", "init", "--sdk=go", "--source=.", "test")).
					With(daggerExec("module", "install", tc.gitTestRepoRef)).
...
func (CLISuite) TestModuleDependencyInstallOrder(ctx context.Context, t *testctx.T) {
	...
	base := goGitBase(t, c).
		...
		With(daggerExec("module", "init", "--source=.", "--sdk=go", "dep-abc")).
		...
		With(daggerExec("module", "init", "--source=.", "--sdk=go", "dep-xyz")).
		...
		WithWorkdir("/work").
		With(daggerExec("module", "init", "--source=test", "--sdk=go", "test"))

	daggerJSON, err := base.
		With(daggerExec("module", "install", "./dep-abc")).
		With(daggerExec("module", "install", "./dep-xyz")).
		File("dagger.json").
		Contents(ctx)
	require.NoError(t, err)
	names := []string{}
	...
	t.Run("local dep", func(ctx context.Context, t *testctx.T) {
		t.Run("uninstall a dependency currently used in module", func(ctx context.Context, t *testctx.T) {
			...
			ctr := c.Container().
				...
				With(daggerExec("module", "init", "--sdk=go", "--source=.", "bar")).
				WithWorkdir("/work").
				With(daggerExec("module", "init", "--sdk=go", "--source=.", "foo")).
		...
		for _, tc := range testcases {
			t.Run(tc.name, func(ctx context.Context, t *testctx.T) {
				...
				if len(tc.installCmd) > 0 {
					ctr = ctr.With(daggerExec(append([]string{"module"}, tc.installCmd...)...))
				}
	...
	t.Run("git dependency", func(ctx context.Context, t *testctx.T) {
		...
		for _, tc := range testcases {
			t.Run(fmt.Sprintf("installed using %q, uninstalled using %q", tc.installCmdMod, tc.uninstallCmdMod), func(ctx context.Context, t *testctx.T) {
				...
				if tc.installCmdMod != "" {
					ctr = ctr.With(daggerExec("module", "install", tc.installCmdMod))
				}
...
func (CLISuite) TestModuleDependencyUpdate(ctx context.Context, t *testctx.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(ctx context.Context, t *testctx.T) {
			...
			daggerjson, err := ctr.
				With(daggerExec(append([]string{"module"}, tc.updateCmd...)...)).
				File("dagger.json").
				Contents(ctx)
