...
func (GoSuite) TestCompileErrorsLoadRuntimeForEagerCommands(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
...
`)
	...
	out, err = modGen.With(daggerExecFail("generate")).CombinedOutput(ctx)
	require.NoError(t, err)
	require.Contains(t, out, "cannot use 1")
