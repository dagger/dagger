...
`).
	...
	t.Run("develop command does not use .dagger when init was in current folder", func(ctx context.Context, t *testctx.T) {
		c := connect(ctx, t)

		entries, err := c.Container().From(golangImage).
			WithMountedFile(testCLIBinPath, daggerCliFile(t, c)).
			WithWorkdir("/work").
			With(daggerExec("init", "--name=bare", "--sdk=go")).
		...
		require.NoError(t, err)
		require.Subset(t, entries, []string{"dagger.gen.go", "dagger.json", "internal", "go.mod"})
	})
}
