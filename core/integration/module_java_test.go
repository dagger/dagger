...
// Scope: Java SDK module initialization, codegen, and runtime behavior.
// Intent: Keep Java module behavior stable while helper and command cleanup proceed separately.
...
func (JavaSuite) TestInit(_ context.Context, t *testctx.T) {
	t.Run("from upstream", func(ctx context.Context, t *testctx.T) {
		...
		modGen := c.Container().From(golangImage).
			...
			WithWorkdir("/work").
			With(daggerExec("init", "--sdk=github.com/dagger/dagger/sdk/java", "bare"))
	...
	t.Run("from alias", func(ctx context.Context, t *testctx.T) {
		...
		modGen := c.Container().From(golangImage).
			WithMountedFile(testCLIBinPath, daggerCliFile(t, c)).
			WithWorkdir("/work").
			With(daggerExec("init", "--sdk=java", "bare"))
	...
	t.Run("from alias with ref", func(ctx context.Context, t *testctx.T) {
		...
		modGen := c.Container().From(golangImage).
			WithMountedFile(testCLIBinPath, daggerCliFile(t, c)).
			WithWorkdir("/work").
			With(daggerExec("init", "--sdk=java@main", "bare"))
