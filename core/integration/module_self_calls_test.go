	...
	for _, tc := range tcs {
		t.Run(tc.sdk, func(ctx context.Context, t *testctx.T) {
			...
			if tc.sdk == "go" {
				...
				t.Run("can self-call through a secondary object type", func(ctx context.Context, t *testctx.T) {
					// The fixture's Box type is installed by the engine as
					// TestBox; the emitter must namespace it the same way or
					// the generated binding queries a type the engine doesn't
					// serve.
					out, err := modGen.
						With(daggerQueryAt(".", `{boxedTag}`)).
						Stdout(ctx)
					require.NoError(t, err)
					require.JSONEq(t, `{"boxedTag":"boxed"}`, out)
				})

				t.Run("can self-call with an object argument", func(ctx context.Context, t *testctx.T) {
					// Object args pass by ID with @expectedType; the emitter
					// must mirror that or the binding takes the wrong
					// parameter type.
					out, err := modGen.
						With(daggerQueryAt(".", `{relabeledTag}`)).
						Stdout(ctx)
					require.NoError(t, err)
					require.JSONEq(t, `{"relabeledTag":"relabeled"}`, out)
				})
			}
		})
	}
