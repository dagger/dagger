...
`,
		},
		{
			...
			source: `from typing import Annotated
import dagger
from dagger import DefaultGit, DefaultPath, function, object_type

@object_type
class Test:
	@function
	async def test_repo_local(self, git: Annotated[dagger.GitRepository, DefaultGit("./.git")]) -> str:
		return await self.commit_and_ref(git.head())

	@function
	async def test_repo_remote(self, git: Annotated[dagger.GitRepository, DefaultGit("https://github.com/dagger/dagger.git")]) -> str:
		return await self.commit_and_ref(git.tag("v0.18.2"))

	@function
	async def test_ref_local(self, git: Annotated[dagger.GitRef, DefaultGit("./.git")]) -> str:
		return await self.commit_and_ref(git)

	@function
	async def test_ref_remote(self, git: Annotated[dagger.GitRef, DefaultGit("https://github.com/dagger/dagger.git#v0.18.3")]) -> str:
		return await self.commit_and_ref(git)
...
export class Test {
	@func()
	async testRepoLocal(
		@argument({ defaultGit: "./.git" }) git: GitRepository,
	): Promise<string> {
	...
	async testRepoRemote(
		@argument({ defaultGit: "https://github.com/dagger/dagger.git" }) git: GitRepository,
	): Promise<string> {
	...
	@func()
	async testRefLocal(
		@argument({ defaultGit: "./.git" }) git: GitRef,
	): Promise<string> {
	...
	@func()
	async testRefRemote(
		@argument({ defaultGit: "https://github.com/dagger/dagger.git#v0.18.3" }) git: GitRef,
	): Promise<string> {
		return await this.commitAndRef(git)
	}

	async commitAndRef(git: DefaultGit): Promise<string> {
