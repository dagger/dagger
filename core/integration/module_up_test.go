...
func (ModuleSuite) TestDaggerUp(ctx context.Context, t *testctx.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			trafficPort:  "23100",
			daggerArgs:   []string{"call", "-m", ".", "ctr", "up"},
		...
		},
		{
			...
			trafficPort:  "",
			daggerArgs:   []string{"call", "-m", ".", "ctr", "up", "--random"},
			cachedModDir: modDirForAsContainerTests,
		},
		{
			...
			endpointFn:   daggerUpAndGetEndpoint,
			trafficPort:  "23101",
			daggerArgs:   []string{"call", "-m", ".", "ctr", "up", "--ports", fmt.Sprintf("23101:%s", defaultTrafficPortForContainerTests)},
			cachedModDir: modDirForAsContainerTests,
		},
		{
			...
			endpointFn:   daggerUpAndGetEndpoint,
			trafficPort:  "23102",
			daggerArgs:   []string{"call", "-m", ".", "--port", "23102", "ctr", "up", "--ports", "23102:23102"},
			cachedModDir: modDirForAsContainerTests,
		},
		{
			...
			endpointFn:   daggerUpAndGetEndpoint,
			trafficPort:  "23103",
			daggerArgs:   []string{"call", "-m", ".", "ctr", "without-exposed-port", "--port", defaultTrafficPortForContainerTests, "with-exposed-port", "--port", "23103", "up", "--args", "python,-m,http.server,23103", "--ports", "23103:23103"},
			cachedModDir: modDirForAsContainerTests,
		},
		{
			...
			endpointFn:   daggerUpAndGetEndpoint,
			trafficPort:  "23104",
			daggerArgs:   []string{"shell", "-m", ".", "-c", fmt.Sprintf("ctr | up --ports 23104:%s", defaultTrafficPortForContainerTests)},
			cachedModDir: modDirForAsContainerTests,
		},
		{
			name:         "service native",
			endpointFn:   daggerUpAndGetEndpoint,
			trafficPort:  "23200",
			daggerArgs:   []string{"call", "-m", ".", "ctr", "as-service", "up"},
		...
		},
		{
			...
			endpointFn:   daggerUpAndGetEndpointFromLogs,
			trafficPort:  "",
			daggerArgs:   []string{"call", "-m", ".", "ctr", "as-service", "up", "--random"},
			cachedModDir: modDirForAsServiceTests,
		},
		{
			...
			endpointFn:   daggerUpAndGetEndpoint,
			trafficPort:  "23201",
			daggerArgs:   []string{"call", "-m", ".", "ctr", "as-service", "up", "--ports", fmt.Sprintf("23201:%s", defaultTrafficPortForServiceTests)},
			cachedModDir: modDirForAsServiceTests,
		},
		{
			...
			endpointFn:   daggerUpAndGetEndpoint,
			trafficPort:  "23202",
			daggerArgs:   []string{"call", "-m", ".", "--port", "23202", "ctr", "as-service", "up", "--ports", "23202:23202"},
			cachedModDir: modDirForAsServiceTests,
		},
	}
...
func daggerUpInitModFn(ctx context.Context, t *testctx.T, defaultPort string) string {
	...
	_, err = hostDaggerExecRaw(ctx, t, modDir, "functions", "-m", ".")
	require.NoError(t, err)
