...
func (ProvisionSuite) TestImageDriverConfig(ctx context.Context, t *testctx.T) {
	for _, tc := range driverTestCases {
		t.Run(tc.name, func(ctx context.Context, t *testctx.T) {
			...
			out, err := dockerc.WithExec([]string{"dagger", "query", "-M"}, dagger.ContainerWithExecOpts{Stdin: "{engine{localCache{reservedSpace,maxUsedSpace,minFreeSpace}}}", InsecureRootCapabilities: true, DisableDaggerInDagger: true}).Stdout(ctx)
			require.NoError(t, err)
...
-----END CERTIFICATE-----`
			...
			`, InsecureRootCapabilities: true, DisableDaggerInDagger: true}).Stdout(ctx)
			require.NoError(t, err)
...
func detectEngineVersion(ctx context.Context, t *testctx.T, ctr *dagger.Container) string {
	out, err := ctr.
		...
		WithExec([]string{"dagger", "query", "-M"}, dagger.ContainerWithExecOpts{Stdin: "{version}", InsecureRootCapabilities: true, DisableDaggerInDagger: true}).
		Stdout(ctx)
	require.NoError(t, err)
