...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Test", dagger.TypeDefWithObjectOpts{SourceMap: dag.SourceMap("main.go", 8, 6)}).
					WithFunction(
						dag.Function("Fn",
							dag.TypeDef().WithListOf(dag.TypeDef().WithKind(dagger.TypeDefKindStringKind))).
							WithSourceMap(dag.SourceMap("main.go", 10, 1)).
							WithArg("s", dag.TypeDef().WithKind(dagger.TypeDefKindStringKind), dagger.FunctionWithArgOpts{SourceMap: dag.SourceMap("main.go", 10, 40)}))), nil
	default:
