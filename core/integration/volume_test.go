...
// TestSSHFSVolume validates the Volume end-to-end stack:
...
`).
	...
	// Readiness: attempt a few ssh connections before proceeding so the
	// sshfs mount doesn't race the service coming up.
	for i := range 10 {
		probe := c.Container().
			From(alpineImage).
