...
// cached value must not be served to later sessions: client-owned workspace
// results carry a per-client session-resource handle that gates results
// embedding them to the session that created them. A regression here surfaces
// on the second run as
