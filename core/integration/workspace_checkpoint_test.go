...
// Coverage for Workspace.snapshot: freezing a live client checkout into a
...
func (WorkspaceSuite) TestWorkspaceSnapshotFreezesLocalCheckout(ctx context.Context, t *testctx.T) {
	...
	git("add", ".")
	git("commit", "-m", "later")
	again := snapshotWorkspace(ctx, t, c, frozen)
	...
	fresh := snapshotWorkspace(ctx, t, c, live)
		...
		t.Run(name, func(ctx context.Context, t *testctx.T) {
			...
			for _, overlay := range []bool{false, true} {
				...
				for path, want := range files {
					got, err := snapshot.File(path).Contents(ctx)
					require.NoError(t, err)
					require.Equal(t, want, got)
				}
				if overlay {
					got, err := snapshot.File("overlay.txt").Contents(ctx)
					require.NoError(t, err)
					require.Equal(t, "overlay", got)
				}
				// Rootless workspaces intentionally have no host export target.
				if overlay && name != "non-repository" {
					require.NoError(t, snapshot.Export(ctx), "fallback retains direct overlay export")
...
func (WorkspaceSuite) TestWorkspaceSnapshotPortableCapture(ctx context.Context, t *testctx.T) {
	...
	require.NoError(t, err)
	for _, vertex := range dag.GetRecipe().CallsByDigest {
		require.NotEqual(t, "snapshot", vertex.Field)
...
`)
	...
	ws := snapshotWorkspace(ctx, t, c, root.AsWorkspace())
	id, err := ws.ID(ctx)
	require.NoError(t, err)
	var got struct {
		Node struct {
   ...
   artifacts { checks: filterCheckCommand { list: items { name: uri } } }
  } } }`,
...
func (WorkspaceSuite) TestWorkspaceSnapshotPreservesRootlessEffectiveTree(ctx context.Context, t *testctx.T) {
	workdir := t.TempDir()
	...
	for _, edited := range []bool{false, true} {
		...
		replay := snapshotWorkspace(ctx, t, c, frozen)
		...
		for _, value := range []*dagger.Workspace{frozen, replay} {
			...
			if edited {
				...
				require.Empty(t, entries, "rootless snapshots must not capture its host path")
			}
		}
	}
}
