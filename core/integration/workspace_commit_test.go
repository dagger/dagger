...
func (WorkspaceSuite) TestWorkspaceWithCommitScopedHistory(ctx context.Context, t *testctx.T) {
	...
	recipe, err := core.NewQuery(c).LLM().WithWorkspace(core.Ref[*core.Workspace](core.NewQuery(c), second.ID)).PortableID(ctx)
	require.NoError(t, err)
	id := new(call.ID)
...
func (WorkspaceSuite) TestWorkspaceWithCommitRestoresWithoutClient(ctx context.Context, t *testctx.T) {
	...
	restored = core.Ref[*core.LLM](core.NewQuery(restoring), core.ID(strings.TrimSpace(recipe))).Workspace()
...
`)
	...
	require.NoError(t, err)
	checks, err := core.Ref[*core.Workspace](core.NewQuery(c), committed.ID).Checks(core.WorkspaceChecksOpts{NoGenerate: true}).List(ctx)
	require.NoError(t, err)
...
func (WorkspaceSuite) TestWorkspaceWithCommitIncomingChanges(ctx context.Context, t *testctx.T) {
	...
	for _, dirty := range []bool{false, true} {
		...
		t.Run(name, func(ctx context.Context, t *testctx.T) {
			...
			recipe, err := core.NewQuery(c).LLM().WithWorkspace(got).PortableID(ctx)
			require.NoError(t, err)
