...
func (WorkspaceSuite) TestWorkspaceWithCommitFreezesHostAndAuthor(ctx context.Context, t *testctx.T) {
	...
	require.NoError(t, err)
	_, err = sink.captureLLMRecipe(ctx, t, c, c.LLM().WithWorkspace(dagger.Ref[*dagger.Workspace](c, committed.ID)))
	require.ErrorContains(t, err, "Host.__gitDir", "commit normalization does not make local repository history portable")
