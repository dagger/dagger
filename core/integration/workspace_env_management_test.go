...
`)
		...
		out, err := hostDaggerExecRaw(ctx, t, subdir, "--env=dev", "module", "install", "../dep", "--here")
		require.NoError(t, err)
		outStr := string(out)
		require.Contains(t, outStr, `Created env "dev"`)
		require.Contains(t, outStr, `Installed module "dep" into env "dev"`)
...
`)
		...
		_, err := hostDaggerExecRaw(ctx, t, workdir, "--env=dev", "module", "install", "./dep")
		require.NoError(t, err)
...
`)
		...
		out, err := hostDaggerExecRaw(ctx, t, workdir, "--env=dev", "module", "list")
		require.NoError(t, err)
		require.Contains(t, string(out), "dep")

		out, err = hostDaggerExecRaw(ctx, t, workdir, "module", "list")
		require.NoError(t, err)
...
`)
		...
		out, err := hostDaggerExecRaw(ctx, t, workdir, "--env=dev", "module", "uninstall", "other")
		require.NoError(t, err)
		...
		_, err = hostDaggerExecRaw(ctx, t, workdir, "--env=dev", "module", "uninstall", "dep")
		require.Error(t, err)
		requireErrOut(t, err, `module "dep" is not installed in env "dev"`)

		// And a missing env stays a strict error for uninstall.
		_, err = hostDaggerExecRaw(ctx, t, workdir, "--env=missing", "module", "uninstall", "dep")
		require.Error(t, err)
		requireErrOut(t, err, `workspace env "missing" is not defined`)
	})
...
`)
		...
		out, err := hostDaggerExecRaw(ctx, t, workdir, "--env=dev", "module", "install", "--name=dep", "./fork")
		require.NoError(t, err)
		outStr := string(out)
		require.Contains(t, outStr, `Installed module "dep" into env "dev"`)
		...
		out, err = hostDaggerExecRaw(ctx, t, workdir, "--env=dev", "module", "install", "--name=other", "./other")
		require.NoError(t, err)
...
`)
		...
		_, err = hostDaggerExecRaw(ctx, t, workdir, "--env=dev", "module", "install", sdkModulePath)
		require.Error(t, err)
