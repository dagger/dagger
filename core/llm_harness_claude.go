...
func (a *ClaudeLLMHarnessAdapter) Interrupt(ctx context.Context, nativeTurnID string, cancelQueued bool) error {
	...
	if err := a.control(ctx, request); err != nil {
		return err
	}
	if !cancelQueued {
		return nil
	...
	err := a.control(ctx, map[string]any{"subtype": "cancel_queued"})
	return err
}
...
func (a *ClaudeLLMHarnessAdapter) CancelQueued(ctx context.Context, messageID string) error {
	...
	err = a.control(ctx, map[string]any{"subtype": "cancel_async_message", "uuid": vendorID})
	return err
}
...
func (a *ClaudeLLMHarnessAdapter) control(ctx context.Context, request map[string]any) error {
...
func (a *ClaudeLLMHarnessAdapter) readLoop() {
	...
	for {
		...
		if err := json.Unmarshal(record, &header); err != nil {
			a.failProtocol(fmt.Errorf("%w: decode Claude frame: %w", ErrLLMHarnessProtocolFailure, err))
			return
		}
		if header.Type == "" {
...
func (a *ClaudeLLMHarnessAdapter) deliverControl(record json.RawMessage) error {
	...
	if err := json.Unmarshal(record, &frame); err != nil {
		return fmt.Errorf("%w: decode Claude control response: %w", ErrLLMHarnessProtocolFailure, err)
	}
...
func (a *ClaudeLLMHarnessAdapter) claudeLifecycle(record json.RawMessage) ([]LLMHarnessEvent, error) {
	...
	if err := json.Unmarshal(record, &frame); err != nil {
		return nil, fmt.Errorf("%w: decode Claude command_lifecycle: %w", ErrLLMHarnessProtocolFailure, err)
	}
...
func (a *ClaudeLLMHarnessAdapter) claudeAssistant(record json.RawMessage) ([]LLMHarnessEvent, error) {
	...
	if err := json.Unmarshal(record, &frame); err != nil {
		return nil, fmt.Errorf("%w: decode Claude assistant: %w", ErrLLMHarnessProtocolFailure, err)
	}
...
func (a *ClaudeLLMHarnessAdapter) claudePartial(record json.RawMessage) ([]LLMHarnessEvent, error) {
	...
	if err := json.Unmarshal(record, &frame); err != nil {
		return nil, fmt.Errorf("%w: decode Claude stream_event: %w", ErrLLMHarnessProtocolFailure, err)
	}
...
func (a *ClaudeLLMHarnessAdapter) claudeResult(subtype string, record json.RawMessage) ([]LLMHarnessEvent, error) {
	...
	if err := json.Unmarshal(record, &frame); err != nil {
		return nil, fmt.Errorf("%w: decode Claude result: %w", ErrLLMHarnessProtocolFailure, err)
	}
	if subtype == "" {
