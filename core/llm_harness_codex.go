...
func (a *CodexLLMHarnessAdapter) Steer(ctx context.Context, nativeTurnID string, input LLMHarnessInput) error {
	...
	if err != nil && codexIsTurnMismatch(err) {
		return fmt.Errorf("%w: %w", ErrCodexTurnMismatch, err)
	}
	return err
}
...
func (a *CodexLLMHarnessAdapter) call(ctx context.Context, method string, params any, dst any) error {
	...
	case response := <-result:
		...
		if err := json.Unmarshal(response.response, dst); err != nil {
			return fmt.Errorf("%w: decode Codex %s response: %w", ErrLLMHarnessProtocolFailure, method, err)
		}
		return nil
	case <-ctx.Done():
		a.removePending(id)
...
func (a *CodexLLMHarnessAdapter) readLoop() {
	...
	for {
		...
		if err := json.Unmarshal(record, &envelope); err != nil {
			a.failProtocol(fmt.Errorf("%w: decode Codex frame: %w", ErrLLMHarnessProtocolFailure, err))
			return
		}
		if envelope.ID != nil {
...
//nolint:gocyclo // Keep vendor item variants together at the protocol decoding boundary.
func (a *CodexLLMHarnessAdapter) codexItemEvent(method string, params json.RawMessage) ([]LLMHarnessEvent, error) {
	...
	if err := json.Unmarshal(p.Item, &item); err != nil {
		return nil, fmt.Errorf("%w: decode Codex %s item: %w", ErrLLMHarnessProtocolFailure, method, err)
	}
	...
	case "mcpToolCall":
		...
		if err != nil {
			return nil, fmt.Errorf("%w: decode Codex MCP tool %q result: %w", ErrLLMHarnessProtocolFailure, item.Tool, err)
		}
	...
	case "subAgentActivity":
		...
		if err != nil {
			return nil, fmt.Errorf("%w: encode Codex sub-agent activity: %w", ErrLLMHarnessProtocolFailure, err)
		}
		if started {
	...
	case "collabAgentToolCall":
		if started {
			...
			if err != nil {
				return nil, fmt.Errorf("%w: encode Codex collaboration call: %w", ErrLLMHarnessProtocolFailure, err)
			}
		...
		if err != nil {
			return nil, fmt.Errorf("%w: encode Codex collaboration result: %w", ErrLLMHarnessProtocolFailure, err)
		}
...
func (a *CodexLLMHarnessAdapter) isPrimaryThreadNotification(params json.RawMessage) (bool, error) {
	...
	if err := json.Unmarshal(params, &scope); err != nil {
		return false, fmt.Errorf("%w: decode Codex notification scope: %w", ErrLLMHarnessProtocolFailure, err)
	}
	if scope.ThreadID == "" {
		return true, nil
	}
...
func decodeHarnessParams(method string, raw json.RawMessage, dst any) error {
	...
	if err := json.Unmarshal(raw, dst); err != nil {
		return fmt.Errorf("%w: decode %s notification: %w", ErrLLMHarnessProtocolFailure, method, err)
	}
	return nil
}
