...
func TestCodexHarnessAuthOfferPrefersOAuth(t *testing.T) {
	...
	router := &LLMRouter{
		...
		reloadCodexAuthToken: func(ctx context.Context) (Credential, error) {
			rejected := secretprovider.RejectedEnvValue(ctx)
			forces = append(forces, rejected != "")
			if rejected != "" {
				require.Equal(t, fmt.Sprintf("%x", sha256.Sum256([]byte(current))), rejected)
