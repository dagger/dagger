...
// that the model discovers with ListSkills and reads with ReadSkill. This gives
...
// the one line the model reads in ListSkills to decide whether to open the skill.
...
func (m *MCP) loadSkillTools(srv *dagql.Server, allTools *LLMToolSet) {
	...
	allTools.Add(LLMTool{
		...
		Description: "Read a skill's guidance. With just a name, returns its " +
			...
			"points to, e.g. ReadSkill(\"dang-language\", \"reference/objects.md\").",
		ReadOnly: true,
		Schema: map[string]any{
			"type": "object",
			"properties": map[string]any{
				"name": map[string]any{
					"type":        "string",
					"description": "The skill name, as shown by ListSkills.",
				},
				"file": map[string]any{
					"type": "string",
