...
func (m *MCP) Tools(ctx context.Context) ([]LLMTool, error) {
	...
	served, err := query.CurrentServedDeps(ctx)
	if err != nil {
		return nil, err
	}
	servedSrv, err := served.Server(ctx)
	if err != nil {
		return nil, err
	}
	...
	if m.staticTools {
		if err := m.loadServedModuleQueryTools(ctx, servedSrv, callMethods); err != nil {
			return nil, err
		}
	} else if err := m.loadServedModuleQueryTools(ctx, servedSrv, allTools); err != nil {
		return nil, err
	}
	if m.staticTools {
	...
	if err := m.loadReachableObjectMethods(envSrv, objectMethods); err != nil {
		return nil, err
	}
	if m.staticTools {
...
func (m *MCP) loadServedModuleQueryTools(ctx context.Context, srv *dagql.Server, allTools *LLMToolSet) error {
	...
	for _, typeDef := range typeDefs {
		...
		if typeDefSelf == nil ||
			...
			typeDefSelf.AsObject.Value == nil ||
			typeDefSelf.AsObject.Value.Name != "Query" {
			continue
		}
		queryTypeDef = typeDefSelf.AsObject.Value
		break
	}
	if queryTypeDef == nil {
		return nil
	...
	for _, fn := range queryTypeDef.Functions {
		if fn == nil || fn.SourceModuleName == "" && fn.Name != "with" {
			continue
		}

		fieldDef := queryDef.Fields.ForName(fn.Name)
		if fieldDef == nil {
			return fmt.Errorf("query field %q not found in schema", fn.Name)
		}
