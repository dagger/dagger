...
func (fn *ModuleFunction) setCallInputs(ctx context.Context, opts *CallOpts) ([]*FunctionCallArgValue, error) {
	...
	for _, arg := range fn.metadata.Args {
		...
		localDefault, hasLocalDefault, err := fn.LocalDefault(ctx, arg.Name)
		if err != nil {
			return nil, fmt.Errorf("load local defaults for function %q: %w", fn.metadata.Name, err)
		}
		hasModuleDefault := (arg.DefaultValue != nil)

		var defaultInput *FunctionCallArgValue
		if hasLocalDefault {
			// 1. User-defined local default
			localDefaultInput, err := localDefault.CallInput()
			if err != nil {
				return nil, err
			}
			defaultInput = localDefaultInput
...
// Load the local defaults for this function, and apply them to the function's
// typedefs. This makes the local defaults visible in typedef introspection.
// It does not affect applying local defaults *at function call*
func (fn *ModuleFunction) mergeLocalDefaultsTypeDefs(ctx context.Context) error {
	for argName, arg := range fn.args {
		argDefault, ok, err := fn.LocalDefault(ctx, argName)
		if err != nil {
			return fmt.Errorf("load local default for %s.%s: %w", fn.mod.NameField, fn.metadata.Name, err)
		}
		if !ok {
			continue
		}
		console(ctx, "applying user default: %s.%s: %s=%q", fn.mod.Name(), fn.metadata.Name, argName, argDefault.UserInput)
...
func (udp *UserDefaultPrimitive) errorf(err error, msg string, args ...any) error {
	fullMessage := fmt.Sprintf("local defaults %s.%s(%s=...): %s",
...
func (fn *ModuleFunction) LocalDefault(ctx context.Context, argName string) (*UserDefault, bool, error) {
	...
	defaults, err := fn.LocalDefaults(mainCtx)
	if err != nil {
...
func (fn *ModuleFunction) LocalDefaults(ctx context.Context) (*EnvFile, error) {
	objDefaults, err := fn.mod.ObjectLocalDefaults(ctx, fn.objDef.OriginalName)
	if err != nil {
		return nil, err
	}
	...
	return objDefaults.LookupPrefix(ctx, fn.metadata.OriginalName)
}
...
func (fn *ModuleFunction) CacheConfigForCall(
	...
	for _, argMetadata := range fn.metadata.Args {
		...
		if argMetadata.TypeDef.Kind != TypeDefKindObject {
			...
			//  2) "local defaults" from user-defined .env
			continue
		}
		localDefault, hasLocalDefault, err := fn.LocalDefault(ctx, argMetadata.Name)
		if err != nil {
			return nil, fmt.Errorf("%s.%s(%s=): load local default: %w",
				fn.mod.Name(),
				fn.metadata.Name,
				argMetadata.Name,
				err,
		...
		if hasLocalDefault {
			localDefaults = append(localDefaults, localDefault)
		} else if argMetadata.isContextual() {
			ctxArgs = append(ctxArgs, argMetadata)
		}
	}

	if len(ctxArgs) > 0 || len(localDefaults) > 0 {
		...
		// Process user-defined local defaults for objects
		localDefaultVals := make([]*argInput, len(localDefaults))
		for i, localDefault := range localDefaults {
			i, localDefault := i, localDefault
			eg.Go(func() error {
				id, err := localDefault.DagqlID(ctx)
				if err != nil {
					return err
				}
				arg := localDefault.Arg
				localDefaultVals[i] = &argInput{
					name: arg.OriginalName,
					val:  id,
				}
				mu.Lock()
		...
		for _, arg := range localDefaultVals {
			if arg != nil {
				dgstInputs = append(dgstInputs, arg.name, arg.val.ID().Digest().String())
			}
		}
