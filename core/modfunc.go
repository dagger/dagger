...
func (fn *ModuleFunction) loadContextualArg(
	...
	case arg.DefaultGit != "":
		...
		if filepath.Clean(arg.DefaultGit) == "." || filepath.Clean(arg.DefaultGit) == ".git" {
		...
		} else if gitURL, err := gitutil.ParseURL(arg.DefaultGit); err == nil {
			...
			return nil, fmt.Errorf("failed to parse git URL %q: %w", arg.DefaultGit, err)
		}
