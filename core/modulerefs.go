...
func FastModuleSourceKindCheck(
	...
	switch moduleref.FastKind(refString, refPin) {
	case moduleref.KindGit:
		return ModuleSourceKindGit
	case moduleref.KindLocal:
...
type gitEndpointError = moduleref.EndpointError
...
func ParseGitRefString(ctx context.Context, refString string) (_ ParsedGitRefString, rerr error) {
	...
	parsed, err := moduleref.ParseGit(refString)
	if err != nil {
		return ParsedGitRefString{}, err
	}
	return ParsedGitRefString{
		modPath:        parsed.ModPath,
		ModVersion:     parsed.ModVersion,
		hasVersion:     parsed.HasVersion,
		RepoRoot:       parsed.RepoRoot,
		RepoRootSubdir: parsed.RepoRootSubdir,
		scheme:         parsed.Scheme,
		sourceUser:     parsed.SourceUser,
		cloneUser:      parsed.CloneUser,
		SourceCloneRef: parsed.SourceCloneRef,
		cloneRef:       parsed.CloneRef,
	}, nil
}
