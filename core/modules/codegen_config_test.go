...
func TestModuleCodegenConfig_Clone(t *testing.T) {
	...
	*clone.LegacyCodegenAtRuntime = true

	if *orig.AutomaticGitignore != true {
		t.Errorf("Clone aliased AutomaticGitignore: got %v, want true", *orig.AutomaticGitignore)
	}
	if *orig.LegacyCodegenAtRuntime != false {
		t.Errorf("Clone aliased LegacyCodegenAtRuntime: got %v, want false", *orig.LegacyCodegenAtRuntime)
	}
}
