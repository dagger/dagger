...
type ModuleSource struct {
	...
	LocalDefaults   *EnvFile                          `field:"true" name:"localDefaults" doc:"User-defined defaults read from local .env files"`
...
// loadocalDefaults loads and merges environment files for local module defaults.
...
func (src *ModuleSource) LoadLocalDefaults(ctx context.Context) error {
	...
	outerForName, err := outerEnvFile.LookupPrefix(ctx, src.ModuleName)
	if err != nil {
		return err
	}
	outerForOriginalName, err := outerEnvFile.LookupPrefix(ctx, src.ModuleOriginalName)
	if err != nil {
		return err
	}
	src.LocalDefaults = NewEnvFile(true).WithEnvFiles(innerEnvFile, outerForName, outerForOriginalName)
	return nil
}
...
func (src *ModuleSource) CalcDigest() digest.Digest {
	...
	// Include local defaults in digest so changes to env files invalidate cache
	if src.LocalDefaults != nil {
		vars, _ := src.LocalDefaults.Variables(context.Background(), false)
		for _, v := range vars {
