...
func (t *ModuleObjectType) ConvertFromSDKResult(ctx context.Context, value any) (dagql.Value, error) {
	if value == nil {
...
func (t *ModuleObjectType) ConvertToSDKInput(ctx context.Context, value dagql.Typed) (any, error) {
	...
	case DynamicID:
		...
		switch x := val.(type) {
		case dagql.ObjectInstance[*ModuleObject]:
			return x.Self().Fields, nil
		case dagql.ObjectInstance[*InterfaceAnnotatedValue]:
			return x.Self().Fields, nil
		default:
...
type Callable interface {
	Call(context.Context, *CallOpts) (dagql.Value, error)
	ReturnType() (ModType, error)
	ArgType(argName string) (ModType, error)
	CacheConfigForCall(context.Context, dagql.Value, map[string]dagql.Input, dagql.View, dagql.CacheConfig) (*dagql.CacheConfig, error)
}
...
func (obj *ModuleObject) installConstructor(ctx context.Context, dag *dagql.Server) error {
	...
	if !objDef.Constructor.Valid {
		...
		dag.Root().ObjectType().Extend(
			spec,
			func(ctx context.Context, self dagql.Value, _ map[string]dagql.Input) (dagql.Value, error) {
				return dagql.NewInstanceForCurrentID(ctx, &ModuleObject{
					Module:  mod,
					TypeDef: objDef,
					Fields:  map[string]any{},
...
func objField(mod *Module, field *FieldTypeDef) dagql.Field[*ModuleObject] {
	...
	return dagql.Field[*ModuleObject]{
		Spec: spec,
		Func: func(ctx context.Context, obj dagql.ObjectInstance[*ModuleObject], _ map[string]dagql.Input, view dagql.View) (dagql.Value, error) {
...
func objFun(ctx context.Context, mod *Module, objDef *ObjectTypeDef, fun *Function, dag *dagql.Server) (dagql.Field[*ModuleObject], error) {
	...
	return dagql.Field[*ModuleObject]{
		Spec: &spec,
		Func: func(ctx context.Context, obj dagql.ObjectInstance[*ModuleObject], args map[string]dagql.Input, view dagql.View) (dagql.Value, error) {
			opts := &CallOpts{
				ParentTyped:  obj,
...
func (f *CallableField) Call(ctx context.Context, opts *CallOpts) (dagql.Value, error) {
