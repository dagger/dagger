...
func (t *ModuleObjectType) CollectIDs(ctx context.Context, value dagql.Typed, ids map[digest.Digest]*call.ID) error {
	obj, ok := value.(*ModuleObject)
	if !ok {
	...
	for k, v := range obj.Fields {
		...
		if err := modType.CollectIDs(ctx, typed, ids); err != nil {
