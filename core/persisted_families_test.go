...
func (env *persistedFamiliesTestEnv) open(t *testing.T) (context.Context, *dagql.Cache, *dagql.Server) {
	...
	for _, class := range []dagql.ObjectType{
		...
		dagql.NewClass(srv, dagql.ClassOpts[*Artifact]{}),
		dagql.NewClass(srv, dagql.ClassOpts[*Artifacts]{}),
		dagql.NewClass(srv, dagql.ClassOpts[*Address]{}),
		dagql.NewClass(srv, dagql.ClassOpts[*WorkspaceSDK]{}),
