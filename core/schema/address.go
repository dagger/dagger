...
func (s *addressSchema) gitRepository(
	...
	repo := inst.Self().Clone()
	repo.OriginalAddress = r.Self()
...
func (s *addressSchema) service(
	...
	service := inst.Self().Clone()
	service.OriginalAddress = r.Self()
	return dagql.NewObjectResultForCurrentID(ctx, srv, service)
}
