...
func (s *containerSchema) Install(srv *dagql.Server) {
	...
	dagql.Fields[*core.Container]{
		...
		dagql.NodeFunc("withErrorMessage", s.withErrorMessage).
			Doc(`Raise an error. All subsequent calls will be ignored.`).
			Args(
				dagql.Arg("err").Doc(`Message of the error to raise.`),
			),
...
func (s *containerSchema) withErrorMessage(ctx context.Context, parent dagql.ObjectResult[*core.Container], args struct{ Err string }) (dagql.ObjectResult[*core.Container], error) {
	_ = ctx
	if args.Err == "" {
		return parent, nil
	}
