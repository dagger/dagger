...
func (s *containerSchema) withLocalhostForward(ctx context.Context, parent *core.Container, args containerWithLocalhostForwardArgs) (*core.Container, error) {
	srv, err := core.CurrentDagqlServer(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get server: %w", err)
	}
	...
	return parent.WithLocalhostForward(ctx, args.Port, svc, args.ServicePort)
}
