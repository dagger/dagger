...
func (s *containerSchema) load(
	...
	if loader.Tarball != nil {
		// create the tarball
		var tarball dagql.Instance[*core.File]
		sel := dagql.Selector{
			Field: "asTarball",
			Args: []dagql.NamedInput{
				{
		...
		err = tarball.Self.Mount(ctx, func(path string) error {
			...
			_, err = io.CopyBuffer(loader.Tarball, f, buf)
			if err != nil {
				loader.Tarball.Close()
				return err
			}
			return loader.Tarball.Close()
		})
		return core.Void{}, err
	}
