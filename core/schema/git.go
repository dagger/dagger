...
func (s *gitSchema) ref(ctx context.Context, parent dagql.ObjectResult[*core.GitRepository], args refArgs) (inst dagql.Result[*core.GitRef], _ error) {
	...
	dgstInputs := []string{
		...
		result.Repo.URL.Value.String(),
		string(result.Repo.Remote.Digest()),
		// resolved commit details
		result.Ref.Name,
		result.Ref.SHA,
		// legacy args
		strconv.FormatBool(result.Repo.DiscardGitDir),
	}
	...
	fmt.Println(inst.ID().Digest(), dgstInputs)
	return inst, nil
}
...
func (s *gitSchema) commonAncestor(
	...
	ref, backend, err := core.MergeBase(ctx, parent.Self(), other.Self())
	if err != nil {
		return inst, err
	}
	// XXX: return from MergeBase directly
	result := &core.GitRef{
