...
func (s *hostSchema) Install(srv *dagql.Server) {
	...
	dagql.Fields[*core.Host]{
		...
		dagql.NodeFuncWithCacheKey("container", s.container, dagql.CachePerClient).
			Doc(`Accesses a container on the host.`).
			Args(
...
func loadDirectoryGitIgnorePatterns(ctx context.Context, parentPath string, hostPath string) (patterns []string, rerr error) {
	...
	gitIgnorePatterns, err := core.LoadGitIgnoreInDirectory(ctx, directory)
	if err != nil {
...
func (s *hostSchema) container(ctx context.Context, parent dagql.ObjectResult[*core.Host], args hostContainerArgs) (inst dagql.Result[*core.Container], err error) {
