...
func (s *httpSchema) http(ctx context.Context, parent dagql.ObjectInstance[*core.Query], args httpArgs) (inst dagql.Instance[*core.File], rerr error) {
	if args.InDagOp() {
		...
		f.Result = snap
		return dagql.NewInstanceForCurrentID(ctx, f)
	}
