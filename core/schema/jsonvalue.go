...
func (s jsonvalueSchema) Install(srv *dagql.Server) {
	...
	dagql.Fields[*core.Query]{
		dagql.Func("json", s.newJSONValue).
			Doc("Initialize a mutable JSON value"),
	}.Install(srv)

	// JSONValue methods
	dagql.Fields[*core.JSONValue]{
		dagql.Func("bytes", s.bytes).Doc("Return the value encoded as json").Args(
			dagql.Arg("pretty").Doc("Pretty-print"),
			dagql.Arg("indent").Doc("Indent each line by the given number of whitespaces"),
		),
		dagql.Func("withBytes", s.withBytes).Doc("Return a new json value, decoded from the given bytes").Args(
			dagql.Arg("bytes"),
		),
...
func (s jsonvalueSchema) bytes(ctx context.Context, obj *core.JSONValue, args struct {
	Pretty dagql.Optional[dagql.Boolean]
	Indent dagql.Optional[dagql.Int] `default:"1"`
}) (core.JSON, error) {
	if args.Pretty.Valid && args.Pretty.Value.Bool() {
		...
		if args.Indent.Valid {
			indent = strings.Repeat(" ", args.Indent.Value.Int())
		}
...
func (s jsonvalueSchema) withBytes(ctx context.Context, obj *core.JSONValue, args struct {
	Bytes core.JSON
}) (*core.JSONValue, error) {
	// Validate JSON
	var v any
	if err := json.Unmarshal([]byte(args.Bytes), &v); err != nil {
		return nil, fmt.Errorf("invalid JSON: %w", err)
	}
	return &core.JSONValue{Data: []byte(args.Bytes)}, nil
}
...
func (s jsonvalueSchema) asInteger(ctx context.Context, obj *core.JSONValue, args struct{}) (dagql.Int, error) {
	...
	switch val := v.(type) {
	case float64:
		return dagql.Int(int(val)), nil
	case int:
		return dagql.Int(val), nil
	default:
...
func (s jsonvalueSchema) field(ctx context.Context, obj *core.JSONValue, args struct {
	...
	for _, pathSegment := range args.Path {
		...
		if !ok {
			return nil, fmt.Errorf("path not found: not an object")
		}
		val, exists := m[pathSegment.String()]
		if !exists {
			return nil, fmt.Errorf("path not found: key %s does not exist", pathSegment.String())
		}
		current = val
	}
