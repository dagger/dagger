...
func (s *moduleSourceSchema) Install() {
	...
	dagql.Fields[*core.ModuleSource]{
		// TODO: clean, had to migrate from struct field for now
		dagql.NodeFunc("contextDirectory", func(ctx context.Context, src dagql.ObjectInstance[*core.ModuleSource], args struct{}) (dagql.Instance[*core.Directory], error) {
			return src.Self().ContextDirectory, nil
		}).
	...
	dagql.Fields[*core.GeneratedCode]{
		// TODO: cleanup
		dagql.NodeFunc("code", func(ctx context.Context, code dagql.ObjectInstance[*core.GeneratedCode], args struct{}) (dagql.Instance[*core.Directory], error) {
			return code.Self().Code, nil
		}).
...
func (s *moduleSourceSchema) moduleSource(
	ctx context.Context,
	query dagql.ObjectInstance[*core.Query],
...
func (s *moduleSourceSchema) localModuleSource(
	...
	if !daggerCfgFound {
		...
		var srcRootDir dagql.ObjectInstance[*core.Directory]
		...
		localSrc.Dependencies = make([]dagql.ObjectInstance[*core.ModuleSource], len(localSrc.ConfigDependencies))
...
func (s *moduleSourceSchema) gitModuleSource(
	...
	gitSrc.Dependencies = make([]dagql.ObjectInstance[*core.ModuleSource], len(gitSrc.ConfigDependencies))
...
func (s *moduleSourceSchema) directoryAsModule(
	ctx context.Context,
	contextDir dagql.ObjectInstance[*core.Directory],
...
func (s *moduleSourceSchema) directoryAsModuleSource(
	...
	dirSrc.Dependencies = make([]dagql.ObjectInstance[*core.ModuleSource], len(dirSrc.ConfigDependencies))
...
func (s *moduleSourceSchema) moduleSourceDirectory(
	ctx context.Context,
	src dagql.ObjectInstance[*core.ModuleSource],
	args struct {
		Path string
	},
...
func (s *moduleSourceSchema) moduleSourceWithDependencies(
	...
	var allDeps []dagql.ObjectInstance[*core.ModuleSource]
	...
	symbolicDeps := make(map[string]dagql.ObjectInstance[*core.ModuleSource], len(allDeps))
	depNames := make(map[string]dagql.ObjectInstance[*core.ModuleSource], len(allDeps))
	...
	finalDeps := make([]dagql.ObjectInstance[*core.ModuleSource], 0, len(symbolicDeps))
...
func (s *moduleSourceSchema) moduleSourceWithUpdateDependencies(
	ctx context.Context,
	parentSrc dagql.ObjectInstance[*core.ModuleSource],
	args struct {
		Dependencies []string
	},
	...
	for _, existingDep := range parentSrc.Self().Dependencies {
		...
		if len(updateReqs) == 0 {
			...
			var updatedDep dagql.ObjectInstance[*core.ModuleSource]
...
func (s *moduleSourceSchema) moduleSourceWithoutDependencies(
	...
	var filteredDeps []dagql.ObjectInstance[*core.ModuleSource]
...
func (s *moduleSourceSchema) runCodegen(
	ctx context.Context,
	srcInst dagql.ObjectInstance[*core.ModuleSource],
) (dagql.ObjectInstance[*core.Directory], error) {
...
func (s *moduleSourceSchema) runClientGenerator(
	ctx context.Context,
	srcInst dagql.ObjectInstance[*core.ModuleSource],
	genDirInst dagql.ObjectInstance[*core.Directory],
	...
	var source dagql.ObjectInstance[*core.ModuleSource]
	...
	if srcInst.Self().SDK != nil {
		...
		if _, ok := srcInst.Self().SDKImpl.AsRuntime(); ok {
			var mod dagql.ObjectInstance[*core.Module]
...
func (s *moduleSourceSchema) moduleSourceGeneratedContextDirectory(
	...
	if err != nil {
		return nil, fmt.Errorf("failed to load module source config: %w", err)
	}
	...
	for _, client := range modCfg.Clients {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to generate client %s: %w", client.Generator, err)
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to add updated dagger.json to context dir: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get context dir diff: %w", err)
	}

	return genDirInst, nil
}

func (s *moduleSourceSchema) runModuleDefInSDK(ctx context.Context, src, srcInstContentHashed dagql.ObjectInstance[*core.ModuleSource], mod *core.Module) (*core.Module, error) {
