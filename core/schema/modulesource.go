...
func (s *moduleSourceSchema) moduleSourceGenerateLocalDependencies(
	...
	// [sdks.<name>.claimed].modules in dagger.toml.
...
func (s *moduleSourceSchema) generateOneLocalDependency(
	...
	if !ok {
		...
		// downstream; this warning is the breadcrumb for a missing SDK claim.
...
// workspace-relative path to the installed module providing its SDK, read from
// sdks.<name>.claimed.modules. The provider name is the generator include
// selector (e.g. "dagger-go-sdk").
...
func sdkOwnersByModulePathFromConfig(configDir string, cfg *workspace.Config) (map[string]string, error) {
	...
	for sdkName, entry := range cfg.SDKs {
		for _, managedPath := range entry.Claimed.Modules {
			path, err := workspace.ResolveSDKManagedPath(configDir, managedPath)
			if err != nil {
				return nil, fmt.Errorf("module managed by %q: %w", sdkName, err)
			}
			if existing, ok := owners[path]; ok && existing != entry.Module {
				moduleNames := []string{existing, entry.Module}
