...
func (s *moduleSourceSchema) moduleSourceAsModule(
	...
	if src.Self.ModuleName == "" || (src.Self.SDK != nil && src.Self.SDK.Source == "") {
		return inst, fmt.Errorf("module name must be set and SDK source must be present when SDK is configured")
	}
	...
	if err != nil {
		return inst, fmt.Errorf("failed to create instance for module %q: %w", mod.NameField, err)
	}
