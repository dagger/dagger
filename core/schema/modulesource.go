...
func (s *moduleSourceSchema) Install(dag *dagql.Server) {
	...
	dagql.Fields[*core.ModuleSource]{
		...
		dagql.NodeFunc("generateLocalDependencies", s.moduleSourceGenerateLocalDependencies).
			View(AfterVersion("v1.0.0-0")).
			Doc(`Generate this module's transitive local dependency closure, leaf-first, and return the accumulated changeset.`,
...
// transitive local dependency closure, leaf-first, and returns the accumulated
// changeset. For each local dependency the engine runs the workspace's SDK
// generators against a workspace scoped to that dependency (its path as cwd)
// with the already-generated dependencies overlaid, so a dependency that itself
// has local dependencies sees them. Remote (git) dependencies are assumed
// committed and are skipped. The result is meant to be overlaid onto the
// workspace (withChanges) before generating this module; it is not this
// module's own generated code.
func (s *moduleSourceSchema) moduleSourceGenerateLocalDependencies(
	...
	for _, dep := range deps {
		...
		depChanges, err := s.generateOneLocalDependency(ctx, dag, workspace, depPath, acc, owners)
		if err != nil {
			return res, fmt.Errorf("generate local dependency %q: %w", depPath, err)
		}
		if depChanges.Self() == nil {
			continue // dependency not managed by any SDK; skipped with a warning
		}
		if acc.Self() == nil {
			acc = depChanges
			continue
...
// workspace scoped to depPath (as cwd) with acc overlaid, and returns that
// dependency's changeset. Returns a nil result when no SDK claims the
...
func (s *moduleSourceSchema) generateOneLocalDependency(
	...
	wsD, err := scopedStagedWorkspace(ctx, dag, workspace, depPath, acc)
	if err != nil {
...
// mergeChangesets folds other into base via the withChangeset field so the
// result stays an attached dagql result (see mergeWorkspaceInitChangeset).
func mergeChangesets(
	ctx context.Context,
	dag *dagql.Server,
	base dagql.ObjectResult[*core.Changeset],
	other dagql.ObjectResult[*core.Changeset],
) (dagql.ObjectResult[*core.Changeset], error) {
	if other.Self() == nil {
		return base, nil
	}
	...
	if err := dag.Select(ctx, base, &merged, dagql.Selector{
		Field: "withChangeset",
		Args: []dagql.NamedInput{
			{Name: "changes", Value: dagql.NewID[*core.Changeset](otherID)},
