...
func (s *projectSchema) loadProject(ctx *router.Context, parent *core.Directory, args loadProjectArgs) (*Project, error) {
	projectState, err := project.Load(ctx, parent, args.ConfigPath, s.projectStates, &s.mu, s.gw, s.cacheImports)
	if err != nil {
		return nil, err
	}
...
func (s *projectSchema) schema(ctx *router.Context, parent *Project, args any) (string, error) {
	...
	return projectState.Schema(ctx, s.gw, s.platform, s.sshAuthSockID, s.cacheImports)
}
...
func (s *projectSchema) extensions(ctx *router.Context, parent *Project, args any) ([]*Project, error) {
	...
	dependencies, err := projectState.Extensions(ctx, s.projectStates, &s.mu, s.gw, s.platform, s.sshAuthSockID, s.cacheImports)
	if err != nil {
		return nil, err
	}
...
func (s *projectSchema) projectToExecutableSchema(ctx context.Context, projectState *project.State) (router.ExecutableSchema, error) {
	schema, err := projectState.Schema(ctx, s.gw, s.platform, s.sshAuthSockID, s.cacheImports)
	if err != nil {
		return nil, err
	}

	resolvers, err := projectState.Resolvers(ctx, s.gw, s.platform, s.sshAuthSockID, s.cacheImports)
	if err != nil {
		return nil, err
	}
