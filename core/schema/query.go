...
func getSchemaJSON(hiddenTypes []string, hiddenFields []string, view call.View, srv *dagql.Server) ([]byte, error) {
	...
	for _, rawField := range hiddenFields {
		typeName, fieldName, ok := strings.Cut(rawField, ".")
		if !ok || typeName == "" || fieldName == "" {
			return nil, fmt.Errorf("invalid hidden field %q, expected Type.field", rawField)
		}
		introspectionResponse.Schema.ScrubField(typeName, fieldName)
	}
