...
func tomlApplyField(source []byte, path []dagql.String, setValue any, rootMap map[string]any) ([]byte, error) {
	...
	if doc, err := neontoml.Parse(source); err == nil {
		if err := doc.Set(tomlDottedPath(path), setValue); err == nil {
			if out := doc.Bytes(); tomlSourceMatchesModel(out, rootMap) {
				return out, nil
			}
		}
	}
	...
	if literal, err := tomlEncodeLiteral(setValue); err == nil {
		if out, ok := tomlSetRawLiteral(source, path, literal); ok && tomlSourceMatchesModel(out, rootMap) {
			return out, nil
		}
	}
...
// tomlSourceMatchesModel reports whether TOML text decodes to exactly the given
// value model. The in-place editor reports success for edits it cannot express
// faithfully — replacing a scalar with a table leaves both the old key and the
// new table in the document, for instance — so its output is only used when it
// round-trips back to the model the edit produced.
func tomlSourceMatchesModel(source []byte, rootMap map[string]any) bool {
	got, err := tomlSourceToData(source)
	if err != nil {
		return false
	}
...
func tomlToJSONModel(v any) any {
	...
	case float64:
		// A float with no fractional part encodes to JSON as "5", which decodes
		// back as an integer and silently changes the value's TOML type, so it
		// is stored as a literal like inf/nan.
		if math.IsInf(x, 0) || math.IsNaN(x) || x == math.Trunc(x) {
			literal, err := tomlFormatFloat(x)
			if err != nil {
				return x
			}
...
func tomlFromJSONModel(v any) (any, error) {
	...
	case map[string]any:
		...
		return tomlDecodeTableMembers(x)
	case []any:
		for i, val := range x {
...
func tomlDecodeTableMembers(x map[string]any) (any, error) {
	for k, val := range x {
		decoded, err := tomlFromJSONModel(val)
		if err != nil {
			return nil, err
...
func tomlUnwrap(kind string, wrapped any) (any, error) {
	if kind == tomlKindTable {
		...
		// m is a genuine table that merely looks like a wrapper, so decode its
		// members directly rather than re-testing m for the wrapper shape.
		return tomlDecodeTableMembers(m)
	}
