...
// TestTOMLApplyFieldQuotesNonBareSegments covers the segment classes that must
// be quoted before they reach the in-place editor: an unquoted non-bare segment
// makes the dotted-path parser reject the edit, which would silently downgrade
// to a full re-encode and lose the document's comments.
func TestTOMLApplyFieldQuotesNonBareSegments(t *testing.T) {
	src := []byte("[ports]\n8080 = \"keep\" # comment\n")
	for _, tc := range []struct{ segment, want string }{
		...
		t.Run(tc.segment, func(t *testing.T) {
			rootMap := map[string]any{"ports": map[string]any{"8080": "keep", tc.segment: "y"}}
			out, err := tomlApplyField(src, []dagql.String{"ports", dagql.String(tc.segment)}, "y", rootMap)
			require.NoError(t, err)
...
// TestTOMLWrapperCollisionShapedLikeWrapper covers a genuine table whose keys
// match the wrapper shape exactly, which must survive as a table rather than
// being unwrapped a second time into the type its sentinel names.
func TestTOMLWrapperCollisionShapedLikeWrapper(t *testing.T) {
	src := []byte("\"$dagger.toml\" = \"datetime\"\nvalue = \"2024-01-02T03:04:05Z\"\n")
	data, err := tomlSourceToData(src)
	require.NoError(t, err)
	v, err := tomlDecodeData(data)
	require.NoError(t, err)
	m, ok := v.(map[string]any)
	require.True(t, ok, "decoded to %T, want a table", v)
	require.Equal(t, "datetime", m[tomlTypeKey])
	require.Equal(t, "2024-01-02T03:04:05Z", m["value"])
}

// TestTOMLApplyFieldKeepsFloatType pins that a float with no fractional part
// stays a float; the JSON value model would otherwise decode 5.0 back as an
// integer and change the value's TOML type.
func TestTOMLApplyFieldKeepsFloatType(t *testing.T) {
	out, err := tomlApplyField([]byte("x = 1\n"), []dagql.String{"f"}, 5.0,
		map[string]any{"x": int64(1), "f": 5.0})
	require.NoError(t, err)
	require.Contains(t, string(out), "f = 5.0")
}

// TestTOMLApplyFieldReplacesScalarWithTable covers an edit the in-place editor
// reports as successful but cannot express: turning a scalar into a table
// leaves both definitions behind, producing TOML that no longer parses. The
// result must be validated against the value model and fall back instead.
func TestTOMLApplyFieldReplacesScalarWithTable(t *testing.T) {
	rootMap := map[string]any{"a": map[string]any{"b": "v"}}
	out, err := tomlApplyField([]byte("a = 1\n"), []dagql.String{"a", "b"}, "v", rootMap)
	require.NoError(t, err)

	got, err := tomlSourceToData(out)
	require.NoError(t, err, "contents() must always return parseable TOML, got %q", out)
	want, err := tomlMarshalData(rootMap)
	require.NoError(t, err)
