...
func (s *workspaceSchema) stageWorkspaceConfigAndLock(
	...
	return s.overlayEdit(ctx, parent, ownedPaths(touched...), func(base dagql.ObjectResult[*core.Directory]) (dagql.ObjectResult[*core.Directory], error) {
...
func (s *workspaceSchema) withConfigValue(
	...
	writeKey := args.Key
	if envName, ok := selectedWorkspaceEnvFor(ctx, parent.Self()); ok && !isExplicitEnvConfigKey(args.Key) {
