...
func TestLoadTraceInspectionTools(t *testing.T) {
	...
	require.Contains(t, invoke("ReadTrace", map[string]any{"span": callID, "view": "inspect"}), digest)
	require.Contains(t, invoke("ReadTrace", map[string]any{"span": rootID, "view": "timings"}), "left running")
