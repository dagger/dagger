...
func TestLoadTraceInspectionTools(t *testing.T) {
	...
	require.Contains(t, invoke("ReadTrace", map[string]any{"span": rootID, "view": "inspect"}), rootID)
	require.Contains(t, invoke("ReadTrace", map[string]any{"span": callID, "view": "inspect"}), callID)
