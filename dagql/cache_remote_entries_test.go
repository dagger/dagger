...
// applyCloudFact applies one engine's fact to a cache of remote entries, as the
...
func applyCloudFact(t *testing.T, cloud *Cache, engine string, f cachefact.Fact) {
	...
	case cachefact.Result:
		_, err := cloud.UpsertRemoteEntry(ctx, engine, body)
		require.NoError(t, err)
	case cachefact.Identity:
		require.NoError(t, cloud.TeachRemoteEntryIdentity(ctx, engine, body))
	case cachefact.Deps:
		unknown, err := cloud.SetRemoteEntryDeps(ctx, engine, body)
		require.NoError(t, err)
		require.Empty(t, unknown, "a deps fact names only announced entries")
	case cachefact.Class:
		require.NoError(t, cloud.UpsertRemoteClass(ctx, body))
	case cachefact.TermFact:
		require.NoError(t, cloud.UpsertRemoteTerm(ctx, body))
	case cachefact.Removed:
		for _, id := range body.IDs {
			require.NoError(t, cloud.RemoveRemoteEntry(ctx, RemoteEntryKey{Engine: engine, ID: id}))
		}
	}
}
...
// entryView is what the round trip compares for one entry.
type entryView struct {
	classes      string // canonical set of output classes, as an opaque key
	classDigests []string
	postings     []string
	// classLabels are the (digest, label) pairs of the entry's classes.
	classLabels []string
	deps        []uint64
	// terms are the terms producing the entry's classes, one per term, each as
	// its self digest over its inputs' class representatives and provenance.
	terms         []string
	expiresAtUnix int64
}

// sourceEntryViews describes an engine cache's own results by result number.
func sourceEntryViews(c *Cache) map[uint64]entryView {
	c.egraphMu.Lock()
	defer c.egraphMu.Unlock()
	out := map[uint64]entryView{}
	for id, res := range c.resultsByID {
		if res == nil {
			continue
		}
		view := entryViewLocked(c, id)
...
// cloudEntryViews describes one engine's entries in a cache of remote entries,
// by that engine's result numbers.
func cloudEntryViews(c *Cache, engine string) map[uint64]entryView {
	c.egraphMu.Lock()
	defer c.egraphMu.Unlock()
	out := map[uint64]entryView{}
	for key, id := range c.remoteEntries {
		...
		view := entryViewLocked(c, id)
		for dep := range res.deps {
			if depRes := c.resultsByID[dep]; depRes != nil && depRes.remote != nil {
				view.deps = append(view.deps, depRes.remote.key.ID)
			}
		}
		slices.Sort(view.deps)
		out[key.ID] = view
...
func entryViewLocked(c *Cache, id sharedResultID) entryView {
	var view entryView
...
// partition groups entry numbers by their classes.
func partition(views map[uint64]entryView) [][]uint64 {
...
// requireRoundTrip requires that the remote entries built from one engine's
// facts alone match the engine's own cache: the same entries, the same
// partition of entries into classes, the same digests per class, the same terms
...
func requireRoundTrip(t *testing.T, source, cloud *Cache, engine string) {
	t.Helper()
	src := sourceEntryViews(source)
	got := cloudEntryViews(cloud, engine)
	require.ElementsMatch(t, keysOf(src), keysOf(got), "the same entries")
	require.Equal(t, partition(src), partition(got), "the same partition into classes")
	for id, want := range src {
		have := got[id]
		require.Equal(t, want.classDigests, have.classDigests, "class digests of entry %d", id)
		require.Equal(t, want.postings, have.postings, "postings of entry %d", id)
		require.Equal(t, want.deps, have.deps, "dependencies of entry %d", id)
		require.Equal(t, want.terms, have.terms, "terms producing the classes of entry %d", id)
		require.Equal(t, want.classLabels, have.classLabels, "labeled digests of the classes of entry %d", id)
		require.Equal(t, want.expiresAtUnix, have.expiresAtUnix, "expiry of entry %d", id)
	}
}
...
func TestRemoteEntriesRoundTripComputed(t *testing.T) {
	t.Parallel()
	s := newFactSource(t)
	leaf := s.int(t, "leaf", 1)
	...
	rootKey := RemoteEntryKey{Engine: "engine-a", ID: uint64(root.cacheSharedResult().id)}
	leafKey := RemoteEntryKey{Engine: "engine-a", ID: uint64(leaf.cacheSharedResult().id)}
	depKey := RemoteEntryKey{Engine: "engine-a", ID: uint64(extraDep.cacheSharedResult().id)}
	require.Equal(t, []RemoteEntryKey{rootKey}, cloud.EquivalentRemoteEntries(s.recipe(t, rootFrame)))
	require.Equal(t, []RemoteEntryKey{leafKey}, cloud.EquivalentRemoteEntries(extra.Digest.String()))
	require.ElementsMatch(t, []RemoteEntryKey{rootKey, leafKey, depKey}, cloud.RemoteEntryClosure(rootKey))
	info, ok := cloud.RemoteEntryInfo(rootKey)
	require.True(t, ok)
	require.Equal(t, "root", info.Field)
	require.Equal(t, "Int", info.TypeName)
	require.ElementsMatch(t, []RemoteEntryKey{leafKey, depKey}, info.Deps)
	...
	// The remote entries are never cache hits.
...
func TestRemoteEntriesRoundTripAliases(t *testing.T) {
	t.Parallel()
	s := newFactSource(t)
	...
	replayOntoCloud(t, cloud, "engine-a", s.rec.all())
	requireRoundTrip(t, s.cache, cloud, "engine-a")
	require.Equal(t, cloud.EquivalentRemoteEntries(cacheTestCallDigest(request).String()), cloud.EquivalentRemoteEntries(cacheTestCallDigest(response).String()))
}
...
func TestRemoteEntriesRoundTripTeaches(t *testing.T) {
	t.Parallel()
	s := newFactSource(t)
	...
	replayOntoCloud(t, cloud, "engine-a", s.rec.all())
	requireRoundTrip(t, s.cache, cloud, "engine-a")
	firstKey := RemoteEntryKey{Engine: "engine-a", ID: uint64(first.cacheSharedResult().id)}
	secondKey := RemoteEntryKey{Engine: "engine-a", ID: uint64(second.cacheSharedResult().id)}
	require.ElementsMatch(t, []RemoteEntryKey{firstKey, secondKey}, cloud.EquivalentRemoteEntries(cacheTestCallDigest(cacheTestIntCall("first")).String()))
}

// A consumer built from a restarted engine's boot facts alone matches the
// restored engine.
func TestRemoteEntriesRoundTripBoot(t *testing.T) {
	t.Parallel()
...
// own facts stay in sequence order) and with duplicates, give the same entries
// and the same partition.
func TestRemoteEntriesOrderIndependence(t *testing.T) {
	t.Parallel()
	...
	want := map[string][][]uint64{"engine-a": partition(cloudEntryViews(reference, "engine-a")), "engine-b": partition(cloudEntryViews(reference, "engine-b"))}
	...
	for seed := range uint64(8) {
		...
		for engine, partitionWant := range want {
			require.Equal(t, partitionWant, partition(cloudEntryViews(cloud, engine)), "seed %d, %s", seed, engine)
		}
		require.Equal(t, wantCross, crossEnginePartition(cloud), "seed %d", seed)
	}
}

// crossEnginePartition groups every entry of every engine by class.
...
// combined cache, with no B entry in that class; every class of A alone is
// contained in one combined class.
func TestRemoteEntriesRefinementAcrossEngines(t *testing.T) {
	t.Parallel()
	...
	aloneViews, combinedViews := cloudEntryViews(alone, "engine-a"), cloudEntryViews(combined, "engine-a")
	classOf := func(views map[uint64]entryView) map[string][]uint64 {
	...
	merged := combined.EquivalentRemoteEntries(fxDigest)
	require.Len(t, merged, 2, "f(x) and f(y) are one class")
	for _, key := range merged {
		require.Equal(t, "engine-a", key.Engine, "with no engine B entry in it")
	}
}

// A removed entry leaves when nothing depends on it; an entry a dependent still
// holds stays, marked removed, until the dependent goes.
func TestRemoteEntriesRemovalFollowsOwnership(t *testing.T) {
	t.Parallel()
	cloud := newCloudCache(t)
	ctx := t.Context()
	...
	for _, fact := range []cachefact.Result{leaf, root} {
		_, err := cloud.UpsertRemoteEntry(ctx, "e", fact)
		require.NoError(t, err)
	}
	unknown, err := cloud.SetRemoteEntryDeps(ctx, "e", cachefact.Deps{ID: 2, Deps: []uint64{1, 99}, Complete: true})
	require.NoError(t, err)
	require.Equal(t, []uint64{99}, unknown, "a dependency the cache does not hold is reported")

	require.NoError(t, cloud.RemoveRemoteEntry(ctx, RemoteEntryKey{Engine: "e", ID: 1}))
	info, ok := cloud.RemoteEntryInfo(RemoteEntryKey{Engine: "e", ID: 1})
	require.True(t, ok, "the root still holds the leaf")
	require.True(t, info.Removed)
	require.NoError(t, cloud.RemoveRemoteEntry(ctx, RemoteEntryKey{Engine: "e", ID: 1}), "removing twice is a no-op")

	require.NoError(t, cloud.RemoveRemoteEntry(ctx, RemoteEntryKey{Engine: "e", ID: 2}))
	for _, id := range []uint64{1, 2} {
		_, ok := cloud.RemoteEntryInfo(RemoteEntryKey{Engine: "e", ID: id})
		require.False(t, ok, "entry %d is collected", id)
	}
	require.Empty(t, cloud.EquivalentRemoteEntries("xxh3:root"))
	require.ErrorIs(t, cloud.TeachRemoteEntryIdentity(ctx, "e", cachefact.Identity{ID: 2}), ErrUnknownRemoteEntry)
}
...
// with their own inputs, and RemoteEntryInfo reports them.
func TestRemoteEntriesRoundTripSameSelfTerms(t *testing.T) {
	t.Parallel()
	s := newFactSource(t)
	...
	info, ok := cloud.RemoteEntryInfo(RemoteEntryKey{Engine: "engine-a", ID: uint64(fxRes.cacheSharedResult().id)})
	require.True(t, ok)
	var inputs []string
...
func TestRemoteEntriesRoundTripImported(t *testing.T) {
	t.Parallel()
	...
	for _, res := range factsOfKind[cachefact.Result](rec.all()) {
		info, ok := cloud.RemoteEntryInfo(RemoteEntryKey{Engine: "engine-b", ID: res.ID})
		require.True(t, ok)
		require.Equal(t, cachefact.OriginImported, info.Origin)
	}
}

func entryFact(id uint64, recipe string, expiresAtUnix int64) cachefact.Result {
...
// An entry stays until its own engine removes it: another engine's removal
// never takes an expired entry or a restored entry without terms with it, even
// when it removes the cache's last live term.
func TestRemoteEntriesSurviveOtherEnginesRemovals(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	cloud := newCloudCache(t)
	expired := entryFact(1, "xxh3:shared", 1) // expired long ago
	_, err := cloud.UpsertRemoteEntry(ctx, "engine-a", expired)
	require.NoError(t, err)
	_, err = cloud.UpsertRemoteEntry(ctx, "engine-b", entryFact(1, "xxh3:shared", 0))
	require.NoError(t, err)
	_, err = cloud.UpsertRemoteEntry(ctx, "engine-r", cachefact.Result{
		ID: 7, Origin: cachefact.OriginRestored, Field: "restored",
		Digests: []cachefact.Digest{{Digest: "xxh3:restored"}},
	})
	require.NoError(t, err)

	a := RemoteEntryKey{Engine: "engine-a", ID: 1}
	r := RemoteEntryKey{Engine: "engine-r", ID: 7}
	require.NoError(t, cloud.RemoveRemoteEntry(ctx, RemoteEntryKey{Engine: "engine-b", ID: 1}))
	info, ok := cloud.RemoteEntryInfo(a)
	require.True(t, ok, "engine B's removal leaves engine A's expired entry")
	...
	require.Equal(t, []RemoteEntryKey{a}, cloud.EquivalentRemoteEntries("xxh3:shared"))
	_, ok = cloud.RemoteEntryInfo(r)
	require.True(t, ok, "and the restored entry")

	require.NoError(t, cloud.RemoveRemoteEntry(ctx, a))
	_, ok = cloud.RemoteEntryInfo(a)
	require.False(t, ok, "engine A's own removal takes it")
	require.Empty(t, cloud.EquivalentRemoteEntries("xxh3:shared"))
	info, ok = cloud.RemoteEntryInfo(r)
	require.True(t, ok, "the restored entry survives the last term's removal")
	require.Equal(t, []string{"xxh3:restored"}, info.ClassDigests)
	require.Equal(t, []RemoteEntryKey{r}, cloud.EquivalentRemoteEntries("xxh3:restored"))

	require.NoError(t, cloud.RemoveRemoteEntry(ctx, r))
	_, ok = cloud.RemoteEntryInfo(r)
	require.False(t, ok)
	_, err = cloud.UpsertRemoteEntry(ctx, "engine-a", entryFact(2, "xxh3:after", 0))
	require.NoError(t, err, "the emptied cache takes new entries")
	require.Equal(t, []RemoteEntryKey{{Engine: "engine-a", ID: 2}}, cloud.EquivalentRemoteEntries("xxh3:after"))
}

// Removals of two engines' entries, interleaved in any order, leave exactly the
// entries not removed, each still in its class.
func TestRemoteEntriesRemovalInterleavings(t *testing.T) {
	t.Parallel()
	a, b, _ := refinementSources(t)
	factsA, factsB := a.rec.all(), b.rec.all()
	var removals []RemoteEntryKey
	survivors := map[RemoteEntryKey]bool{}
	for i, res := range factsOfKind[cachefact.Result](factsA) {
		key := RemoteEntryKey{Engine: "engine-a", ID: res.ID}
		if i%2 == 0 {
	...
	for _, res := range factsOfKind[cachefact.Result](factsB) {
		removals = append(removals, RemoteEntryKey{Engine: "engine-b", ID: res.ID})
	}
	...
	for seed := range uint64(8) {
		...
		for _, key := range order {
			require.NoError(t, cloud.RemoveRemoteEntry(t.Context(), key))
		}
		cloud.egraphMu.Lock()
		held := map[RemoteEntryKey]bool{}
		for key, id := range cloud.remoteEntries {
			if res := cloud.resultsByID[id]; res != nil && !res.remote.removed {
				held[key] = true
			}
		}
		...
		for key := range survivors {
			...
			for _, dig := range info.Digests {
				require.Contains(t, cloud.EquivalentRemoteEntries(dig), key)
			}
		}
	}
}

// A remote entry's number never loads a value, a call frame or a schema
// module, with or without a session.
func TestRemoteEntriesNeverLoadByResultID(t *testing.T) {
	t.Parallel()
	ctx := cacheTestContext(t.Context())
	cloud := newCloudCache(t)
	_, err := cloud.UpsertRemoteEntry(ctx, "engine-a", entryFact(1, "xxh3:load", 0))
	require.NoError(t, err)
	cloud.egraphMu.RLock()
	id := uint64(cloud.remoteEntries[RemoteEntryKey{Engine: "engine-a", ID: 1}])
	cloud.egraphMu.RUnlock()
	require.NotZero(t, id)

	for _, session := range []string{"", "s"} {
		_, err := cloud.LoadResultByResultID(ctx, session, nil, id)
		require.ErrorIs(t, err, errRemoteEntryHasNoValue, "session %q", session)
		_, err = cloud.ResultCallByResultID(ctx, session, id)
		require.ErrorIs(t, err, errRemoteEntryHasNoValue, "session %q", session)
	}
	_, err = cloud.LoadResultByResultIDForSchema(ctx, "s", nil, id, nil)
	require.ErrorIs(t, err, errRemoteEntryHasNoValue)
}

// Readers of a merged-away digest run concurrently with each other and with
// writers. Run with -race.
func TestRemoteEntriesConcurrentReadsOfMergedDigest(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	cloud := newCloudCache(t)
	const n = 8
	for i := range n {
		_, err := cloud.UpsertRemoteEntry(ctx, "engine-a", entryFact(uint64(i+1), "xxh3:merge-"+strconv.Itoa(i), 0))
		require.NoError(t, err)
	}
	// Chain the classes so that finding a digest's root walks a path.
	for i := range n - 1 {
		require.NoError(t, cloud.UpsertRemoteClass(ctx, cachefact.Class{Digests: []cachefact.Digest{
	...
	for g := range 8 {
		wg.Go(func() {
			for i := range 50 {
				...
				require.Len(t, cloud.EquivalentRemoteEntries(dig), n)
				info, ok := cloud.RemoteEntryInfo(RemoteEntryKey{Engine: "engine-a", ID: uint64((g+i)%n + 1)})
				require.True(t, ok)
	...
	wg.Go(func() {
		for i := range 20 {
			_, err := cloud.UpsertRemoteEntry(ctx, "engine-w", entryFact(uint64(i+1), "xxh3:writer-"+strconv.Itoa(i), 0))
			require.NoError(t, err)
		}
	})
	wg.Wait()
