...
//
// Agents this session already holds are left out: an agent with a control
// record or loop span in the LIVE trace has a runtime entry here already,
// whether it was spawned in this session or restored into it earlier, and
...
// its control record into the new trace (§4.5), so nothing else about its
// entry distinguishes it from a source-trace one.
