...
func (fe *frontendPretty) renderFinalReport(ctx tuist.Context, r *renderer) {
	...
	// above, where the install span carries its logs and health checks. A
	// subtree-scoped report skips them, for the same reason the CONVERSATION
	// section does: without a tool-call display span to zoom to, the scope root
	// sits at or above the services the enclosing run started.
	if RunningInAgent() {
