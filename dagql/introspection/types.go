...
// TODO: dupe w/ core/cachekey.go to avoid import cycle, fix
func cachePerCall[P dagql.Typed, A any](ctx context.Context, inst dagql.Instance[P], args A, cacheCfg dagql.CacheConfig) (*dagql.CacheConfig, error) {
	randID := identity.NewID()
	const XXH3 digest.Algorithm = "xxh3"
	h := xxh3.New()
	h.WriteString(randID)
	cacheCfg.Digest = digest.NewDigest(XXH3, h)
	return &cacheCfg, nil
}
