...
// CallPayloadSeenKeyStore reports whether any target in the current delivery
// route still needs an immutable call payload digest. Implementations must not
// claim from this producer-side check; the delivery exporter owns the atomic
// per-target claim.
