...
(* does at candidate selection: LookupHit and CanonicalPick               *)
(* (selectLookupCandidateForSessionLocked). HELD-RESULT guards use         *)
(* ownership, never current satisfaction: FnComplete's reuse,              *)
(* PubIndexFresh's structural deps, PubAttachAddDep's attachment deps,     *)
(* and AddDepLate's possession, because ownership proves only that the     *)
(* result satisfied the session WHEN OBTAINED - requirement growth after   *)
(* the filter (the gated-growth finding) can invalidate it, and the code   *)
(* re-checks nothing on held results.                                      *)
