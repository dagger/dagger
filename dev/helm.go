...
func (h *Helm) Test(ctx context.Context) error {
	...
	test, err := h.chart().
		...
		WithExec([]string{"helm", "install", "--create-namespace", "--namespace=dagger", "dagger", "."}).
		WithExec([]string{"kubectl", "wait", "--for=condition=Ready", "--timeout=120s", "pod", "--selector=name=dagger-dagger-helm-engine", "--namespace=dagger"}).
		Sync(ctx)
	if err != nil {
		return err
	}
