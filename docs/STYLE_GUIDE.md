...
- Avoid using `you`, `we`, `our` and other personal pronouns, except in guides (the quickstart and everything under `getting-started/platform-guides/`), which are written as companion journeys. Alternatives are (e.g. instead of `this is where you will deploy the application`):
