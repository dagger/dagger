...
To update the Dagger CLI, download and uncompress the [latest release of the Dagger CLI](https://github.com/dagger/dagger/releases) for your platform (replace the placeholders with the correct values) and replace your existing Dagger CLI binary with the newer version:
...
1. Remove the `dagger` sub-directory of your `$XDG_CACHE_HOME` and `$XDG_CONFIG_HOME` directories. These are Dagger cache locations containing the auto-provisioned Dagger CLI (used for SDK operations).
