...
const vars = ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_DEFAULT_REGION"];
vars.forEach(v => {
  if(!process.env[v]) {
...
connect(async (client) => {
  ...
  let response = await aws
    ...
    .withExec(["lambda", "update-function-code", "--function-name", "myFunction", "--zip-file", "fileb:///tmp/function.zip"])
    .withExec(["lambda", "get-function-url-config", "--function-name", "myFunction"])
    .stdout();
