...
const vars = ["DOCKERHUB_USERNAME", "DOCKERHUB_PASSWORD", "AZURE_SUBSCRIPTION_ID", "AZURE_TENANT_ID", "AZURE_CLIENT_ID", "AZURE_CLIENT_SECRET"];
vars.forEach(v => {
  if(!process.env[v]) {
...
connect(async (daggerClient) => {
  ...
  const secret = daggerClient.setSecret("password", process.env.DOCKERHUB_PASSWORD);

  // get reference to the project directory
  const source = daggerClient.host().directory(".", { exclude: ["node_modules/", "ci/"] });

  // get Node image
  const node = daggerClient.container({ platform: "linux/amd64" }).from("node:18");
  ...
  const azureClient = new ContainerInstanceManagementClient(new DefaultAzureCredential(), process.env.AZURE_SUBSCRIPTION_ID);
  ...
  // print ref
  console.log(`Deployment for image ${address} now available at http://${result.ipAddress.ip}:${result.ipAddress.ports[0].port}`);
