...
- It uses the `Container.Stderr()` method to return the error stream of the last executed command, `Null` if no command has been executed - no `panic` or `stderr` output implies successful execution (all tests pass).
