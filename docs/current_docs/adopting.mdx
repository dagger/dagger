...
- Learn more about key Dagger concepts like [function chaining](manual/functions/chaining.mdx), [just-in-time artifacts](manuals/user/artifacts/artifacts.mdx), [host access](manual/functions/host-resources.mdx) and [pipeline visualization](manual/cli/visualization.mdx) in the [Dagger manual](manual/index.mdx).
