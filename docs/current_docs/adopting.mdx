...
- Learn more about key Dagger concepts like [function chaining](api/functions/chaining.mdx), [just-in-time artifacts](api/functions/return-values.mdx), [host access](api/functions/host-resources.mdx) and [pipeline visualization](features/visualization.mdx).
