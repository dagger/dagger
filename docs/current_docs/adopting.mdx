...
- Learn more about key Dagger concepts like [function chaining](api/chaining.mdx), [just-in-time artifacts](api/return-values.mdx), [host access](api/host-resources.mdx) and [pipeline visualization](features/visualization.mdx).
