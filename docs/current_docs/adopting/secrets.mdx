...
```shell
dagger api call my-function --secret=vault://path/my-app.token
```

Reads the `token` field of the KVv2 secret `my-app`. The path is relative to the KVv2 mount, which defaults to `secret` and can be changed with `VAULT_PATH_PREFIX`. Add `?ttl=1h` to control client-side caching.

Alternatively, use `vault-full://` to specify the full path including the secrets engine mount point (no `VAULT_PATH_PREFIX` needed):

```shell
dagger api call my-function --secret=vault-full://mount/path/my-app.token
```

This is useful when reading from multiple secrets engines in the same module, as each `vault-full://` reference can target a different engine.

Requires the Dagger CLI to be authenticated with Vault (via `VAULT_ADDR` and `VAULT_TOKEN` environment variables or other standard Vault auth methods).
