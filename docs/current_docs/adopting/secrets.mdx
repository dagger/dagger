...
Dagger has built-in support for secrets — passwords, API keys, tokens — sourced from multiple providers. Dagger redacts secret values from logs, avoids writing them to container layers, and excludes them from cache keys.
...
Dagger provides safeguards for secret values:
...
These safeguards do not make untrusted module code safe. A module that receives a secret can intentionally read it, send it over the network, write it to a file, or print derived values. Treat modules that receive secrets as trusted code, and avoid sending secrets to workflows triggered by untrusted forks.

For rollout guidance, see [Security and Operations](/adopting/security).
