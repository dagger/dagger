...
:::tip
For checks with no workflow YAML at all, enable [Cloud Checks](/adopting/triggers) with `dagger cloud check on`. The CircleCI workflow below is the hybrid-mode alternative for when you want to drive Dagger from your existing CircleCI setup.
:::
