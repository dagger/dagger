...
:::tip
For checks with no workflow YAML at all, enable [Cloud Checks](/adopting/triggers) with `dagger ws autocheck on`. The CircleCI workflow below is the hybrid-mode alternative for when you want to drive Dagger from your existing CircleCI setup.
:::
...
- A repository with a Dagger workspace (`dagger.toml`) — see [Workspace Setup](/adopting/workspace-setup).
