...
:::tip
For checks with no workflow YAML at all, enable [Cloud Checks](./index.mdx) with `dagger ws autocheck on`. The GitHub Action below is the hybrid-mode alternative for when you want to drive Dagger from your existing GitHub Actions setup.
:::
...
- A GitHub repository with a Dagger workspace (`dagger.toml`) — see [Workspace Setup](../workspace-setup.mdx).
...
To send traces to [Dagger Cloud](../observability.mdx) and offload execution to a managed Dagger Engine, add your Cloud token:
