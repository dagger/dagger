...
:::tip
For checks with no pipeline YAML at all, enable [Cloud Checks](./index.mdx) with `dagger ws autocheck on`. The Tekton Task below is the hybrid-mode alternative for when you want to drive Dagger from your existing Tekton setup.
:::
