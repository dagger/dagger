...
*Note: the latest version is `0.17.0-llm.8`. It was released on March 8, 2025. If you are running an older build, we recommend upgrading.*
...
```console
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.17.0-llm.8 BIN_DIR=/usr/local/bin sh
```
...
- `ANTHROPIC_MODEL`: if unset, defaults to latest Claude 4.5 Sonnet
additional option
- `ANTHROPIC_BASE_URL`: tbd
...
#### OpenAI-compatible Google Gemini API
- `GEMINI_API_KEY`: required
- `GEMINI_BASE_URL`: see the [Google documentation](https://ai.google.dev/gemini-api/docs/openai)
