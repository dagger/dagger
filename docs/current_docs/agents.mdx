...
*Note: the latest version is `0.17.0-llm.7`. It was released on March 8, 2025. If you are running an older build, we recommend upgrading.*
...
```console
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.17.0-llm.7 BIN_DIR=/usr/local/bin sh
```
