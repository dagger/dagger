...
- [toy-programmer](https://github.com/dagger/agents/tree/main/toy-programmer): a very, very simple programmer micro-agent for demo purposes
...
```console
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.17.0-llm.4 BIN_DIR=$HOME/.local/bin sh
```
...
```console
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.17.0-llm.4 BIN_DIR=/usr/local/bin sudo -E sh
```
