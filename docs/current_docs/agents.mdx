...
*Note: the latest version is `0.17.0-llm.6`. It was released on March 6, 2025. If you are running an older build, we recommend upgrading.*
...
```console
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.17.0-llm.6 BIN_DIR=/usr/local/bin sh
```
...
If you're using Azure OpenAI Service, convigure your environment with the following variables:

```plaintext
OPENAI_BASE_URL=https://<azure-openai-resource>.cognitiveservices.azure.com
OPENAI_API_KEY=<azure openai deployment api key>
OPENAI_MODEL=<model to use by default>
OPENAI_AZURE_VERSION=<azure openai version> "example: 2024-12-01-preview"
```
