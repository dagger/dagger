...
```shell
dagger call read-file --file=/my/file/path/README.md
```

And here is an example of passing a file from a remote Git repository as argument:

```shell
dagger call read-file --file=https://github.com/dagger/dagger.git
```

:::tip
Accessing remote files over HTTP and SSH works the same way for both file and directory arguments.
:::
