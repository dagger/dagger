...
Google Cloud Secret Manager requires authentication via [Application Default Credentials (ADC)](https://cloud.google.com/docs/authentication/provide-credentials-adc), which can be configured in several ways:
...
- Use workload identity on GKE or the metadata service on GCE

The project ID can be specified via the `GCP_PROJECT_ID` environment variable (or `GOOGLE_CLOUD_PROJECT` / `GCLOUD_PROJECT` as fallbacks).

GCP secrets are accessed with the scheme `gcp://PATH`. The path can be in several formats:
- Simple secret name: `gcp://my-secret` (uses latest version)
- Secret with version: `gcp://my-secret/versions/2`
- Full resource name: `gcp://projects/my-project/secrets/my-secret`
- With TTL for caching: `gcp://my-secret?ttl=5m`
...
<TabItem value="System shell">
```shell
dagger -c 'github-api gcp://github-token'
```
</TabItem>
<TabItem value="Dagger Shell">
```shell title="First type 'dagger' for interactive mode."
github-api gcp://github-token
```
</TabItem>
<TabItem value="Dagger CLI">
```shell
dagger call github-api --token=gcp://github-token
```
</TabItem>
</Tabs>
