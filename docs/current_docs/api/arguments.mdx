...
In addition to supporting HTTP(S) access for public repositories, Dagger also supports the use of SSH for both public and private repositories. Note that this requires [SSH authentication to be properly configured](./remote-modules.mdx#configuring-ssh-authentication) on your Dagger host. Here is the same example, this time using SSH:
...
For more information, refer to the [documentation on configuring SSH authentication for remote repositories, best practices, and current limitations](./remote-modules.mdx).
...
When determining how to resolve a default path, Dagger first identifies a "context directory". The context directory is either the repository root (for Git repositories, as defined by the presence of a `.git` sub-directory), or the directory containing a `dagger.json` file (for all other cases). The default path is then resolved starting from the context directory.
...
The following table describes how the arguments are resolved for differing values of the default path and differing directory types.
...
| `.` | `/` | `dagger call -m my-module read-dir` | `/` |
| `..` | `/` | `dagger call -m my-module read-dir` | `Error: path outside context directory` |
| `/README.md` | `/` | `dagger call -m my-module read-file` | `/README.md` |
| `./README.md` | `/` | `dagger call -m my-module read-file` | `/README.md` |
| `../README.md`| `/`  | `dagger call -m my-module read-file` | `Error: path outside context directory` |
...
| `.` | `/my-module` | `dagger call -m my-module read-dir` | `/my-module` |
| `..` | `/my-module` | `dagger call -m my-module read-dir` | `Error: path outside context directory` |
| `/README.md` | `/my-module` | `dagger call -m my-module read-file` | `Error: file does not exist` |
| `./README.md` | `/my-module` | `dagger call -m my-module read-file` | `Error: file does not exist` |
| `../README.md` | `/my-module` | `dagger call -m my-module read-file` | `Error: path outside context directory` |
...
:::tip
It's also possible to provide an `ignore` parameter to a contextual argument of type `Directory` to [automatically ignore or include files in the directory](./fs-filters.mdx). The `ignore` parameter follows the [`.gitignore` syntax](https://git-scm.com/docs/gitignore): prefixing a path with `!` will actually include it instead of ignoring it.
:::
