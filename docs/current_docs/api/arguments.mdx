...
Here is an example of a Dagger function that accepts a integer argument:

<Tabs groupId="language">

<TabItem value="Go">
```go file=./snippets/functions/arguments-integer/go/main.go
```
</TabItem>

<TabItem value="Python">
```python file=./snippets/functions/arguments-integer/python/main.py
```
</TabItem>

<TabItem value="TypeScript">
```typescript file=./snippets/functions/arguments-integer/typescript/index.ts
```
</TabItem>
...
:::warning
Floats are currently only supported in Go and TypeScript SDKs for now.
:::

To pass a float argument to a Dagger function, add the corresponding flag to the `dagger call` command, followed by the float value.

Here is an example of a Dagger function that accepts a float argument:

<Tabs groupId="language">
<TabItem value="Go">
```go file=./snippets/functions/arguments-float/go/main.go
```
</TabItem>
<TabItem value="TypeScript">
...
To declare a `float` argument on the function signature, you need to import `float` from `@dagger.io/dagger` and use it as argument's type.
The imported `float` type is a `number` underneath, so you can use it as you would use a `number` inside your function.
:::

```typescript file=./snippets/functions/arguments-float/typescript/index.ts
```
</TabItem>
</Tabs>
