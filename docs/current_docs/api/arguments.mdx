...
In addition to supporting HTTP(S) access for public repositories, Dagger also supports the use of SSH for both public and private repositories. Note that this requires [SSH authentication to be properly configured](./remote-repositories.mdx#configuring-ssh-authentication) on your Dagger host. Here is the same example, this time using SSH:
...
For more information, refer to the [documentation on configuring SSH authentication for remote repositories, best practices, and current limitations](./remote-repositories.mdx).
