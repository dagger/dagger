...
This command will take some time to complete on the first run, as the cache volumes will not exist at that point. Subsequent runs will be significantly faster (assuming there is no other change), since Dagger will simply use the dependencies from the cache volumes instead of downloading them again.
...
You can also see an [example of invalidating the layer cache in a custom function](../cookbook/cookbook.mdx#invalidate-cache).
