...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container | with-exec ls,/etc/ | stdout
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container | with-exec cat,/etc/os-release | stdout'
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container | with-exec rm,/etc/os-release | with-exec ls,/etc | stdout'
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container | with-exec apk,add,curl | with-exec curl,-L,dagger.io | stdout'
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container --packages=cowsay | terminal'
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container --packages=cowsay,zsh | terminal --cmd=zsh'
```
...
```shell
dagger -m github.com/kpenfound/dagger-modules/golang@v0.2.1 shell -c 'build ./cmd/dagger --source=https://github.com/dagger/dagger  | export ./my-build'
```
...
```shell
dagger -m github.com/kpenfound/dagger-modules/golang@v0.2.1 shell -c 'build ./cmd/dagger --source=https://github.com/dagger/dagger  | export ./my-build --wipe'
```
...
```shell
dagger -m github.com/kpenfound/dagger-modules/golang@v0.2.1 shell -c 'build ./cmd/dagger --source=https://github.com/dagger/dagger  | file ./dagger | export ./my-file'
```

Here is another example, this time exporting the results of a `ruff` linter Dagger Function as `/tmp/report.json` on the host:

```shell
dagger -m github.com/dagger/dagger/modules/ruff shell -c 'lint https://github.com/dagger/dagger | report | export /tmp/report.json'
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container | export ./tarball.tar'
```
...
dagger -m github.com/kpenfound/dagger-modules/golang@v0.2.1 shell -c 'build . --source=./example/hello | directory . | entries'
```

Here is an example of using the `File.contents()` method to print the JSON report of a linter run:

```shell
dagger -m github.com/dagger/dagger/modules/ruff shell -c 'lint https://github.com/dagger/dagger | report | contents'
```
...
```shell
dagger -m github.com/sagikazarmark/daggerverse/arc@40057665476af62e617cc8def9ef5a87735264a9 shell -c 'archive-directory dagger-cli https://github.com/dagger/dagger#main:cmd/dagger |  create zip | size'
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container | publish ttl.sh/my-wolfi'
```
...
```shell
dagger -m github.com/kpenfound/dagger-modules/nginx@v0.1.0 shell -c 'container | as-service | up'
```
...
```shell
dagger -m github.com/kpenfound/dagger-modules/nginx@v0.1.0 shell -c 'container | as-service | up --ports=8080:80'
```
...
```shell
dagger -m github.com/kpenfound/dagger-modules/nginx@v0.1.0 shell -c 'container | as-service | up --random'
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container | with-directory /src . | with-exec ls,/src | stdout'
```
...
```shell
dagger -m github.com/dagger/dagger/modules/wolfi@v0.16.2 shell -c 'container | with-file /README.md ./README.md | with-exec cat,/README.md | stdout'
```
