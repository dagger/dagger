...
```shell
dagger -m github.com/shykes/daggerverse/wolfi@v0.1.4 call \
  container \
...
dagger -m github.com/kpenfound/dagger-modules/golang@v0.2.0 call \
  build --source=./example/hello --args=. \
...
dagger -m github.com/shykes/daggerverse/wolfi@v0.1.4 call \
  container \
  publish --address=ttl.sh/my-wolfi
```
...
```shell
dagger -m github.com/kpenfound/dagger-modules/nginx@v0.1.0 call \
  container \
  as-service \
  up
```
