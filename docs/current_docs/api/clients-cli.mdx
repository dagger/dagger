...
- You want a nice alternative to `docker run`.
    ...
    <TabItem value="dagger shell">
    ```shell
    dagger shell -c 'container | from cgr.dev/chainguard/wolfi-base | terminal'
    ```
    </TabItem>
    <TabItem value="dagger core">
    ```shell
...
<TabItem value="dagger shell">
```shell
dagger shell -c 'container-echo "Welcome to Dagger!" | stdout'
```
</TabItem>
...
:::tip
When using `dagger call` and `dagger shell`, all names (functions, arguments, struct fields, etc) are converted into a shell-friendly "kebab-case" style.
:::
