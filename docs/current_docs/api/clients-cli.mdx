...
The `dagger core` command lets you call the core Dagger API, without needing to first load a Dagger [module](../features/reusable-modules.mdx). This is useful in some scenarios:
...
The Dagger API can be extended with Dagger [modules](../features/reusable-modules.mdx). The `dagger call` command lets you call Dagger functions from Dagger modules, either remotely or after installing them locally with `dagger install`.
