...
- Use Dagger as an alternative to `docker run`.
    ...
    :::tip
    If only the core Dagger API is needed, the `-n` (`--no-mod`) flag can be provided. This results in quicker startup, because the Dagger CLI doesn't try to find and load a current module.
    :::
