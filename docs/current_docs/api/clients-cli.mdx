...
The `dagger core` command lets you call the core Dagger API, without needing to first load a Dagger module. This is useful in some scenarios:
...
The Dagger API can be extended with Dagger modules. The `dagger call` command lets you call Dagger functions from Dagger modules, either remotely or after installing them locally with `dagger install`.
