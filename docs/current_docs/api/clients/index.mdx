...
Most of the time you call the Dagger API through the CLI or from inside a [module](../../reference/sdks/index.mdx). A client is for your own program, whether a script, a service, or a test harness, that needs to talk to the engine directly.
...
Only SDKs that implement the client contract can generate clients. Today that is [Go](../../reference/sdks/go.mdx), [TypeScript](../../reference/sdks/typescript.mdx), and [PHP](../../reference/sdks/php.mdx). The bindings pin the engine version the bound module requires, so a client and the module it came from stay in step.
