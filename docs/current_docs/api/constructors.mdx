...
:::important
Dagger `@object_type` classes are Python [data classes](https://docs.python.org/3/library/dataclasses.html) with the Dagger API exposed. This means that you can adjust the generation of the constructor using standard data class features, such as attributes and post-init processing.
:::
...
- Attributes are both fields and constructor arguments by default. So by setting `init=False` on the container argument, it's no longer added as a constructor argument but is still a required field that must be initialized if there's no default value.
...
- There are two important differences between `dataclasses.field()` and `dagger.field()`
  - `dagger.field()` exposes the attribute as a Dagger Function in the API. However, `dagger.field()` doesn't expose everything in `dataclasses.field()`.
  - With `dataclass.field()`, you can specify separate `default` and `default_factory` arguments for either a simple value or a default using a callable respectively. However, with `dagger.field()`, the same `default` argument supports both cases.
...
[`dataclasses.field(default_factory=...)`](https://docs.python.org/3/library/dataclasses.html#dataclasses.field) or `dagger.field(default=...)`:
