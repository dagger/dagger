...
:::important
Dagger modules have only one constructor. Entrypoint function of [custom types](./custom-types.mdx) are not registered; they are constructed by the function that [chains](./index.mdx#chaining) them.
:::
