...
:::caution
When relying on default paths in Dagger Shell, it's important to know that the source directory is re-evaluated on each command execution within the shell session. This differs from passing the source directory explicitly as an argument, where it's evaluated once and cached. This re-evaluation can lead to unintended behavior where changes to the source directory during the session (such as through exports or logs) invalidate the cache, causing the entire pipeline to re-execute.
