...
Filtering improves the performance of your Dagger Functions in two ways:
...
- It enables different use-cases, such as setting up pipelines for monorepos.
...
- Monorepos containing multiple files and directories. Typically in a monorepo, your Dagger Function only operates on a subset (representing a specific component or feature) of your repository, so uploading the entire worktree on every call imposes a prohibitive cost.

- Repositories containing binary objects. Binary objects, such as audio/video files, are usually extremely large in size and therefore take time to upload. If they're not directly relevant, you'll usually want your Dagger Function to ignore them.
...
- Dependencies. If you're developing locally, you'll typically have your project dependencies installed locally as well: `node_modules` (Node.js), `.venv` (Python), `vendor` (PHP) and so on. When you call your Dagger Function locally, Dagger will upload all these installed dependencies as well. This is both bad practice and inefficient. Typically, you'll want your Dagger Function to ignore locally-installed dependencies and only operate on the project source code.
...
To implement a pre-call filter in your Dagger Function, add an `ignore` parameter to your `Directory` argument. The `ignore` parameter follows the `.gitignore` syntax: prefixing a path with `!` will actually include it instead of ignoring it.
...
```go file=./snippets/filesystem-filters/pre-call/go/main.go
```
...
```python file=./snippets/filesystem-filters/pre-call/python/main.py
```
...
```typescript file=./snippets/filesystem-filters/pre-call/typescript/index.ts
```
...
```go file=./snippets/filesystem-filters/post-call/go/main.go
```
...
```python file=./snippets/filesystem-filters/post-call/python/main.py
```
...
```typescript file=./snippets/filesystem-filters/post-call/typescript/index.ts
```
