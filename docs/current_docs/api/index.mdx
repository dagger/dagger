...
Dagger [modules](../features/reusable-modules.mdx) are collections of Dagger functions, packaged together for easy sharing and consumption. They are portable and reusable across languages.
...
The Dagger API is extensible and shareable by design. You can extend the API with [custom functions](./custom-functions.mdx), which are loaded via Dagger [modules](../features/reusable-modules.mdx). You are encouraged to write your own Dagger modules and share them with others.
