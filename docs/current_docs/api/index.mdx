...
- From a [custom application created with a Dagger SDK](./clients-sdk.mdx#custom-applications)
  - [`dagger run` for command execution with live progress](./clients-sdk.mdx#custom-applications)
- From a [command-line HTTP client like `curl`](./clients-http.mdx#command-line-http-clients) or a [language-native GraphQL client](./clients-http.mdx#language-native-http-clients)
