...
The TypeScript SDK supports interfaces, which allow you to define set of functions
that a object must implement to be considered a valid instance of that interface.
...
Functions defined in interface definitions must match the client-side API
signature style:
- Always define async function in interface (wrap the return type in a `Promise<T>`).
- You can declare it as a method signature (e.g., `foo(): Promise<string>`) or a property signature (e.g., `foo: () => Promise<string>`).
...
Dagger automatically detects if an object coming for the module itself or one of its dependencies implements
an interface defined in its module or its dependencies. 
If so, it will add new conversion function to the object that implement the interface
so it can be passed as argument.

Here is an example of a module that uses the `Example` module defined above and
pass it as argument of the `foo` function of the `MyModule` object:
