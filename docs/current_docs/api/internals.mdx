...
In practice, this means that if you create a Dagger object (like a container or a directory) but never access its state - either by returning it to the Dagger CLI or using it in a different Dagger Function - Dagger automatically skips it as part of its  optimization process, thinking it's not needed. This is analogous to running a build command but never using the output binary; since the output is never used, Dagger optimizes away the build step.

There are some cases where this behavior causes unexpected results, such as when the command run has external effects. In this case, the optimization can be forcefully disabled with the `sync` field.
...
1. Clone the Go examples repository and make it the working directory:
...
The return value of this query is an ID representing the state of the `hello` repository sub-directory on the host
