...
To remove a dependency from your Dagger module, use the `dagger uninstall` command, as in the following example:

```sh
dagger uninstall github.com/shykes/daggerverse/hello
```

The `dagger uninstall` command can be passed either a remote repository reference or a local module name. If a version number is specified, only the specified version of the dependency is uninstalled.

Assuming the following `dagger.json`:
  ...
  "dependencies": [
    {
      ...
      "pin": "54d86c6002d954167796e41886a47c47d95a626d"
    }
  ],
  "source": "."
}
...
```sh
dagger uninstall github.com/shykes/daggerverse/hello@v0.3.0
dagger uninstall github.com/shykes/daggerverse/hello
