...
Dagger lets you import [private Go modules](https://go.dev/ref/mod#private-modules) in your Dagger module. To do this, add the URL to the private repository(ies) hosting the module(s) in a `goprivate` key in the module's `dagger.json` file, as shown below:
...
Note that this feature requires [SSH authentication to be properly configured](./remote-modules.mdx#ssh-authentication) on your Dagger host.
</TabItem>
