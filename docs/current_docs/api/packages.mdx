...
Dagger also lets you import [private Go modules](https://go.dev/ref/mod#private-modules) in your Dagger module. To do this, add the URL to the private repository(ies) in the module's `dagger.json` file, as shown below:

```json
{
  "sdk": {
    "source": "go",
    "goprivate": "git@github.com:username/private-repository.git
  },
}
```

Note that this requires [SSH authentication to be properly configured](./remote-modules.mdx#ssh-authentication) on your Dagger host.
</TabItem>
