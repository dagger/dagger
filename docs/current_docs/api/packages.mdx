...
Note that this feature requires [SSH authentication to be properly configured](./remote-modules.mdx#ssh-authentication) on your Dagger host, and also requires a `.gitconfig` file entry to use SSH instead of HTTPS for the host. Use the command `git config --global url."git@github.com:".insteadOf "https://github.com/"` to create the necessary `.gitconfig` entry.
</TabItem>
