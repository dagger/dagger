...
Inside a module, you don't take the user's project directory as a function argument. Instead, your module's constructor receives a [`Workspace`](../../reference/sdks/index.mdx#developer-workflow) that Dagger auto-populates from the current workspace, and you read directories and files from it lazily with `Workspace.directory(path)` and `Workspace.file(path)` — nothing is uploaded until a function actually uses it. See your [SDK guide](../../reference/sdks/index.mdx) for the exact syntax.
