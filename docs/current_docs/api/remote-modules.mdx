...
### Using Git credentials with tokens

The examples below demonstrate how to set up Git credentials using tokens, which works in both local development and CI environments. For CI environments specifically, these examples show how to use provider-supplied tokens and avoid persistent credential storage.
...
- For private repositories:
  - HTTPS: Ensure your Git credentials are properly configured using your provider's recommended method.
    - For local development: Use your provider's CLI tool or Git Credential Manager
    - For CI environments:
      - GitLab `CI_JOB_TOKEN` -> username: `gitlab-ci-token`
      - GitHub tokens -> username: `x-access-token`
      - BitBucket App Passwords -> username: your BitBucket username
