...
This is why the initial `pyproject.toml` file doesn't include any [dependencies](./packages.mdx).

### UV installer

[uv](https://github.com/astral-sh/uv) is used internally to create the virtual environment, install dependencies and create the [lock file](./packages.mdx#lock-file) (starting with Dagger v0.11.0). It's possible to specify a newer version, if needed, in `pyproject.toml`:
...
Note, however, that [pinning](./packages.mdx#lock-file) via `requirements.lock` is only supported with `uv`.
</TabItem>
<TabItem value="TypeScript">
