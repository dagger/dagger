...
```shell
dagger shell -c 'build-env .'
```

:::tip POSITIONAL AND OPTIONAL ARGUMENTS
When passing values to Dagger Functions with `dagger shell -c`, order matters. Required arguments are positional and don't have flags, while optional ones can be placed anywhere and have flags. In this Dagger Function, `source` is the first required argument and so Dagger automatically matches it with the first value passed (the current directory `.`).
:::
...
:::tip FUNCTION NAMES
When calling Dagger Functions with `dagger`, all names (functions, arguments, fields, etc.) are converted into a shell-friendly "kebab-case" style. This is why Dagger Functions named `FooBar` in Go, `foo_bar` in Python and `fooBar` in TypeScript/PHP/Java are called with `dagger shell -c foo-bar ...`.
:::

### Interact with a just-in-time container

Just-in-time containers come with useful functions of their own, which you can call by chaining them to the artifact in the `dagger` command. One of the most interesting functions is `terminal`, which can be used to [open an interactive terminal session with the running container](../../features/debugging.mdx).
