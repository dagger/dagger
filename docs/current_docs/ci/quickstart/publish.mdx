...
```shell
# all equivalent
dagger shell -c 'base | with-exec apk,add,bash,git | publish ttl.sh/bar'
dagger shell -c 'build | publish ttl.sh/bar'
dagger shell -c 'build-and-publish'
```
</TabItem>
</Tabs>
:::
