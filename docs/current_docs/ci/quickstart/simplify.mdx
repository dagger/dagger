...
As before, you should see the application being tested, built, and published to the [ttl.sh container registry](https://ttl.sh). With the default path in place, everything works as before, even though no argument is specified in the Dagger Function call.
