...
A service that `dagger up` can start is a module function that returns a `Service` and carries your SDK's up pragma. In Go that is `// +up`, in Python `@up`, in TypeScript `@up()`. The SDKs guide covers how services fit next to checks and generators under [Checks, generators, and services](../sdks/index.mdx#checks-generators-and-services). The syntax for each language lives in its SDK guide:

- [Dang](../sdks/dang.mdx#up-services)
- [Go](../sdks/go.mdx#up-services)
- [TypeScript](../sdks/typescript.mdx#up-services)
- [Python](../sdks/python.mdx#checks-generators-and-services)
- [Java](../sdks/java.mdx#up-services)
