...
:::note
These examples are using the shell, you can enter the shell by running `dagger` in your terminal.
:::
...
This will return a container based on the alpine image, next lets access a terminal in the container:
...
If we wanted to install a package in the container we can do that using `with-exec`:
...
● Attaching terminal:
    container: Container!
    .from(address: "docker.io/library/alpine:latest@sha256:a8560b36e8b8210634f77d9f7f9efd7ffa463e380b75e2e74aff4511df3ef88c"): Container!
    .withExec(args: ["apk", "add", "go"]): Container!
...
:::tip
If you get stuck, you can always run the `.help` command to get a list of available commands. If you need help with a specific command, you can run `.help <command>` to get more information.
:::
