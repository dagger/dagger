...
Most per-environment differences are module settings, such as a base image or a package manager. Set them with `dagger module settings` and `--env`, which targets that environment's overlay instead of the base configuration:

```shell
dagger module settings --env staging eslint baseImageAddress node:22-alpine
```
...
```shell
dagger module settings --env staging eslint
```
...
Values that shouldn't be committed, such as private profiles, local paths, and
personal clusters, belong in your [user configuration](./user.mdx), which can
also define environments of its own on top of the repository's.
