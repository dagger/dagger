...
Most per-environment differences are module settings, such as a base image or a package manager. Set them with `dagger settings` and `--env`, which targets that environment's overlay instead of the base configuration:
