...
**Modules are managed from the top level.** `dagger module search` finds them; `dagger module install` and `dagger module uninstall` update `dagger.toml`. A module's own source metadata lives in `dagger-module.toml`. Edit that file directly when authoring a module or adding code dependencies.
