...
## Wire a list of objects

A constructor argument typed as a list of objects (`[File!]`, `[Directory!]`,
`[Container!]`, `[Service!]`, ...) wires the same way: the setting is an array
of references, and each element resolves independently.

```toml
[modules.packager.settings]
files = ["builder:binary", "docs:readme"]
```
...
The [Playwright module](../reference/modules/js/playwright.mdx) is a worked example of all
three. [Daggerize a Go Project](../getting-started/platform-guides/go/index.mdx) wires a project's own
