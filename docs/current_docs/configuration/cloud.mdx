...
To initiate this process, navigate to the organization page using the cogwheel icon in the top navigation bar and select the `Integrations` sub-menu. Click `Add new target` to add a GitHub account or organization.
...
Complete the process and then, once the GitHub account or organization appears in the Dagger Cloud user interface, click `Map to this organization` to have Dagger Cloud begin scanning for Dagger modules.
...
Modules are automatically categorized by their source repository. However, it is possible to create custom views of modules filtered by team, service, or domain. To create a custom view, click the `+` icon.
