...
The dagger engine is designed to be run out-of-the-box with sensible defaults.
However, even with these defaults, it's sometimes needed to tweak various
settings.

TODO: update version when base PR is merged

To do so, there are two config mechanisms: using dagger's `engine.json` file
(introduced in v0.14.X), or using the legacy buildkit-style `engine.toml` file.
The `engine.json` file is a configuration file that follows the [Engine config schema](https://docs.dagger.io/reference/engine.schema.json),
while the legacy `engine.toml` file uses [BuildKit's config syntax](https://github.com/moby/buildkit/blob/v0.17.0/docs/buildkitd.toml.md)).
...
To configure `engine.json` when using an automatically provisioned engine (the
default), you should write your desired JSON config to `~/.config/dagger/engine.json`
(or to `$XDG_CONFIG_HOME/dagger/engine.json` if set).

Then, when an engine is automatically started from a `dagger` command, it will
pull in that config - changes after the engine has already been created and
is running will **not** automatically apply to the new engine. To reapply the
config after changing it, you'll need to manually stop the engine.

```shell
docker rm -f $(docker ps -q --filter "name=dagger-engine-*")
```

If you are not using an automatically provisioned engine, and have followed the
steps to setup a [custom runner](./custom-runner), then you'll need to manually
mount your `engine.json` to the engine container's filesystem at
`/etc/dagger/engine.json`. For example:
...
To configure `engine.toml`, a [custom runner](./custom-runner) is required -
unlike with `engine.json`, the config is **not** automatically mounted to the
engine.
...
Note, that by default, the engine container itself contains a default
...
The dagger engine supports various log verbosity levels - the default level is to enable `debug`.
...
The engine by default has an open security policy. This policy allows exec
...
The garbage collector runs in the background of the dagger engine,
...
To adjust the garbage collector fine-grained policies:
...
Currently, custom configuration authorities cannot be configured through
`engine.json` or `engine.toml`, and must be configured [separately](./proxy.mdx).
