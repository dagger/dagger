...
"Rootless mode" means running the Dagger Engine as a container without the `--privileged` flag. In this case, the container would not run as the `root` user of the system. Currently, the Dagger Engine cannot be run as a rootless container; [network and filesystem constraints related to rootless usage](../faq.mdx#why-does-the-dagger-engine-need-to-run-in-a-privileged-container) would currently significantly limit its capabilities and performance.
:::
