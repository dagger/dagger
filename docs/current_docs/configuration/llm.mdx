...
Dagger supports a wide range of popular Large Language Models (LLMs), including those from OpenAI, Anthropic, Google, Microsoft, and Amazon. Dagger can access these models either through their respective cloud-based APIs or using a local provider like Ollama.
