...
If `dagger.runtime` is not set, you must execute `bun init` or `bun install` to create a `bun.lock` or `bun.lockb` file, which Dagger will then [automatically detect](#automatic-detection) and configure Bun for use:
...
<TabItem value="Deno">
Execute `deno init` to create a `deno.json` file, which Dagger will [automatically detect](#automatic-detection) and configure Deno for use:
