...
Alternatively, executing `bun init` or `bun install` will create a `bun.lock` or `bun.lockb` file, which Dagger will [automatically detect](#automatic-detection) and configure Bun for use:
...
<TabItem value="Deno">
Set the field to `deno` to use the Deno runtime.
...
Alternatively, executing `deno init` will create a `deno.json` file, which Dagger will [automatically detect](#automatic-detection) and configure Deno for use:
