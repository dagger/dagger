...
If `dagger.runtime` is not set, but a `bun.lock` or `bun.lockb` file is present, Dagger will then [automatically detect](#automatic-detection) and configure Bun for use. You can generate this file with `bun init` and `bun install`, as in the example below:
...
<TabItem value="Deno">
If a `deno.json` file is present, Dagger will then [automatically detect](#automatic-detection) and configure Deno for use. You can generate this file with `deno init`, as in the example below:
