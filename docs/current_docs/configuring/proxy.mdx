...
As mentioned above, these proxy environment variables set on Dagger will also be automatically set on all containers created by userspace Dagger Functions (i.e. containers created via a `withExec` API call).
...
Additionally, if the `withEnvVariable` API is used to explicitly set any of those proxy environment variable values, those will override any settings inherited from Dagger's proxy configuration.
