...
```go file=./snippets/copy-dir-runtime/go/main.go
```

</TabItem>
<TabItem value="Python">

```python file=./snippets/copy-dir-runtime/python/main.py
```

</TabItem>
<TabItem value="TypeScript">

```typescript file=./snippets/copy-dir-runtime/typescript/index.ts
```
...
Dagger represents operations performed by a Dagger Function as [OpenTelemetry spans](https://opentelemetry.io/docs/concepts/signals/traces/#spans). If a Dagger Function emits custom OpenTelemetry spans, the related information will appear in the Dagger TUI and Traces. The following Dagger Function demonstrates this by emitting custom spans for various file operations.

<Tabs groupId="language">
<TabItem value="Go">

```go file=../api/snippets/logging/go/custom-spans/main.go
```

</TabItem>
<TabItem value="Python">
```python file=../api/snippets/logging/python/custom-spans/main.py
```

</TabItem>
<TabItem value="TypeScript">

```typescript file=../api/snippets/logging/typescript/custom-spans/index.ts
```
...
Execute a Dagger Function to create three files in an `alpine` container and return the resulting directory, injecting a custom log message for each file operation using OpenTelemetry spans:
