...
If a Dagger Function emits custom OpenTelemetry spans, the related information will appear in the Dagger TUI and Traces. The following Dagger Function demonstrates this by emitting custom spans for various tasks.
...
Execute a Dagger Function to lint, format, type-check and run unit tests on the `dagger/hello-dagger` source code repository, emitting a custom span for each operation:
