...
:::note
If you need SSH-based cloning, including private or public Git repositories with an SSH ref format, select the (SSH) tabs below. This approach requires forwarding `SSH_AUTH_SOCK`. See [Security-by-Design Docs](../api/security-model.mdx) for why Dagger enforces this pattern. You could also directly use the `Directory` pattern as shown in the [Directory cookbook example](#copy-a-directory-or-remote-repository-to-a-container)
:::
