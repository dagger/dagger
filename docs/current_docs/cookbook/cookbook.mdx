...
The following Dagger Function demonstrates how to use native-language concurrency features ([goroutines](https://gobyexample.com/goroutines) in Go, [task groups](https://docs.python.org/3/library/asyncio-task.html) in Python), and [promises](https://basarat.gitbook.io/typescript/future-javascript/promise) in TypeScript to execute other Dagger Functions in parallel.
