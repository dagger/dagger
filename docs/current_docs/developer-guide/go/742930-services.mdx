...
## Work with service hostnames and ports

Each service container has a canonical, content-addressed hostname and an optional set of exposed ports. You can query a service container's canonical hostname with the `Service` type's `Hostname` function.

Here is an example of a Dagger Function that creates a service and returns its hostname:

```go file=./snippets/services/hostnames/main.go
```

Here is an example call for this Dagger Function:

```shell
dagger call http-service
```
...
Here is an example of a Dagger Function that creates a service and sets the port on which the service container will listen. Note also the `Endpoint` function of the `Service` object, which returns an address pointing to a particular port, optionally with a URL scheme. You can either specify a port or let Dagger pick the first exposed port.

```go file=./snippets/services/ports/main.go
```
