...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "MyModule":
		...
		case "Tree":
			...
			var depth string
			if inputArgs["depth"] != nil {
				err = json.Unmarshal([]byte(inputArgs["depth"]), &depth)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg depth", err))
				}
			}
			return (*MyModule).Tree(&parent, ctx, dir, depth)
		default:
				...
				dag.TypeDef().WithObject("MyModule").
					WithFunction(
						dag.Function("Tree",
							...
							WithArg("depth", dag.TypeDef().WithKind(StringKind)))), nil
	default:
