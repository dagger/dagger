...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "MyModule":
		...
		case "GetUser":
			...
			var gender string
			if inputArgs["gender"] != nil {
				err = json.Unmarshal([]byte(inputArgs["gender"]), &gender)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg gender", err))
				}
			}
			return (*MyModule).GetUser(&parent, ctx, gender)
		default:
		...
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("MyModule").
					WithFunction(
						dag.Function("GetUser",
							dag.TypeDef().WithKind(StringKind)).
							WithArg("gender", dag.TypeDef().WithKind(StringKind)))), nil
	default:
