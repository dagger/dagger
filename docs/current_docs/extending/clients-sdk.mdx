...
Dagger SDKs can also be used to build [custom applications](./custom-applications/index.mdx) that use the Dagger API. These applications can be standalone or integrated into existing systems, allowing you to leverage Dagger's capabilities in your own software solutions.
:::
