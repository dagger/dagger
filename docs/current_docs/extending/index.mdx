...
Install a module into a workspace ([`dagger.toml`](/reference/configuration/workspace)) to make it part of a project's command surface; publish a reusable module when several projects need the same capability. See [Workspace Setup](/adopting/workspace-setup).
...
1. **Scaffold** a new module — the SDK writes its `dagger-module.toml` and a source skeleton, returned as a [changeset](/using-dagger/changesets) you apply.
...
- **Type inputs honestly.** Prefer real Dagger types over strings, defaults for conventions, [settings](/reference/configuration/workspace) for team-wide values, and `Secret` for credentials — not magic environment variables or assumed host paths.
...
- **Checks** validate the project — tests, linters, scans, policy. They run with `dagger check`, locally, in CI, and as [Cloud Checks](/adopting/triggers). See [Checks](./how-dagger-works/checks.mdx).
- **Generators** produce source changes as reviewable [changesets](/using-dagger/changesets) — codegen, formatting, lockfile updates. They run with `dagger generate`.
...
Round these out with the rest of the platform: [settings and environments](/reference/configuration/workspace) for team and per-context defaults, explicit `Secret` inputs for credentials, and [Dagger Cloud](/adopting/observability) for traces and remote execution.
