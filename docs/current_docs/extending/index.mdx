...
Install a module into a workspace ([`.dagger/config.toml`](/reference/configuration/workspace)) to make it part of a project's command surface; publish a reusable module when several projects need the same capability. See [Workspace Setup](/adopting/workspace-setup).
...
1. **Scaffold** a new module — the SDK writes its `dagger.json` and a source skeleton, returned as a [changeset](/using-dagger/changesets) you apply.
