...
Dagger [modules](../features/reusability.mdx) are collections of Dagger Functions. When you give a Dagger module to the `LLM` type, every Dagger Function is turned into a tool that the LLM can call.
