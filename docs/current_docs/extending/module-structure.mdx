...
The runtime container is currently hardcoded to run in Node.js 22.11.0, but it can be overridden by [setting an alternative base image](../reference/configuration/modules.mdx#alternative-base-images).
...
:::note
To configure the SDK as a local vendored dependency, read [bundled or vendored SDK](../reference/configuration/modules.mdx#bundled-or-vendored-sdk).
:::
