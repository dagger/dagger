...
The reason for this is that some functions may require a specific TTL, "session" or "never" caching in order to behave as expected. For example, if a function pulls data from an external network service, it may not be desirable for it to have the default TTL of 7 days in the case where that data changes more frequently than once a week.
