...
The runtime container is based on the [python:3.14-slim](https://hub.docker.com/_/python/tags?name=3.14-slim) base image by default, but it can be overridden by setting [`requires-python`](https://packaging.python.org/en/latest/guides/writing-pyproject-toml/#python-requires), or pinned with a `.python-version` file next to your `pyproject.toml`:
