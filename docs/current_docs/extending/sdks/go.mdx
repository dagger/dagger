...
Like every Dagger tool that modifies your workspace, `init` returns a [changeset](/using-dagger/changesets) — a structured diff of the files to create — which Dagger shows you to review before anything is written to disk.
...
Manage dependencies with the SDK module's `mod deps` operations rather than hand-editing `dagger-module.toml`. Each returns a [changeset](/using-dagger/changesets) for you to review and apply.
