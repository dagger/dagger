...
├── .gitignore                    # ignores generated files by default
└── internal/
...
:::note
`dagger module init` writes `dagger-module.toml` and `main.go` and updates the workspace config. The first [binding regeneration](#regenerate-bindings-and-generated-files) produces the Go module files and generated client; generated code paths are git-ignored by default.
:::
...
These files are generated, not authored. The generated `.gitignore` excludes them by default, and `.gitattributes` marks them as `linguist-generated` for repositories that choose to commit them:
...
:::tip
Generated SDK files are git-ignored by default. To commit them instead, opt out of automatic gitignore management in `dagger-module.toml`:
...
1. **Choose a generated-file policy.** Generated files are git-ignored by default. If you set `codegen.automaticGitignore = false`, commit `dagger.gen.go` and `internal/` with each change.
2. **Version with Git tags.** Releases are Git refs. Tag a release (e.g. `v1.2.0`) and consumers pin it with `@v1.2.0` in the reference.
3. **Pin a sensible engine version.** Use `dagger module engine require` so consumers know which engine your module targets.
