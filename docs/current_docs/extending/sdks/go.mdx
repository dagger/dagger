...
The Go SDK is itself a Dagger module — `github.com/dagger/go-sdk`. Install it into your workspace once, then scaffold, generate, and maintain Go modules with its dynamic SDK commands:
...
# Create a Go module
dagger sdk go module init my-module
```
...
Like every Dagger tool that modifies your workspace, `dagger sdk go module init` returns a [changeset](../../using-dagger/changesets.mdx) — a structured diff of the files to create — which Dagger shows you to review before anything is written to disk.

:::tip
If the repository does not have a `dagger.toml` yet, use `dagger install --here github.com/dagger/go-sdk` to create it in the current directory.
:::

### Where the module is created

By default, `dagger sdk go module init` places the new module beside the `dagger.toml`
it is editing:
...
```shell
dagger sdk go module init my-module --path ci     # ./ci
dagger sdk go module init my-module --path /ci    # <workspace root>/ci
```
...
```shell
dagger sdk go module init my-module --legacy-template
```

List the Go SDK's module initialization options with:

```shell
dagger sdk go module init --help
```
...
:::note
`dagger sdk go module init` writes `dagger-module.toml` and `main.go`, updates the workspace config, and runs the Go SDK's generator for the new module — so `go.mod`, `go.sum`, and the generated Go files shown above are part of the same changeset. Commit the generated Go files along with the rest of the module. Dagger needs the generated client to load the module, both from your local checkout and from a Git reference. Pass `--no-generate` to scaffold without generating.
:::
...
`runtime.source = "go"` tells the engine to use the Go runtime. The workspace's `dagger.toml` separately records the SDK under `modules.go-sdk.as-sdk`; that authoring relationship is how Go binding generation discovers workspace modules. The generated files in `dagger.gen.go` and `internal/` are not handwritten; see [Regenerate bindings](#regenerate-bindings-and-generated-files).
...
`dagger sdk go module init` creates a `go.mod` and `go.sum` inside your Dagger module. These files tell Go which dependencies the generated client needs. If the Dagger module lives inside a larger Go project, the additional `go.mod` can make an editor choose the wrong Go module for a file.
...
**`dagger init` / `dagger develop` not found.** Install the Go SDK with `dagger install github.com/dagger/go-sdk`, scaffold with `dagger sdk go module init <name>`, and regenerate with `dagger generate`.

**`dagger install` reports "no current workspace".** Add `--here` to create `dagger.toml` in the current directory: `dagger install --here github.com/dagger/go-sdk`.

**Nothing was written after `dagger sdk go module init`.** The command returns a changeset. Review and accept it, or rerun with `-y` to apply without prompting.
