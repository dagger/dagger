...
This page is a standalone guide to the Go SDK. It assumes you already understand the platform concepts covered in the [Module Developer Guide](/extending):

- [Type system](/extending/type-system) — how SDK types map to the Dagger API
- [Checks](/extending/how-dagger-works/checks) — functions that validate your code
- [Changesets](/using-dagger/changesets) — how generators and tooling return diffs for you to apply
...
Module development tooling lives in the SDK module, not the `dagger` CLI: **the Go SDK is itself a Dagger module — `github.com/dagger/go-sdk`** — and you scaffold, generate, and maintain Go modules by calling its functions:
...
`init` does **not** write files directly. Like every Dagger tool that modifies your workspace (for example `dagger call prettier write`, or a module's generators), it returns a [changeset](/using-dagger/changesets) — a structured diff of files to create. You then choose how to apply it.
...
:::tip
You can list the Go modules in a workspace with `dagger -m github.com/dagger/go-sdk call modules` — it returns every module whose `runtime.source` is `"go"`.
:::
...
Manage dependencies with the SDK module's `mod deps` operations rather than hand-editing `dagger-module.toml`. Each returns a changeset you apply directly (review, or pass `-y` to apply without prompting).
...
`dagger generate` (the user-facing command described in [Generating Code](/using-dagger/generating)) discovers and runs your module's generators, which for a Go module includes this binding regeneration. Both paths produce a [changeset](/using-dagger/changesets) you review and apply.
...
A useful, reusable module provides at least one of the three first-class function types — a **check**, a **generator**, or a **service** — so that the platform verbs (`dagger check`, `dagger generate`, `dagger up`) have something to run. These work the same in Go as in any SDK; see the [Module Developer Guide](/extending) for the full treatment. In Go, you mark each one with a doc-comment **pragma** on the function:
...
See [Generating Code](/using-dagger/generating) and [Changesets](/using-dagger/changesets).
...
**`dagger init` / `dagger develop` not found.** Module development tooling lives in the Go SDK module, not the CLI. Use `dagger -m github.com/dagger/go-sdk call init ...` to scaffold and `... call mod generate ...` to regenerate.
...
**A new function or dependency doesn't show up.** Regenerate bindings with `dagger -m github.com/dagger/go-sdk call mod generate` (or `dagger generate`). The API surface comes from the generated `internal/dagger` package, which must be in sync with your code and `dagger-module.toml`.
...
- [Module Developer Guide](/extending) — platform concepts that apply to every SDK
- [Type system](/extending/type-system)
- [Checks](/extending/how-dagger-works/checks)
- [Generating Code](/using-dagger/generating) and [Changesets](/using-dagger/changesets)
