...
`init` creates a new Python SDK module and returns a **changeset** — a diff of files to write — rather than writing directly to disk. This is the same review-then-apply flow as any Dagger generator (see [Changesets](../../using-dagger/changesets.mdx)).
