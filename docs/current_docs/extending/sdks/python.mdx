...
dagger mod install github.com/dagger/python-sdk

# Then call its functions
dagger call python-sdk <function> ...
```

```shell
dagger call python-sdk init --help
```
...
`init` returns a [changeset](/using-dagger/changesets) — a diff of the files to write — which Dagger shows you to review before anything is written to disk.
...
dagger call python-sdk mod deps list

# Update one dependency by name, or all remote deps if --name is omitted
dagger call python-sdk mod deps update --name=hello

# Remove a dependency by name or source
dagger call python-sdk mod deps remove --name=hello
```
...
```shell
dagger call python-sdk mod generate
```
...
dagger call python-sdk mod engine required

# Require a specific version
dagger call python-sdk mod engine require --version=v1.0.0-beta.3

# Pin to the engine you're running now
dagger call python-sdk mod engine require-current

# Pin to the latest stable release
dagger call python-sdk mod engine require-latest
```
...
dagger call python-sdk mod config get
...
- **`dagger init` / `dagger develop` not found.** Module development tooling lives in the Python SDK module, not the CLI. Install it with `dagger mod install github.com/dagger/python-sdk`, then use `dagger call python-sdk init …` and `dagger call python-sdk mod generate`.
