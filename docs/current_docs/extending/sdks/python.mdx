...
Module development tooling lives in the **Python SDK module itself**: `github.com/dagger/python-sdk`, not the `dagger` CLI. You scaffold and maintain modules by calling its functions:
...
- **`dagger init` / `dagger develop` not found.** Module development tooling lives in the Python SDK module, not the CLI. Use `dagger -m github.com/dagger/python-sdk call init …` and `… mod generate …`.
