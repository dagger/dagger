...
- **`dagger init` / `dagger develop` not found.** Module development tooling lives in the Python SDK module, not the CLI. Install it with `dagger install github.com/dagger/python-sdk`, then use `dagger call python-sdk init …` and `dagger call python-sdk mod generate`.
