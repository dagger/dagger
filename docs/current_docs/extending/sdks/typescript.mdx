...
- [Module Developer Guide](/extending) — what a module is and how it runs
- [Type System](/extending/type-system) — how language types map to the Dagger API
- [Checks](/extending/how-dagger-works/checks) — how `dagger check` discovers and runs checks
- [Changesets](/using-dagger/changesets) — how generated files are reviewed and applied
...
`init` returns a [changeset](/using-dagger/changesets) describing the files to
...
the Dagger type system (see [Type System](/extending/type-system)):
...
module. See [Checks](/extending/how-dagger-works/checks).
...
A generator produces a [changeset](/using-dagger/changesets) — a diff between the
...
starts every service in the module. See [Services](/using-dagger/services).
