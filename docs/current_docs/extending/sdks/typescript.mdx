...
```shell
dagger mod install github.com/dagger/typescript-sdk
dagger call typescript-sdk init --name=my-module
```

`init` returns a [changeset](../../using-dagger/changesets.mdx) describing the files to
...
:::note
`init` writes only your source (`dagger-module.toml`, `package.json`, `tsconfig.json`, `src/`). The generated `sdk/` client is produced by the first [binding regeneration](#regenerate-bindings) — committed by default — not by `init` itself.
:::
...
return `Promise`s (every call is remote). Any object that provides matching
