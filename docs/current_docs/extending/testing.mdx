...
mkdir tests
cd tests
dagger module init --sdk=go --source=. tests
...
mkdir tests
cd tests
dagger module init --sdk=python --source=. tests
dagger module install ..
```
...
mkdir tests
cd tests
dagger module init --sdk=typescript --source=. tests
dagger module install ..
```
...
mkdir -p examples/go
cd examples/go
dagger module init --sdk=go --source=. examples/go
dagger module install ../..
```
...
cd examples/python
dagger module init --sdk=python --source=. examples/python
dagger module install ../..
```
...
dagger module init --sdk=typescript --source=. examples/typescript
dagger module install ../..
```
...
In some situations, you may need to provide specific values to your test modules, such as when authenticating against an external service. In these cases, you can rely on [module constructors](./editions/dang#configuration-with-defaults) to inject any required inputs.
