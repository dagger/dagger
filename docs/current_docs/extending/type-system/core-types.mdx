...
Each type below links to its [reference page](../types/index.mdx) for the full field listing. The guidance here is about *design*: when to accept it, when to return it, and the mistake to avoid.

### `Container`

A filesystem, environment, execution, and publish pipeline in one value. See [`Container`](../types/container.mdx).
...
A content-addressed tree. See [`Directory`](../types/directory.mdx).
...
A content-addressed file. See [`File`](../types/file.mdx).
...
Sensitive data passed without exposing plaintext. See [`Secret`](../types/secret.mdx).
...
A long-running endpoint for other work to use. See [`Service`](../types/service.mdx).
...
A proposed diff against a directory or workspace. See [Changesets](../../using-dagger/changesets.mdx).
...
A set of typed inputs and outputs that can pass through a workflow, including to an LLM. See [`Env`](../types/env.mdx).
...
Model interaction when enabled for a module. See [`LLM`](../types/llm.mdx).
...
This chapter teaches how module authors should think with core types. It does not list every field — that is the job of the [Types Reference](../types/index.mdx). When deciding *whether* to accept or return a type, stay here; when deciding *which field* to call, go there.
