...
- Whether a default list comes from [workspace settings](/reference/configuration/workspace).
...
- **`registry` from settings.** A team-wide value moves to a [workspace setting](/reference/configuration/workspace) so it is not repeated on every call or hard-coded in the module.
