...
The CI view shows all the recent changes that were made to a hosted repository where Dagger was triggered with a `DAGGER_CLOUD_TOKEN` environment variable set. This view shows a list of grouped traces. The group is determined by the branch/reference combination where Dagger was triggered.

### What is the "focused view" of a group (when clicking a group in the CI view)?

The "focused" view is used for pinning a specific point in your Git history and listening for Traces emitted under that context.

### What is the flamegraph at the top of the Trace detaills page?

The flamegraph shows the emitted spans of a trace, with a visual cue of how much time it represents of the parent span.
