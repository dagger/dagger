...
- It may sometimes be necessary to explicitly force execution of specific workflow operations, bypassing the Dagger layer cache. The typical approach for this is to [invalidate the Dagger layer cache](../cookbook/index.mdx#invalidate-cache) by introducing a volatile time variable at a specific point in the Dagger workflow
...
- [Create and use cache volumes](../extending/cache-volumes.mdx)
- [Invalidate the layer cache]
- [Inspect and prune the cache](../reference/configuration/cache.mdx)
