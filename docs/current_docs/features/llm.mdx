...
Dagger has built-in experimental MCP support that allows you to easily serve a Dagger module (and optionally Dagger core API functions) as an MCP server. The MCP client/agent must implement the [dynamic tool discovery and updates](https://modelcontextprotocol.io/docs/concepts/tools#tool-discovery-and-updates) portion of protocol. In our testing, only [Goose](https://github.com/block/goose) fully implements this today.

```mermaid
flowchart TD
    A["MCP Client<br>e.g. Goose"] -- "dagger -m &lt;module&gt; mcp " <--> B["Dagger MCP Server: tools"]
    B <--> C["Dagger module: functions"]
```
...
extensions:
  dagger:
    name: Demo
    description: "Dagger module and core API as MCP server in Goose"
    enabled: true
    cmd: /opt/homebrew/bin/dagger   # your dagger cli >= 0.18.6
    args:
    - -m
    - github.com/jpadams/daggerverse/trivy  # dagger module to use
    - mcp
    - --env-privileged     # add core api: host, container, directory, git, ...
    envs:
      DAGGER_CLOUD_TOKEN: <your https://dagger.cloud token>
