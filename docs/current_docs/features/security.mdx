...
Secrets can also be read from external secret managers:

#### Hashicorp Vault

Using Vault as the secret provider:
...
Using 1Password as the secret provider. The secret is passed from 1Password via the `op` provider. This requires the Dagger CLI to be authenticated with 1Password, which can be done by running `op signin` in the terminal.
...
Using Google Cloud Secret Manager as the secret provider. This requires authentication via Application Default Credentials (ADC) or the `GOOGLE_APPLICATION_CREDENTIALS` environment variable.

```shell
# Using just the secret name (requires GCP_PROJECT_ID environment variable)
dagger call github-api --token=gcp://my-github-token

# Using secret name with specific version
dagger call github-api --token=gcp://my-github-token/versions/2

# Using full resource name
dagger call github-api --token=gcp://projects/my-project/secrets/github-token

# Using secret with TTL for caching
dagger call github-api --token="gcp://my-github-token?ttl=5m"
```
