...
:::note
Dagger automatically detects OpenTelemetry resource attributes. By utilizing the standard `OTEL_RESOURCE_ATTRIBUTES` environment variable, operators can now set custom resource attributes to annotate Traces, providing more detailed and contextual information for monitoring and debugging. This improvement aligns Dagger’s observability capabilities with industry standards, facilitating better integration with existing monitoring tools and workflows.
:::
