...
This quickly creates the need to visualize your pipelines to understand and measure what's going on. Dagger includes two features for this purpose - a [Terminal UI](#terminal-ui) and [Dagger Cloud's Traces](#traces).

## Terminal UI

The [Dagger CLI](../reference/cli/) includes a real-time visualization feature called the terminal UI (TUI). It shows a full-blown DAG in a style similar to `git log --graph`, with the current state of your DAG's evaluation and a full snapshot of all output at the end.
...
:::info
Dagger Cloud is a hosted service to help visualize your pipelines in a friendly way. You can create a free account by visiting [dagger.cloud](https://dagger.cloud).
:::
...
- **Duration**: See exactly how long each Dagger Function in your pipeline takes. This helps identify bottlenecks and optimize your workflows for faster execution.
