...
Dagger Functions are just like regular functions: they accept [arguments](../extending/arguments.mdx) and [return values](../extending/return-types.mdx). In addition to common types (string, boolean, integer, arrays...), the Dagger API also defines powerful [types](../extending/types.mdx) which Dagger Functions can use, such as `Directory`, `Container`, `Service`, `Secret`, and many more.
