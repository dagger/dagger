...
- you know [the basics of Dagger](core-concepts.mdx).
...
```go Go file=./ci/snippets/go/main.go
```
...
```python file=./ci/snippets/python/__init__.py
```
...
```typescript file=./ci/snippets/typescript/index.ts
```
...
```php file=./ci/snippets/php/src/HelloDagger.php
```
...
```java file=./ci/snippets/java/src/main/java/io/dagger/modules/hellodagger/HelloDagger.java
```
...
The `build-env` Dagger Function returns a `Container` type representing the application's build environment. One of the most interesting built-in functions of this type is `terminal`, which can be used to [open an interactive terminal session with the running container](../features/observability.mdx).
