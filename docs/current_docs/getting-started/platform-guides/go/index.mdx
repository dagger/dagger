...
`postgres` describes the database. The `@start` directive also makes it a service that `dagger start` runs for local development, so one definition serves both purposes.
