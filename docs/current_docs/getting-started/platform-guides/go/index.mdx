...
- **`test`**: which Go modules are tested, as module-root selectors. The `go` module covers every `go.mod` it finds, including ones under directories like `examples/` or `tools/`. To cover only the root module, set it to `["."]`. To cover everything except one module, keep `"**"` and add a `!`-prefixed path, as in `["**", "!legacy-service", "!test-fixtures"]`.
