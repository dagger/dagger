...
- **`includeExtraFiles`**: files the `go` module does not mount on its own. By default each Go command sees the module's Go source files, its `go.mod`, `go.sum`, and `go.work`, the files named by `go:embed` directives, and local `replace` targets, and tests also see every `testdata/` directory. Anything else a test opens by relative path can be listed here as a pattern from the repository root. A file that only one package needs is better declared next to that package with a directive, covered in [Declare the files a test or generator reads](#declare-the-files-a-test-or-generator-reads).
- **`test`**: which Go modules are tested, as module-root selectors. The `go` module covers every `go.mod` it finds, including ones under directories like `examples/` or `tools/`. To cover only the root module, set it to `["."]`. To cover everything except one module, keep `"**"` and add a `!`-prefixed path, as in `["**", "!legacy-service"]`.
- **`generate`**: which directories `go generate` runs in, as paths and globs from the repository root. A literal path selects only that directory, `internal/**` selects `internal` and everything below it, and a `!` prefix excludes.
...
The [Go module reference](../../../reference/modules/go.mdx) describes every setting.
...
By default the `go` module runs every Go command in `golang:<version>-alpine`. The most common reason to provide a custom runtime is if your tests or generators require system dependencies such as `git`, a C compiler for cgo, or `protoc`. The `base` setting takes that runtime, and module wiring lets a function of your own build it.

### Create a runtime module

The runtime is a few lines of [Dang](../../../reference/sdks/dang.mdx) in a module of your own. Install the Dang SDK into the workspace if it is not there already, then create the module if you do not already have a dev module for your project:
...
`dagger module init` creates the module at `.dagger/modules/<project>-dev` and records it in `dagger.toml`. Add the following function to your module:
...
- **The image sets the Go version.** Once `base` is wired, the toolchain is whatever your container provides. The `go` module requires this container to have Go 1.26 or newer.

Open a shell in the container the `go` module will receive, and run `protoc --version` to confirm the package is there:

```shell
dagger shell <project>-dev:test-runtime
```
