...
- **`version`**: one Go toolchain for every Go module in the workspace. Without it, each module is tested and generated with the toolchain its own `go.mod` asks for, in a `golang:<version>-alpine` image.
- **`base`**: a container to use instead of that image. It supplies its own toolchain, so `version` is ignored alongside it. The second half of this guide sets `base`.
