...
`postgres` describes the database. The `@start` directive also makes it a service that `dagger start` starts for local development, so one definition serves both purposes.
