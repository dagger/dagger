...
# Replace a Dockerfile with Go (or Python, or TypeScript)

## Introduction

This guide explains how to use a Dagger SDK to perform all the same operations that you would typically perform with a Dockerfile, except using Go, Python or TypeScript. You will learn how to:
...
The Dagger SDK enables you to develop a CI/CD pipeline in one of the supported languages (Go, Python or TypeScript) to achieve the same result as using a Dockerfile.
...
```typescript file=./snippets/replace-dockerfile/index.mjs
```

There's a lot going on here, so let's step through it in detail:

- The TypeScript CI pipeline imports the Dagger SDK and creates a Dagger client with `connect()`. This client provides an interface for executing commands against the Dagger engine.
