...
- **Checks**: A fully managed CI platform with integrated compute and caching. Dagger Cloud runs your [check functions](/core-concepts/checks) on managed infrastructure and reports results back to GitHub as commit statuses — no self-hosted runners required.
- **Traces**: A web interface to visualize each step of your workflow, drill down to detailed logs, understand how long operations took to run, and whether operations were cached.
