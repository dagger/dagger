...
The following example runs the tests for the [greetings-api](https://github.com/kpenfound/greetings-api) demo project using a Dagger Function. This project uses the Dagger Go SDK for CI.

Create a file called `workflow.yaml` with the following content:

```yaml file=./snippets/argo-workflow.yaml
```
