...
Depot provides managed, Dagger-powered GitHub Actions runners. These runners, which serve as drop-in replacements for GitHub's own runners, come with Dagger pre-installed and pre-configured to best practices, automatic persistent layer caching, and multi-architecture support. They make it faster and easier to run Dagger pipelines in GitHub repositories.
...
1. Depot receives this event, processes it, and runs one or more jobs in the workflow, each running in an isolated Depot GitHub Actions Runner.
1. Each Depot GitHub Actions Runner is automatically connected to an already running Dagger Engine (this exists outside of the Depot GitHub Actions Runner).
  1. The external Dagger Engine comes with persistent caching pre-configured.
1. The Depot GitHub Actions Runner has the Dagger CLI pre-installed.
  1. The Dagger CLI running inside the Depot GitHub Actions Runner gets automatically connected to Dagger Cloud.
  1. The Dagger CLI is also pre-configured to connect to the external Dagger Engine with persistent caching
