...
To be applied, the custom CAs should be placed in the `/usr/local/share/ca-certificates/` directory of the Dagger Engine container. No further commands are necessary; the CAs will be automatically installed on Dagger Engine startup if found in that directory.

## Configuration Applied to User Containers

As mentioned above, there is *best-effort* support for automatically installing the Dagger Engine's custom CAs in all Containers created by user pipelines (i.e. those created via a `withExec` API call).
...
Currently, the Dagger Engine supports automatically installing custom CAs in images with the following base distributions:
- Alpine
- Debian-based (e.g. `debian` and `ubuntu`)
- Redhat-based (e.g. `rhel`, `fedora`, `centos`, etc.)

If custom CAs are installed and the Dagger Engine detects one of these base images are being used, it will attempt to install the custom CAs into the container before executing it.
