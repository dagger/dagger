...
Secrets, such as credentials or access tokens, can be passed to Dagger Functions as arguments using the `Secret` core type.

Here is an example of a Dagger Function which accepts a GitHub personal access token as a secret, and uses the token to authorize a request to the GitHub API:
...
Here is an example of calling this Dagger Function, with the secret sourced from a host environment variable named `GITHUB_API_TOKEN`:

```shell
dagger call github-api --token=env:GITHUB_API_TOKEN
```

The result will be a JSON-formatted list of issues from Dagger's GitHub repository.
