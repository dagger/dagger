...
The following Dagger Function returns a just-in-time container. This can be exported to the host with `dagger call ... export ...`.
...
Export the container image returned by the Dagger Function as an OCI tarball to the `/home/admin/mycontainer.tgz` path on the host:
...
Create a Redis service with a cache volume to persist its data:
