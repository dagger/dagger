...
### Copy a local directory or remote repository to a container
...
- Copy the `/myapp` directory on the host to the `/src` directory in the container and return the modified container:

    ```shell
    dagger call copy-directory --d=./myapp/
    ```

- Copy the contents of the `dagger/dagger` GitHub repository to the `/src` directory in the container and return the modified container:

    ```shell
    dagger call copy-directory --d=github.com/dagger/dagger#main
    ```

- Copy the contents of the `dagger/dagger` GitHub repository to the `/src` directory in the container and list the contents of the directory:

    ```shell
    dagger call \
      copy-directory --d=https://github.com/dagger/dagger#main \
      directory --path=/src \
      entries
    ```

### Copy a local directory or remote repository to a container and modify its contents
...
- Copy the `/myapp` directory on the host to the `/src` directory in the container, add a file to it, and return the modified container:

    ```shell
    dagger call copy-and-modify-directory --d=./myapp/
    ```

- Copy the contents of the `dagger/dagger` GitHub repository to the `/src` directory in the container, add a file to it, and return the modified container:

    ```shell
    dagger call copy-and-modify-directory --d=github.com/dagger/dagger#main
    ```

- Copy the contents of the `dagger/dagger` GitHub repository to the `/src` directory in the container, add a file to it, and list the contents of the directory:

    ```shell
    dagger call \
      copy-and modify-directory --d=https://github.com/dagger/dagger#main \
      directory --path=/src \
      entries
    ```
...
The first Dagger Function below creates and returns an HTTP service. This service can be used:
- from a different Dagger Function, via a service binding using an alias like `www`; or
- from the host.
...
The following Dagger Function demonstrates explicitly starting and stopping a service. This example uses a Redis service.
