...
### Write a local or remote directory to a container
...
dagger call write-directory \
    --dir=github.com/dagger/dagger#main \

dagger call write-directory \
    --dir=https://github.com/dagger/dagger#main \
  directory \
    --path=/src \
  entries
```

### Write a local or remote directory to a container and modify it
...
:::note
Modifications made to a directory after it is written to a container filesystem do not appear on the host. Data flows only one way between Dagger operations, because they are connected in a DAG. To transfer modifications back to the host, you must explicitly export the directory back to the host filesystem.
:::
