...
:::note
Installing a local module using a relative path is only possible if your project is initialized as a Git repository (the project root has a `.git` directory).
:::
