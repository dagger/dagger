...
- If an absolute path is specified:
   - in a Git repository (defined by the presence of a `.git` sub-directory), resolution starts from the root of the Git repository.
   - in a non-repository location (defined by the absence of a `.git` sub-directory), resolution starts from the directory containing a `dagger.json` file.
- If a relative path is specified
  - resolution always starts from the directory containing a `dagger.json` file.

:::important
For security, it is not possible to retrieve files or directories outside the default context's root directory.
:::
