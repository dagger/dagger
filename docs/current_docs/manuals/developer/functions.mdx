...
The default context is set by adding a `defaultPath` pragma (Go), `DefaultPath` annotation (Python), or `defaultPath` decorator (TypeScript) on the corresponding Dagger Function argument.

The default context is resolved depending on various factors:
...
Here is an example of a Dagger Function with a default context for its directory argument:

<Tabs groupId="language">
<TabItem value="Go">
```go file=./snippets/functions/arguments-contextual-dir/go/main.go
```
</TabItem>
<TabItem value="Python">
```python file=./snippets/functions/arguments-contextual-dir/python/main.py
```
</TabItem>
<TabItem value="TypeScript">
```typescript file=./snippets/functions/arguments-contextual-dir/typescript/index.ts
```
</TabItem>
</Tabs>

Assuming the directory structure below, the following table describes how the argument is resolved:
...
| Location | Default context ('defaultPath`) | Dagger Function invocation | `source` resolves to |
| --- | --- | --- | --- |
| Git repository | `/` | `dagger call -m my-module read-dir` | `/` |
| Git repository | `/src` | `dagger call -m my-module read-dir` | `/src` |
| Git repository | `.` | `dagger call -m my-module read-dir` | `/my-module` |
| Git repository | `..` | `dagger call -m my-module read-dir` | `/` |
| Non-Git directory | `/` | `dagger call -m my-module read-dir` | `/` |
| Non-Git directory | `/src` | `dagger call -m my-module read-dir` | `/src` |
| Non-Git directory | `.` | `dagger call -m my-module read-dir` | `/my-module` |
| Non-Git directory | `..` | `dagger call -m my-module read-dir` | `/` |
...
/* TBC
The context is set through the `defaultPath` parameter metadata pointing to a location inside the module's context directory.

- If the path is absolute, its root is the path to the git repository containting that module (called `context directory`), falling into the location of the `dagger.json` if no `.git` directory is found.
- If the path is relative, its root is the path to `dagger.json` location (called `root directory`).

- `repoFiles` has an argument `repo` of type `Directory` and a `defaultPath` parameter metadata set to `/` and returns the list of files in the given directory.
- `moduleFiles` has an argument `module` of type `Directory` and a `defaultPath` parameter metadata set to `.` and returns the list of files in the given directory.
- `readme` has an argument `readmeFile` of type `File` and a `defaultPath` parameter metadata set to `/README.md` and returns the contents of the given file.

<Tabs groupId="language">

<TabItem value="Go">

For the example, we will use a repository with the following files:
...
To define a contextual argument, you can add the `+defaultPath` pragma to the function parameter and set it to the path to default to.

```go file=./snippets/functions/arguments-contextual/go/main.go
```
...
To define a contextual argument, you can annotate the function parameter with the `DefaultPath` annotation and set it to the path to default to.

```python file=./snippets/functions/arguments-contextual/python/main.py
```
...
To define a contextual argument, you can add the decorator `@argument` to the function parameter and set the `defaultPath` parameter metadata to the path to default to.

```typescript file=./snippets/functions/arguments-contextual/typescript/index.ts
```
