...
Function arguments can define a default contextual value if no value is supplied for them. This default value will be load prior to the function call which makes it more cache efficient.

:::note
Contextual values are only available for arguments of type `Directory` and `File` for now.

It's specially useful for static directory or file provided to the module. For example, a codebase source's code.
:::

The contextual value consist of a path to a directory or a file set through the `defaultPath` parameter metadata pointing to a location inside the module's context directory.
...
Here's an example of Dagger Functions with a contextual arguments:
...
It's also possible to provide an `ignore` parameter metadata to a contextual argument of type `Directory` to automatically ignore or include files in the directory.

:::note
The ignore directive follows the `.gitignore` syntax, prefix a path with `!` will actually include it instead of ignoring it.
:::
