...
```go file=./snippets/services/service-lifecycle-1/main.go
```
...
```go file=./snippets/services/service-lifecycle-2/main.go
```
...
It would be better to chain both commands together, which ensures that the service stays running for both.

```go file=./snippets/services/service-lifecycle-3/main.go
```
