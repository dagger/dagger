...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "MyModule":
		...
		case "GithubApi":
			...
			return (*MyModule).GithubApi(&parent, ctx, endpoint, token)
		default:
		...
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("MyModule").
					WithFunction(
						dag.Function("GithubApi",
							dag.TypeDef().WithKind(StringKind)).
							WithArg("endpoint", dag.TypeDef().WithKind(StringKind)).
							WithArg("token", dag.TypeDef().WithObject("Secret")))), nil
	default:
