...
Dagger doesn't support running the `poetry` CLI. It depends on standards like [PEP 517](./820256-module-structure.mdx#modules-as-python-libraries). That means that it doesn't know to use the `poetry.lock` file, but the `requirements.lock` file can be generated with [poetry export](https://python-poetry.org/docs/cli#export):
