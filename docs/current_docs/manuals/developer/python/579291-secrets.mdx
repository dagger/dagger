---
slug: /manuals/developer/python/579291/secrets
...
```python file=./snippets/secrets/main.py
```

When invoking the Dagger Function using the Dagger CLI, secrets can be sourced from host environment variables (`env:`, the default), the host filesystem (`file:`) or the result of host command execution (`cmd:`).
