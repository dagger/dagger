...
Dagger Modules run in a [container](../overview/942201-execution-environment.mdx) that's bootstrapped specifically by the Python SDK, with the necessary environment to run the module's code. It currently runs Python 3.11 by default but starting with [v0.11.0][v11] that can be changed.
...
This can be useful either to pin the version in case the default one gets an upgrade, or to choose another a newer version sooner.

As an alternative, setting [requires-python](https://packaging.python.org/en/latest/guides/writing-pyproject-toml/#python-requires) in `pyproject.toml` will also pin the version, but using a lower bound constraint (minimum version). For example, the following will use 3.11, even though there's a 3.12 available:
...
The default base image is [python:VERSION-slim](https://hub.docker.com/_/python/tags?name=3.11-slim) (based on the [Python version](#python-version)), but it can be overridden in `pyproject.toml` since [v0.11][v11]:
...
This can be useful to add a few requirements to the module's execution environment, like operating system packages such as `git` for example.
...
Each release of Dagger bundles the Python SDK inside the Engine container image. During the Dagger Module's code generation phase, that directory is unpacked and installed automatically, including dependencies. That's the copy that is exported to the host on `dagger init --sdk=python` or `dagger develop --sdk=python`. It's **not** the same as installing from [dagger-io](https://pypi.org/project/dagger-io/).
...
[uv](https://github.com/astral-sh/uv) is used internally to create the virtual environment, install dependencies and create the [lock file](./077133-python-dependencies.mdx#lock-file) (starting with [v0.11.0][v11]). It's possible to specify a newer version if needed, in `pyproject.toml`:
...
Rye will create an initial `src/main/__init__.py` but Dagger won't recognize any Dagger Functions in it. Either remove it and let Dagger create the default one, or adapt to own's needs before running:
