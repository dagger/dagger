Just-in-time files are produced by calling a function that returns the File type

Just-in-time files might be produced by a function that:
...
Here is an example of a linter function that returns the linting rules used by it as a file:
...
Here is another example of a file builder function that returns a ZIP archive of the contents of the `cmd/dagger` directory in the Dagger GitHub repository:
