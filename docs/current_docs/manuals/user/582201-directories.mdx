Just-in-time directories are produced by calling a function that returns the Directory type

Just-in-time directories might be produced by a function that:
...
Here is an example of a Go builder function that accepts a remote Git address (the address of Dagger's open-source repository) as a `Directory` argument, builds a Go binary from the source code in that repository, and returns the build directory containing the compiled binary.
