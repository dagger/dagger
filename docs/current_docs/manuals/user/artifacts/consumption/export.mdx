...
Just-in-time artifacts such as containers, directories and files expose an `Container.export()` API method, enabling them to be exported to the host filesystem from the Dagger Function that produced them. The destination path on the host is passed to the function using the `--path` argument.
