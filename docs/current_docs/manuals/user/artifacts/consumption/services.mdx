...
:::note
It is also possible to [pass host network services to a Dagger Function](../../host/host-services.mdx), by explicitly specifying them as arguments in the form `tcp://<host>:<port>` when executing `dagger call`.
:::
