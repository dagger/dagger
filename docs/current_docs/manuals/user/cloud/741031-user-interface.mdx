...
# User Interface
:::info
Dagger 0.11.0 adds support for visualizing Dagger Function calls in Dagger Cloud. With this update, Dagger Cloud will default to use the new **Traces** views for both `dagger call` and `dagger run` commands. Dagger v0.11.0 or later will automatically use the new **Traces** views. Older versions of Dagger will continue to use the legacy Runs experience.
:::

Dagger Cloud visualizes your Dagger pipeline runs invoked from your local development host and in CI. 
...
![View Traces](/img/current_docs/manuals/user/cloud/user-interface/all-traces.png)
...
The **Trace Details** page includes detailed status and duration metadata about the pipeline steps. The tree view shows Dagger pipelines and steps within those pipelines. If there are any errors in the run, the UI shows the errors at the top. Clicking any of the errors takes you directly to the step that failed, making it easy to debug your pipelines and collaborate with your teammates.  

![View Trace details](/img/current_docs/manuals/user/cloud/user-interface/trace-detail-error-list.png)

A Trace shows a detailed timeline for operations completed in a pipeline run. You see detailed logs and output of each step, making it easy for you to debug your pipelines and collaborate with your teammates. Dagger Cloud also applies unique IDs and icons for related operations, which are essential for debugging your pipeline when something does wrong.

![View Trace Timeline](/img/current_docs/manuals/user/cloud/user-interface/trace-detail-error-timeline.png)

## Runs
:::info
The Runs views only apply to pipeline runs done with Dagger versions older than v0.11.0. This view has been replaced by the new [**Traces**](#traces) view in Dagger Cloud.
:::

### All Runs page

The **All Runs** page lists runs you've kicked off from your local development host and in CI.
