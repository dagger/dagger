...
1. Jenkins performs a checkout to a detached HEAD state by default. In order to visualize Jenkins builds in Dagger Cloud, ensure that your pipeline is configured to check out to matching local branch. 
    1. For a Multibranch pipeline project this can be configured in the Jenkins UI by navigating to Configuration -> Branch Sources -> General -> Add -> **Check out to matching local branch**
    1. For a Pipeline project this can be configured in the Jenkins UI by navigating to Configure -> Pipeline ->  Pipeline script from SCM -> Definition -> SCM -> Additional Behaviors -> Add -> **Check out to specific local branch** -> Branch name -> `**`
    1. If the UI based options described above do not work for you, you can also add the following configuration directly to your Jenkinsfile. Be sure to replace `$org` and `$repo` with project specific values. 

      ```groovy title="Jenkinsfile" file=../../../integrations/snippets/Jenkinsfile_checkout
      ```
