...
List the current settings and their values with `dagger settings prettier`, then set one with `dagger settings prettier <key> <value>`. Settings are stored in `dagger.toml` under `[modules.prettier.settings]`.
