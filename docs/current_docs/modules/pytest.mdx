...
Install the module, then run `dagger check`. The check should run Pytest for the selected source directory.

If the project is not at the default source path, set the source directory in workspace settings. If the project needs system packages, a pinned Python version, or extra setup, set a custom container.
