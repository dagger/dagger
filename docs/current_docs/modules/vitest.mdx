...
List the current settings and their values with `dagger settings vitest`, then set one with `dagger settings vitest <key> <value>`. Settings are stored in `dagger.toml` under `[modules.vitest.settings]`.
