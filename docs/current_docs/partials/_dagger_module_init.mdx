...
<a id="ts-configure-sdk-origin"></a>

**Configure the SDK origin**
...
and it doesn't add extra dependencie to the module's `package.json` file.

The `tsconfig.json` is configured to import the bundled SDK from the `sdk` directory and correctly maps imports paths:
...
The bundled SDK ensure better performances and compatibility with all the runtime without conflicting with the module's own dependencies.
...
This will generate a new `sdk` directory with the SDK as a vendored library and will update the `tsconfig.json` with the new imports paths.
...
The migrate from the local vendored SDK dependency to the bundle SDK, you need to run the opposite commands:
