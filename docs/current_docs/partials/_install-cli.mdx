...
```shell
cd /usr/local
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.13.1 sh
...
```shell
cd /usr/local
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.13.1 sudo -E sh
```
...
```powershell
Invoke-WebRequest -UseBasicParsing -Uri https://dl.dagger.io/dagger/install.ps1 | Invoke-Expression;
Install-Dagger -DaggerVersion 0.13.1
```
