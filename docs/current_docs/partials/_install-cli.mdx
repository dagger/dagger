...
```shell
cd /usr/local
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.13.5 sh

./bin/dagger version
dagger v0.13.5 (registry.dagger.io/engine:v0.13.5) darwin/arm64
```
...
```shell
cd /usr/local
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.13.5 sudo -E sh
```
...
```powershell
Invoke-WebRequest -UseBasicParsing -Uri https://dl.dagger.io/dagger/install.ps1 | Invoke-Expression;
Install-Dagger -DaggerVersion 0.13.5
```
