...
```shell
cd /usr/local
curl -L https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.11.3 sh
...
```shell
cd /usr/local
curl -L https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.11.3 sudo sh
```
...
./bin/dagger version
dagger 0.11.3 (registry.dagger.io/engine) linux/amd64
```
