...
./bin/dagger version
dagger 0.11.7 (registry.dagger.io/engine) darwin/arm64
```
...
```shell
cd /usr/local
curl -L https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.11.7 sudo sh
```
...
./bin/dagger version
dagger 0.11.7 (registry.dagger.io/engine) linux/amd64
```
...
```powershell
Invoke-WebRequest -UseBasicParsing -Uri https://dl.dagger.io/dagger/install.ps1 | Invoke-Expression;
Install-Dagger -DaggerVersion 0.11.7
```
