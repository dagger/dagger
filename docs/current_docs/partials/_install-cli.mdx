...
```shell
cd /usr/local
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.13.4 sh

./bin/dagger version
dagger v0.13.4 (registry.dagger.io/engine:v0.13.4) darwin/arm64
```
...
```shell
cd /usr/local
curl -fsSL https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.13.4 sudo -E sh
```
...
:::note
This installs `dagger` in `$HOME/.local/bin` which is not generally in the user's $PATH
...
dagger v0.13.4 (registry.dagger.io/engine:v0.13.4) linux/amd64
```
...
```powershell
Invoke-WebRequest -UseBasicParsing -Uri https://dl.dagger.io/dagger/install.ps1 | Invoke-Expression;
Install-Dagger -DaggerVersion 0.13.4
```
