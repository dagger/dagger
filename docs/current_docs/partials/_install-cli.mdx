...
```shell
cd /usr/local
curl -L https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.11.8 sudo sh
```
...
./bin/dagger version
dagger v0.11.8 (registry.dagger.io/engine) linux/amd64
```
...
```powershell
Invoke-WebRequest -UseBasicParsing -Uri https://dl.dagger.io/dagger/install.ps1 | Invoke-Expression;
Install-Dagger -DaggerVersion 0.11.8
```
