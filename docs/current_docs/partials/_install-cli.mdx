...
./bin/dagger version
dagger 0.11.4 (registry.dagger.io/engine) darwin/arm64
```
...
```shell
cd /usr/local
curl -L https://dl.dagger.io/dagger/install.sh | DAGGER_VERSION=0.11.4 sudo sh
```
...
./bin/dagger version
dagger 0.11.4 (registry.dagger.io/engine) linux/amd64
```
