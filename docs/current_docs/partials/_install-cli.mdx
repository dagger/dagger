...
Install the latest release of the Dagger CLI following the steps below.
...
<TabItem value="Linux">
The quickest way of installing the latest release of `dagger` on Linux is to use [install.sh](https://github.com/dagger/dagger/blob/main/install.sh):
...
The quickest way of installing the latest release of `dagger` on Windows is to use [install.ps1](https://github.com/dagger/dagger/blob/main/install.ps1):
...
:::warning
Development versions should be considered unstable and may produce unexpected results.
:::
...
This will replace your current Dagger CLI version, so you may prefer to install it in an isolated environment so as to avoid conflicts.
