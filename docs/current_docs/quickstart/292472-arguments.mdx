...
This feature is most powerful when calling a function from your own code (see the section on [creating a module](./428201-custom-function.mdx)). But you can also pass a container argument from the command-line. To do so, add the corresponding flag, followed by the address of an OCI image. The CLI will dynamically pull the image, and pass the resulting container object as argument to the function.
