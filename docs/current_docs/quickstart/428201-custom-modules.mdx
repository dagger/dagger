...
So far, you've been calling Dagger Functions using the Dagger CLI. However, you can also call Dagger Functions through code, by creating your own Dagger Module and installing other Dagger Modules into it. This allows you to express more complex workflows and operations by reusing modules created by others and connecting them together using the programming language you're most comfortable with.
...
1. Dagger lets you add dependencies to your modules, so that you can build on modules created by others. Add the `golang` and `wolfi` modules as dependencies in your new Dagger Module using `dagger install`:

    ```shell
    dagger install github.com/kpenfound/dagger-modules/golang@v0.1.5
...
Call the new Dagger Function using the Dagger CLI:
