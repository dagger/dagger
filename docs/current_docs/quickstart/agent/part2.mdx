...
The `develop-issue` function will use a dependency from [Daggerverse](https://daggerverse.dev) called [github-issue](https://daggerverse.dev/mod/github.com/kpenfound/dag/github-issue@b316e472d3de5bb0e54fe3991be68dc85e33ef38). This module gives the functions needed to read Github issues and create pull requests.
...
Github Actions will need the ability to call out to an LLM. It will need the same environment variables you [configured](../../configuration/llm.mdx) locally setup as repository secrets.
...
Create the file `.github/workflows/develop.yml` with the following content:
...
When the label is added, the Github Actions workflow will get kicked off. You can see it running by clicking on the "Actions" tab or by opening Dagger Cloud.
