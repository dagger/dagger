...
Dagger works by expressing workflows as combinations of functions from the [Dagger API](../../api/index.mdx), which exposes a range of tools for working with containers, files, directories, network services, secrets. You [call the Dagger API](../../api/calling.mdx) from shell ([Dagger Shell](../../features/shell.mdx)) or code ([custom Dagger Functions](../../api/custom-functions.mdx) written in a programming language).

[Dagger Shell](../../features/shell.mdx) is an interactive client for the Dagger API, giving you typed objects, built-in documentation, and access to a cross-language ecosystem of reusable modules. You launch it by typing `dagger`. Here's an example of using it to build and return an Alpine container:
...
![Terminal](/img/current_docs/quickstart/basics/terminal.gif)
...
When you create a custom Dagger Function and tell Dagger about it (by installing the corresponding module), the Dagger API is [dynamically extended](../../api/internals.mdx#api-extension-with-dagger-functions) to include that new function. You can then call this function from Dagger Shell, or from your system shell using `dagger call`, in exactly the same way as you would call functions from the original Dagger API.

![Publish with a Dagger Function](/img/current_docs/quickstart/basics/publish-code.gif)
...
![Daggerverse](/img/current_docs/quickstart/basics/daggerverse.png)
...
![Modules](/img/current_docs/quickstart/basics/modules.gif)
...
- [Layers](../../api/cache-volumes.mdx#layer-caching): Build instructions and the results of some API calls.
- [Volumes](../../api/cache-volumes.mdx#volume-caching): Contents of Dagger filesystem volumes.
...
<TabItem value="Go">
```go
func (m *Basics) Env(ctx context.Context) (*dagger.Container, error) {
...
![Dagger Trace](/img/current_docs/features/trace.webp)
...
- [Build a CI pipeline](../../ci/quickstart/index.mdx)
- [Build an AI agent](../../agents/quickstart.mdx)
