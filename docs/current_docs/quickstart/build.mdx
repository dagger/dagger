...
func (m *HelloDagger) BuildWithExport(ctx context.Context, source *Directory) (bool, error) {
	_, err := dag.Container().
		From("node:21-slim").
...
```python
@function
async def build_with_export(self, source: dagger.Directory) -> bool:
    """Exports a directory with the production build to the host"""
    return (
        dag.container()
        .from_("node:21-slim")
...
/**
  ...
  */
@func()
async buildWithExport(source: Directory): Promise<boolean> {
  return await dag
    .container()
    .from('node:21-slim')
    .withDirectory('/src', source.withoutDirectory('dagger'))
