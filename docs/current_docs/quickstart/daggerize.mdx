...
:::note
This step is optional and requires signing up for Dagger Cloud's individual plan. The individual plan is free of charge for a single user. You will require a GitHub account for account setup and identity verification. If you prefer not to sign up for Dagger Cloud, you can skip this section.
:::
...
Once you confirm that the keys match, your device will be authenticated and you will be transferred to your auto-created Dagger Cloud organization page.

FIXME: Add image

FIXME: Update the signup steps here, and the result ie. the user is dropped into an autoconfigured org. Ideally don't point to any other doc for Cloud setup, and keep the steps brief to avoid adding friction to this page.
