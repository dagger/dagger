...
Bootstrap a new Dagger module in Go, Python, TypeScript or PHP by running `dagger init` in the application's root directory, using the `--source` flag to specify a directory for the module's source code.
...
```php file=./snippets/daggerize/php/HelloDagger.php
```
