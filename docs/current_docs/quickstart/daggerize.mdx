...
:::important
By default, the module name is automatically generated from the name of the directory in which `dagger init` runs. In this case, the default name of the cloned application directory is `hello-dagger`, so the module name is `HelloDagger`. If you cloned the application into a different directory, add the `--name=hello-dagger` flag to `dagger init` to correctly set the module name.
:::
