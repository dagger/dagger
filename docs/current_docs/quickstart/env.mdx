...
title: "Daggerize the build environment"
---
...
## Daggerize the build environment
...
Since the starting point of the DAG is the `build-env` Dagger Function, let's look at that first.
...
:::tip FUNCTION NAMES
When using `dagger call`, all names (functions, arguments, fields, etc.) are converted into a shell-friendly "kebab-case" style. This is why Dagger Functions named `BuildEnv` in Go, `build_env` in Python and `buildEnv` in TypeScript are invoked at the command line as `dagger call build-env ...`.
:::
