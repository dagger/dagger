...
This output means that the build succeeded, and a `Container` type representing the built container image was returned. This is a ["just-in-time container"](../features/programmable-builds.mdx) - a transient artifact produced as the result of a Dagger Function.
