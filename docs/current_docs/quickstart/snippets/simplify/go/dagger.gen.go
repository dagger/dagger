...
// The `NodeCommandsID` scalar type represents an identifier for an object of type NodeCommands.
type NodeCommandsID = dagger.NodeCommandsID
...
// NodeWithPnpmOpts contains options for Node.WithPnpm
type NodeWithPnpmOpts = dagger.NodeWithPnpmOpts
...
type NodeCommands = dagger.NodeCommands
...
// NodeOpts contains options for Client.Node
type NodeOpts = dagger.NodeOpts
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "HelloDagger":
		...
		case "Ci":
			...
			return (*HelloDagger).Ci(&parent, ctx, source)
		case "Package":
			var parent HelloDagger
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			var build *Directory
			if inputArgs["build"] != nil {
				err = json.Unmarshal([]byte(inputArgs["build"]), &build)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg build", err))
				}
			}
			return (*HelloDagger).Package(&parent, build), nil
		case "Build":
			var parent HelloDagger
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*HelloDagger).Build(&parent, source), nil
		case "Test":
			var parent HelloDagger
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*HelloDagger).Test(&parent, ctx, source)
		default:
				...
				dag.TypeDef().WithObject("HelloDagger").
					WithFunction(
						dag.Function("Ci",
							...
							WithDescription("Tests, builds, packages and publishes the application").
							WithArg("source", dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("Package",
							dag.TypeDef().WithObject("Container")).
							WithDescription("Returns a container with the production build").
							WithArg("build", dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("Build",
							dag.TypeDef().WithObject("Directory")).
							WithDescription("Returns a directory with the production build").
							WithArg("source", dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("Test",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Returns the result of running unit tests").
