...
In some situations, you may need to provide specific values to your test modules, such as when authenticating against an external service. In these cases, you can rely on [module constructors](../../extending/constructors.mdx) to inject any required inputs.
...
1. Module functions should have [descriptions](../../extending/documentation.mdx).
1. Modules versions should be tagged with [semver](../../extending/daggerverse.mdx#semantic-versioning).
1. Modules should have hand-crafted [examples](../../extending/daggerverse.mdx#examples).
