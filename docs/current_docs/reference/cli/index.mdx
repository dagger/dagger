...
```
      --checkpoint-exclude stringArray             Exclude paths from checkpoints (repeatable)
      --checkpoint-include stringArray             Approve nonignored untracked paths for checkpoints (repeatable)
      --checkpoint-max-untracked-file-bytes int    Maximum bytes per untracked checkpoint file (default 16 MiB)
      --checkpoint-max-untracked-files int         Maximum untracked checkpoint files (default 4096)
      --checkpoint-max-untracked-total-bytes int   Maximum total untracked checkpoint bytes (default 64 MiB)
