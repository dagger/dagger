...
List artifacts by type, or values for a collection dimension. Use -a to list all artifacts.
...
```
  ...
  -a, --all                           List all artifacts, optionally filtered by address
      --dimension-key DIMENSION=KEY   Select items with DIMENSION=KEY (repeat to select more)
