...
* [dagger list](#dagger-list)	 - List artifacts or collection items
...
* [dagger workspace](#dagger-workspace)	 - Inspect workspace files and config
...
```
dagger agent [FILTERS] [OPTIONS]
```

### Options

```
      --absolute                  Show absolute links (alias: --abs)
      --agent string              With --trace, focus this restored agent (runtime handle or name) instead of the top-level one
  -a, --all                       Expand collections and list each item
  -f, --format FORMAT             Output FORMAT: table, link, or cli (requires --list) (default "table")
  -l, --list                      List available agents
      --module NAME               Select artifacts from module NAME
...
```
dagger check [--failfast] [FILTERS] [OPTIONS]
```
...
```
  ...
  -f, --format FORMAT        Output FORMAT: table, link, or cli (requires --list) (default "table")
      --generated            Include staleness checks for 'dagger generate' (default true)
  -l, --list                 List available checks
  -m, --load-module string   Use a one-off module (local path or git ref)
      --module NAME          Select artifacts from module NAME
      --skip link            Exclude checks selected by this link
```
...
* [dagger cloud traces](#dagger-cloud-traces)	 - View Dagger Cloud traces
* [dagger cloud usage](#dagger-cloud-usage)	 - Show Dagger Cloud usage
...
* [dagger cloud checks rerun](#dagger-cloud-checks-rerun)	 - Re-run checks on Dagger Cloud for the current commit
...
```
dagger cloud checks rerun [--check NAME ...] [--failed | --all]
```
...
* [dagger cloud](#dagger-cloud)	 - Manage Dagger Cloud
* [dagger cloud traces list](#dagger-cloud-traces-list)	 - List Dagger Cloud traces
* [dagger cloud traces status](#dagger-cloud-traces-status)	 - Show if this installation sends traces to Dagger Cloud
* [dagger cloud traces view](#dagger-cloud-traces-view)	 - View a Dagger Cloud trace

## dagger cloud traces list

List Dagger Cloud traces

### Synopsis

List the traces of a Dagger Cloud org, local and CI, newest first.
...
```
dagger cloud traces list --status failed --branch main --since 7d
...
```
      --author string           Only traces whose commit author or committer matches this text
      --branch string           Only traces of this branch
      --ci                      Only traces of CI runs
      --command string          Only traces whose command matches this glob, e.g. 'dagger check*'
      --commit string           Only traces of this commit (full or short SHA)
      --json                    Print JSON output
  -L, --limit int               Maximum number of traces to list (at most 1000) (default 20)
      --local                   Only traces of local runs
      --max-duration duration   Only traces that ran for at most this time
      --min-duration duration   Only traces that ran for at least this time, e.g. 5m
      --mine                    Only traces that you sent
      --pr string               Only traces of this pull request number
      --provider string         Only traces of this CI provider, e.g. github
      --repo strings            Only traces of this repository, e.g. github.com/org/repo (repeatable)
      --since string            Only traces that started after this time: a duration (30m, 2d, 1w) or a date (2006-01-02, RFC 3339)
      --sort string             Sort order: start (newest first) or duration (longest first) (default "start")
      --status string           Only traces with this status: passed, failed or running
      --tag string              Only traces of this git tag
      --token string            Only traces that this token sent (token ID or name)
      --until string            Only traces that started before this time: a duration or a date, as for --since
      --user string             Only traces that this user sent (user ID or email)
```
...
## dagger cloud traces status

Show if this installation sends traces to Dagger Cloud

### Synopsis

Show if the dagger CLI on this machine sends traces to Dagger Cloud, with
...
View a Dagger Cloud trace

### Synopsis

View a Dagger Cloud trace: the overall result, the commands that caused a
...
```
dagger cloud traces view [trace]
```

### Examples

```
dagger cloud traces view 2f123ba77bf7bd2d4db2f70ed20613e8
...
```
      --check string       Show only this check, by name
      --descendants        With --log and --span, include the logs of the span's descendants
      --last               View your most recent trace
      --log                Show the full logs instead of the tree
  -o, --output string      With --log, write the logs to this file
      --span string        Show only this span, by span ID
      --test string        Show only this test, by name
      --timeout duration   With --log, the maximum time to write logs, when the command does not open a pager (default 2m0s)
```
...
Show Dagger Cloud usage

### Synopsis

Show Dagger Cloud usage for a UTC calendar month.
...
```
dagger cloud usage [options] [org] [flags]
```
...
```
      --json           Print JSON output
      --month string   UTC month to report, as YYYY-MM (defaults to the current month)
```
...
```
dagger generate [FILTERS] [OPTIONS]
```
...
How to select artifacts with DAG links

### Synopsis

A DAG link selects artifacts. A relative link uses the current workspace.
...
List artifacts or collection items

### Synopsis

List artifacts by type or collection. Use -a to expand collections.

```
dagger list [TYPE | COLLECTION]
```
...
```
      --absolute        Show absolute links (alias: --abs)
  -a, --all             Expand collections and list each item
  -f, --format FORMAT   Output FORMAT: table, link, or cli (default "table")
      --module NAME     Select artifacts from module NAME
  -t, --type TYPE       Select artifacts of this TYPE (repeat to select more)
```
...
* [dagger module settings](#dagger-module-settings)	 - Get, set, or unset module settings (use --env for an env overlay)
...
```
  ...
  -u, --unset    Remove the setting from workspace config
      --wide     List settings with full values and descriptions, wrapped to the terminal width instead of truncated
```
...
```
      --absolute             Show absolute links (alias: --abs)
  -a, --all                  Expand collections and list each item
  -c, --command command      Run a shell command and return its exit code
      --copy [PATH=]SOURCE   Copy a directory into the container: [PATH=]SOURCE (repeatable)
  -f, --format FORMAT        Output FORMAT: table, link, or cli (requires --list) (default "table")
      --init command         Run a shell command before opening the shell (repeatable)
...
Inspect workspace files and config
