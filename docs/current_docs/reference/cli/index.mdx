...
```
      --agent string                               With --trace, focus this restored agent (instance ID or name) instead of the top-level one
      --checkpoint-exclude stringArray             Exclude a workspace path pattern from the portable checkpoint (repeatable)
      --checkpoint-include stringArray             Approve a workspace path pattern for the portable checkpoint (repeatable)
      --checkpoint-max-untracked-file-bytes int    Maximum bytes for one untracked checkpoint file (default 16 MiB)
      --checkpoint-max-untracked-files int         Maximum untracked checkpoint files (default 4096)
      --checkpoint-max-untracked-total-bytes int   Maximum aggregate untracked checkpoint bytes (default 64 MiB)
