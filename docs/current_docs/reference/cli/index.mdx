...
* [dagger api schema](#dagger-api-schema)	 - Print the API schema currently served by the engine
...
Print the API schema currently served by the engine

### Synopsis

Print the API schema the engine is currently serving, in GraphQL schema
...
```
dagger api schema [options] [module...]
```
...
```
      --allow-llm strings    List of URLs of remote modules allowed to access LLM APIs, or 'all' to bypass restrictions for the entire session
      --core                 Print only the core API, excluding schema extensions from modules
      --eager-runtime        load module runtime eagerly
      --json                 Print the raw introspection JSON instead of SDL
  -m, --load-module string   Use a one-off module (local path or git ref)
  -M, --no-load-module       Don't load any module for this command
```
