...
```
      ...
      --generation string       With --trace and --source-session, select the exact archive generation
  -l, --list                    List available agents
      --list-archives           List retained engine archives without restoring; --trace filters the list
      --partial                 With --trace, restore the agents the trace carries enough to restore instead of failing on the first one it does not
      --source-session string   With --trace, select the source session in an engine archive or Cloud trace
      --trace string            Restore agents from a retained engine archive, falling back to their Dagger Cloud trace
```
...
```
  -c, --command string     Run a command in the shell, and exit with its exit code
      --copy stringArray   Copy a directory into the container: [PATH=]SOURCE, where SOURCE is a local path, Git URL, or other address (repeatable)
      --init stringArray   Run a command in the shell before it opens. Only its changes to files are kept (repeatable)
