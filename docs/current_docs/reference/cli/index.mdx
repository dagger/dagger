...
* [dagger blueprint add](#dagger-blueprint-add)	 - Add a blueprint to the current module

## dagger blueprint add

Add a blueprint to the current module

### Synopsis

Add another module as a blueprint to the current module.

```
dagger blueprint add [options] <module> [flags]
```

### Examples

```
dagger blueprint add github.com/example/blueprint
```
...
```
  ...
  -m, --mod string          Module reference to load, either a local path or a remote git repo (defaults to current directory)
  -n, --name string         Name to use for the blueprint in the module. Defaults to the name of the blueprint being added.
```
