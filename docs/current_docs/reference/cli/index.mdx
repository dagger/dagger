...
```
      --agent string            With -r, focus this restored agent (runtime handle or name) instead of the top-level one
      --generation string       With -r and --source-session, select the exact archive generation
  -l, --list                    List available agents
  -r, --resume trace[=list]     Restore agents from a past session's trace: a retained engine archive, falling back to Dagger Cloud. With no trace ID, list retained engine archives
      --source-session string   With -r, select the source session in an engine archive or Cloud trace
```
