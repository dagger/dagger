...
```
      --absolute                  List absolute artifact addresses, including the workspace and revision (alias: --abs)
...
```
  ...
  -t, --type type                   Keep artifacts of this type (CLI or GraphQL case; repeat for alternatives)
```
...
```
      ...
      --generated            Check that generated files are up to date (default true)
  -l, --list                 List available checks
  -m, --load-module string   Use a one-off module (local path or git ref)
      --skip pattern         Skip checks matching pattern (repeat for multiple patterns)
```
