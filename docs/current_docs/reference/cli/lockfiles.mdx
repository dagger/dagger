...
Lock mode is set for a run, typically with `--lock`. If omitted, it defaults to `disabled`.
...
Three aliases are also accepted: `update` (= `live`), `auto` (= `pinned`), and `strict` (= `frozen`).
