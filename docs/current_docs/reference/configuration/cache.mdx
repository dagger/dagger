...
```shell
dagger -m core function call engine local-cache entry-set entries
```

- Show high level summaries of cache usage:

```shell
dagger -m core function call engine local-cache entry-set
```
...
```shell
dagger -m core function call engine local-cache prune
```
...
```shell
dagger -m core function call engine local-cache prune --use-default-policy
```
