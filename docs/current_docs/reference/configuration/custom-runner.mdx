...
- Run `dagger api call` or execute SDK code directly with that environment variable set.
...
Some engine settings are not in `engine.toml` — they are environment variables
you pass when you start the runner container (for example
[proxy settings](./proxy.mdx)). That is runner provisioning, not something the
CLI picks up from your host shell: put `-e` on the container, then point the
CLI at it with `_EXPERIMENTAL_DAGGER_RUNNER_HOST`.
...
```shell
VERSION=$(dagger version | awk '/^version:/{print $2}')
...
dagger -m github.com/samalba/dagger-modules/nvidia-gpu api call deploy-dagger-on-fly --token env://FLYIO_TOKEN --org env://FLYIO_ORG
```
...
dagger -m github.com/samalba/dagger-modules/nvidia-gpu api call destroy-dagger-on-fly --token env://FLYIO_TOKEN --app dagger-v0-14-0-smart-gerhard-2024-12-06
```
...
```shell
dagger -m github.com/samalba/dagger-modules/nvidia-gpu api call has-gpu
```
...
```shell
dagger -m github.com/samalba/dagger-modules/nvidia-gpu api call ollama-run --prompt "What color is the sky?"
```
