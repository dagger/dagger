...
1. `image://<container image reference>` - Start the runner in Docker using the provided container image, pulling it locally if needed
    ...
    - Incus is supported via `image+incus://<container image reference>` when the `incus` client and daemon socket are available on the host. See the [Incus guide](../container-runtimes/incus.mdx) for installation and validation steps.
