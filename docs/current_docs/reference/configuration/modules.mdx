...
Modules can be configured by editing their `dagger.json` file. The configuration in there contains all module metadata - from the name of the module and the SDK it uses, to the dependencies it requires. An initial configuration is automatically generated when using `dagger module init` or `dagger develop` for the first time, and is kept up-to-date with `dagger config`/`dagger module install`/etc.
...
dagger module init --sdk=typescript --name=example --source=.
```
