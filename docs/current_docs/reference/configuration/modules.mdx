...
Modules can be configured by editing their `dagger-module.toml` file. It contains module metadata: the module name, runtime, source directory, file filters, and module code dependencies. Workspace commands such as `dagger install`, `dagger workspace config`, and `dagger settings` update `dagger.toml`; they do not update a module's `dagger-module.toml`.

For more information, refer to the [schema for the Dagger module configuration file](https://docs.dagger.io/reference/dagger-module.schema.json).

## File and directory filters

The Dagger module configuration file `dagger-module.toml` supports an `include` field. It tells the Dagger Engine of additional files to include or exclude when loading the module itself. By default, only `dagger-module.toml` and all files under the module's source directory are included.

Notably, these filters only apply to loading the Dagger module. They do not apply to any directories passed as arguments to the Dagger module's functions ([pre- and post-call filtering](../../api/reference/directory.mdx#filters) is separately available for directory arguments).
...
When a runtime is not explicitly defined within the `package.json` file, Dagger automatically identifies the appropriate runtime by examining the files in the module source directory.

- If a `bun.lock` or `bun.lockb` file is present, Dagger chooses `bun` as the runtime (Bun is checked first because it can also emit a `yarn.lock`). Alternatively, if a `deno.json`, or `deno.lock`, file is present, Dagger will choose `deno` as the runtime.
- Otherwise, if Dagger finds any of the following lock files: `package-lock.json`, `yarn.lock`, or `pnpm-lock.yaml`, it automatically selects `node` as the runtime.
...
If no version is specified, `npm@11.8.0` is used by default.
...
When a package manager is not explicitly defined within the `package.json` file, Dagger automatically identifies the appropriate package manager by examining the module source directory's lock files.

- If Dagger finds any of the following lock files : `package-lock.json`, `yarn.lock`, or `pnpm-lock.yaml`, it automatically selects the corresponding package manager with a predefined default version: `npm@11.8.0`, `yarn@1.22.22` or `pnpm@8.15.4`.
- If a `bun.lock` or `bun.lockb` file is present (checked before the node lock files, since Bun can also emit a `yarn.lock`), Dagger will choose `bun` as the runtime and package manager with a predefined default version: `bun@1.3.0`.
...
# Verify the module still loads with the new dependency layout
dagger -m . api functions
```

This configuration uses the SDK as a vendored library and expects the `tsconfig.json` import paths below.
...
# Verify the module still loads with the bundled SDK layout
dagger -m . api functions
```
