...
Modules are configured by editing their `dagger.json` file. This file contains module metadata: the module name, SDK, source directory, file filters, and module code dependencies. Workspace commands such as `dagger install`, `dagger config`, and `dagger settings` update `.dagger/config.toml`; they do not update a module's `dagger.json`.
