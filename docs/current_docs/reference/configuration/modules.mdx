...
Modules can be configured by editing their `dagger.json` file. The configuration in there contains module metadata, including the module name, SDK, source path, and module dependencies.
...
cat > dagger.json <<'EOF'
{"name":"example","sdk":{"source":"typescript"},"source":"."}
EOF
```
...
# Regenerate the module's SDK files
dagger functions -m .
```
