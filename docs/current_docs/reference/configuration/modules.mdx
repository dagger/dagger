...
Modules can be configured by editing their `dagger-module.toml` file. It contains module metadata: the module name, runtime, source directory, file filters, and module code dependencies. Workspace commands such as `dagger install`, `dagger config`, and `dagger settings` update `dagger.toml`; they do not update a module's `dagger-module.toml`.
