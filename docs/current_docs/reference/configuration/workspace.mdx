...
Resolved lookups — module sources, container image references, and Git references — are recorded in `.dagger/lock` alongside the config. Refresh it with `dagger workspace update` or `dagger lock update`. See [Lockfiles](/reference/cli/lockfiles) for lock modes and policies.
