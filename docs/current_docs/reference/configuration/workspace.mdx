...
`dagger install` creates `dagger.toml` on first install if it doesn't already exist. To convert a legacy `dagger.json` project, run `dagger setup` — its workspace-migration step handles the conversion. See [Workspace Setup](/adopting/workspace-setup) to get started.
