...
- a path — `common/base.toml` relative to the including config's directory, or `/common/base.toml` from the workspace root, the same rule every other path a workspace resolves follows. A source naming a directory reaches the `dagger.toml` inside it.
...
**Pinning.** A Git include's ref is resolved and recorded in `dagger.lock` like any other Git reference: later runs reuse the recorded pin, and `dagger update` refreshes it. See [Lockfile](#lockfile). A path include is read from the workspace as it is on disk, so there is nothing to pin.
