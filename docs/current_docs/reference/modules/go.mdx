...
- `lint`, `test`, and `generate` (each defaults to `["**"]`) are module-root
  selector arrays for `lint-all`, `test-all`, and `generate-all`.
...
[Daggerize a Go Project](../../guides/go/index.mdx) walks through it.
