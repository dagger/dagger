---
title: "Deno"
slug: /reference/modules/js/deno
...
and exposes workspace-level functions that run across all of them. That makes
it a good fit for monorepos and repos where every project should share one Deno
version and the same CI checks.
...
```bash
dagger module install dagger.io/js/deno
```
...
dagger check dag://deno/test-all         # deno test across every project
dagger check dag://deno/lint-all         # deno lint across every project
dagger check dag://deno/type-check-all   # deno check across every project
dagger check dag://deno/format-check-all # deno fmt --check across every project
```
...
List the current settings with `dagger module settings deno`, then change one with
`dagger module settings deno <key> <value>`. They live in `dagger.toml` under
`[modules.deno.settings]`:

- `version` (default `2.9.3`) is the Deno version used to build the check
  containers. Pin this so every project is tested, linted, and formatted against
  the same Deno release.
- `base` is the base container image Deno runs in (for example
...
dagger module settings deno version 2.9.3
```
...
[source code](https://github.com/dagger/deno)
