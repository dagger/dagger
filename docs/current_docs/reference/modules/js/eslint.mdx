---
title: "ESLint"
slug: /reference/modules/js/eslint
...
dagger check dag://eslint/lint # run ESLint against the workspace source
```
...
List the current settings and their values with `dagger module settings eslint`, then set one with `dagger module settings eslint <key> <value>`. Settings are stored in `dagger.toml` under `[modules.eslint.settings]`.

- `packageManager` (default `npm`) is the package manager used to install dependencies before linting. Set it to `yarn` or `pnpm` to match the project, so the same lockfile and dependency versions are used.
- `baseImageAddress` (default `node:25-alpine`) is the Node base image ESLint runs in. Pin it to the project's Node version, for example `node:22-alpine`.
...
[source code](https://github.com/dagger/eslint)
