---
title: "Helm"
slug: /reference/modules/kubernetes/helm
...
```bash
dagger module install dagger.io/kubernetes/helm
```
...
dagger check dag://helm/lint             # lint every discovered chart
dagger check dag://helm/assert-template  # render discovered values files and fail on errors
```
...
List the current settings and their values with `dagger module settings helm`, then set one with `dagger module settings helm <key> <value>`. Settings are stored in `dagger.toml` under `[modules.helm.settings]`:

- `version` (default `3.18.4`) is the Helm version used for `lint` and `template`. Pin it so local runs and CI use the same Helm release. The module resolves it as the Wolfi `helm~<version>` package, which now tracks the 4.x line, so 3.x versions no longer resolve.
- `valuesGlob` (default `ci/*-values.yaml`) is a glob, relative to each chart root, that selects the values files to check. Every matching file becomes a separate scenario in both `lint` and `assert-template`. The default follows Helm chart-testing's CI convention, so files like `ci/prod-values.yaml` are picked up automatically.

```bash
dagger module settings helm version 4.0.1
dagger module settings helm valuesGlob "ci/*-values.yaml"
```
...
[source code](https://github.com/dagger/helm)
