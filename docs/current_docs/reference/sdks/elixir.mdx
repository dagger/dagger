...
dagger module init elixir-sdk --name my-module
```

:::note
Unlike Go, Python, or TypeScript, `elixir` is not a name in the CLI's built-in SDK registry, so `dagger module install elixir` does not resolve. Pass the full reference instead. Dagger installs a directly referenced SDK under the basename of its ref, here `elixir-sdk`, and that is the name you pass to `dagger module init`. If you want a shorter name, pick it at install time with `dagger module install --name elixir github.com/dagger/elixir-sdk`, then run `dagger module init elixir --name my-module`.
:::
...
Like every Dagger tool that modifies your workspace, `dagger module init` returns a [changeset](../../using/generating.mdx), a structured diff of the files it wants to create. Dagger shows it to you for review before writing anything to disk.
...
```shell
dagger module init elixir-sdk --name my-module --path ci     # ./ci
dagger module init elixir-sdk --name my-module --path /ci    # <workspace root>/ci
```
...
```shell
dagger module init elixir-sdk --name my-module --template empty
```
...
Elixir has no built-in engine runtime, so `runtime.source` points at the Elixir SDK's own runtime module. That runtime compiles your module in an `elixir:*-alpine` container and runs `mix dagger.entrypoint.invoke <ModuleName>`. The workspace's `dagger.toml` records the SDK under `sdks.elixir-sdk` and records the module path as one of its scopes. Elixir binding generation uses that scope to discover the module. Nobody writes the files in `dagger_sdk/` by hand; see [Regenerate bindings](#regenerate-bindings-and-generated-files).
...
**`dagger init` / `dagger develop` not found.** Install the Elixir SDK with `dagger module install github.com/dagger/elixir-sdk`, scaffold with `dagger module init elixir-sdk --name <name>`, and regenerate with `dagger generate`.

**`dagger module install elixir` reports the SDK is not in the registry.** Install Elixir by full reference: `dagger module install github.com/dagger/elixir-sdk`. Add `--name elixir` if you want `dagger module init elixir` to work.

**`dagger module install` reports "no current workspace".** Add `--here` to create `dagger.toml` in the current directory.
