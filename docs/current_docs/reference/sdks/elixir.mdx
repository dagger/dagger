...
The Elixir SDK is itself a Dagger module, `github.com/dagger/elixir-sdk`. Install it into your workspace once, then create and maintain Elixir modules with Dagger's module commands:
...
:::tip
If the repository does not have a `dagger.toml` yet, use `dagger module install --here github.com/dagger/elixir-sdk` to create it in the current directory.
:::
...
**`dagger init` / `dagger develop` not found.** Install the Elixir SDK with `dagger module install github.com/dagger/elixir-sdk`, scaffold with `dagger module init elixir --name <name>`, and regenerate with `dagger generate`.
