...
The Go SDK is itself a Dagger module, `github.com/dagger/go-sdk`. Install it into your workspace once, then scaffold, generate, and maintain Go modules with Dagger's module commands:

```shell
# Install the Go SDK into your workspace (once)
dagger module install github.com/dagger/go-sdk
...
:::tip
If the repository does not have a `dagger.toml` yet, use `dagger module install --here github.com/dagger/go-sdk` to create it in the current directory.
:::
...
**`dagger init` / `dagger develop` not found.** Install the Go SDK with `dagger module install github.com/dagger/go-sdk`, scaffold with `dagger module init go --name <name>`, and regenerate with `dagger generate`.
