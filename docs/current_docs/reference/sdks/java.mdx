...
Like every Dagger tool that modifies your workspace, `dagger module init` returns a [changeset](../../using/generating.mdx), a structured diff of the files to create. Dagger shows it to you for review before writing anything to disk.

:::tip
If the repository does not have a `dagger.toml` yet, use `dagger module install --here java` to create it in the current directory.
:::
...
**`dagger init` / `dagger develop` not found.** Install the Java SDK with `dagger module install java`, scaffold with `dagger module init java --name <name>`, and regenerate with `dagger generate`.

**`dagger module install java` reports "no current workspace".** Add `--here` to create `dagger.toml` in the current directory: `dagger module install --here java`.
