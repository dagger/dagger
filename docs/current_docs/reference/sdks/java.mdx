...
:::note
Editing a function's body needs no regeneration. Changing the *shape* of your module does, and not only when dependencies change: a new `@Function`, a renamed argument, a new `@Object`. If you forget, the module still builds but the new function is not registered. `dagger check --generated=true` catches this drift.
:::
...
Before publishing, run `dagger check --generated=true` to confirm that the committed files are up to date.
