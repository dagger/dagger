...
The Java SDK is itself a Dagger module, `github.com/dagger/java-sdk`. Install it into your workspace once, then use Dagger's module commands to scaffold, generate, and maintain Java modules:
...
:::tip
If the repository does not have a `dagger.toml` yet, use `dagger module install --here github.com/dagger/java-sdk` to create it in the current directory.
:::
...
**`dagger init` / `dagger develop` not found.** Install the Java SDK with `dagger module install github.com/dagger/java-sdk`, scaffold with `dagger module init java --name <name>`, and regenerate with `dagger generate`.
