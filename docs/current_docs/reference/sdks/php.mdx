...
:::note
The PHP SDK module still uses the previous beta SDK interface. It needs an update before the current `dagger module init` and module client commands can use it. The module runtime is unchanged.
:::
...
The PHP SDK is itself a Dagger module, `dagger.io/sdk/php`. Install it into your workspace once, then use Dagger's module commands to scaffold, generate, and maintain PHP modules:

```shell
# Install the PHP SDK into your workspace (once)
dagger module install dagger.io/sdk/php
...
Install the PHP SDK into your workspace, then create a new module. The SDK argument is required. This example also sets the optional module name:
...
`dagger module init` returns a [changeset](../../using/generating.mdx), a structured diff of the files it wants to create. Dagger shows you that diff to review before it writes anything to disk.

:::tip
If the repository does not have a `dagger.toml` yet, use `dagger module install --here dagger.io/sdk/php` to create it in the current directory.
:::
...
```shell
dagger module init php --name my-module --path ci     # ./ci
dagger module init php --name my-module --path /ci    # <workspace root>/ci
```

Dagger registers a module at a custom path as authored by the SDK but does not install it as a callable workspace module. Run `dagger module install ./ci` to install it too.
...
:::note
`dagger module init` writes `dagger-module.toml`, `composer.json`, `entrypoint.php`, and `src/MyModule.php`, updates the workspace config, and runs the PHP SDK's generator for the new module. That is why `composer.lock`, `.gitattributes`, and `.gitignore` land in the same changeset.
:::
...
`runtime.source = "php"` tells the engine to use the PHP runtime. The workspace's `dagger.toml` records the SDK under `sdks.php` and records the module path as one of its scopes. That scope is how the PHP SDK's generator finds workspace modules.
...
Model a closed set of values as a PHP enum marked with `#[DaggerObject]`. Dagger turns it into an enum and validates inputs. `#[Doc]` works on the enum itself and on each case:

```php title="src/Severity.php"
<?php
...
Type a parameter with the enum and Dagger hands you the matching case; return one and the caller sees its API identifier:

```php
#[DaggerFunction]
public function scan(string $ref, Severity $severity): string
{
    return dag()
        ->container()
        ->from('aquasec/trivy:0.50.4')
        ->withExec(['trivy', 'image', "--severity={$severity->value}", $ref])
        ->stdout();
}
```

The `#[DaggerObject]` attribute is required on the enum. Without it the enum is never discovered, and the module fails to load with an error like `find mod type for function "scan" arg "severity" type: "Severity"`, which makes every function uncallable, not just the one referencing the enum.

The API identifier for each choice comes from the PHP case name, upper-snake-cased (`Critical` becomes `CRITICAL`, `NotFound` becomes `NOT_FOUND`), never from the backing value. The backing value stays yours to use inside PHP, through `$severity->value`. Keeping the two aligned, as above, avoids surprises.

Matching is case-insensitive, so `--severity=critical` and `--severity=CRITICAL` both work. The backing value is not accepted: only the API identifier is.
...
# invalid argument "FOO" for "--severity" flag: value should be one of CRITICAL,HIGH,LOW,MEDIUM,UNKNOWN
```

Any PHP enum works. The backing type is your business: the API identifier always comes from the case name, so a pure enum (`enum Severity { case Low; }`) and an int-backed one (`enum Severity: int`) register exactly like the string-backed one above. Only an enum with no cases at all is rejected.

The naming rule above applies to the enums your module declares. The generated client also includes enums that already exist in the Dagger API (for example `Dagger\NetworkProtocol`) as PHP enums, with case names taken straight from the API; pass them to core API calls and use them in your own signatures as usual.
...
|---|---|---|
| `#[DaggerObject]` | class or enum | expose the class as a Dagger object, or the enum as a Dagger enum |
| `#[DaggerFunction]` | public method or property | expose the method as a function, or the property as a field |
| `#[Doc('...')]` | class, enum, enum case, method, or parameter | API documentation |
...
:::tip
Do not publish Dagger modules to Packagist. Consumers install modules through Dagger (`dagger module install`), not Composer. Dagger does not register a module pulled in as a Composer package, so you cannot call its functions.
:::
...
**`dagger init` / `dagger develop` not found.** Install the PHP SDK with `dagger module install dagger.io/sdk/php`, scaffold with `dagger module init php --name <name>`, and regenerate with `dagger generate`.
