...
The PHP SDK is itself a Dagger module, `github.com/dagger/php-sdk`. Install it into your workspace once, then use Dagger's module commands to scaffold, generate, and maintain PHP modules:
...
:::tip
If the repository does not have a `dagger.toml` yet, use `dagger module install --here github.com/dagger/php-sdk` to create it in the current directory.
:::
...
**`dagger init` / `dagger develop` not found.** Install the PHP SDK with `dagger module install github.com/dagger/php-sdk`, scaffold with `dagger module init php --name <name>`, and regenerate with `dagger generate`.
