...
A useful, reusable module provides at least one of the three first-class function types: a check, a generator, or a service. That gives the platform verbs (`dagger check`, `dagger generate`, `dagger start`) something to run. See the [SDKs overview](./index.mdx) for the full treatment.
