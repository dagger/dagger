...
|---|---|
| `init` | Create a new Python SDK module. Returns a [changeset](../../using/generating.mdx) of files to write. |
...
`init` returns a [changeset](../../using/generating.mdx), a diff of the files to write. Dagger shows it to you to review before anything is written to disk.
...
:::note
`init` writes only your source (`dagger-module.toml`, `pyproject.toml`, `src/`). The generated `sdk/` client is produced by the first [binding regeneration](#regenerate-bindings), not by `init` itself, and is committed by default.
:::
...
- **`dagger init` / `dagger develop` not found.** Module development tooling lives in the Python SDK module, not the CLI. Install it with `dagger module install github.com/dagger/python-sdk`, then use `dagger api call python-sdk init …` and `dagger api call python-sdk mod generate`.
