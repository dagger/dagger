...
This page is the standalone reference for developing modules with Python. For platform concepts that apply to every SDK, start with the [SDKs overview](./index.mdx) and the [Types](../../api/reference/index.mdx) reference.
...
|---|---|
| `init` | Create a new Python SDK module. Returns a [changeset](../../cli/generating.mdx) of files to write. |
...
`init` returns a [changeset](../../cli/generating.mdx), a diff of the files to write. Dagger shows it to you to review before anything is written to disk.
