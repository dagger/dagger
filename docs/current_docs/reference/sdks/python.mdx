...
A **service** returns a `dagger.Service` and is run by `dagger start`. Combine `@function` with `@start`:
...
`dagger start` runs every `@start` service and exposes its ports. A plain `@function` returning `dagger.Service` can still be bound into other containers with `with_service_binding`, but only `@start` functions are run by `dagger start`. See [Services](#services).
