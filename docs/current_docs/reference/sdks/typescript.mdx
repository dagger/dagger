...
dagger api call typescript-sdk init --name=my-module
```

`init` returns a [changeset](../../using/generating.mdx) describing the files to
create, which Dagger shows you to review before anything is written to disk.

`init` arguments (from `dagger api call typescript-sdk init --help`):
...
:::note
`init` writes only your source (`dagger-module.toml`, `package.json`, `tsconfig.json`, `src/`). The generated `sdk/` client is produced by the first [binding regeneration](#regenerate-bindings), not by `init` itself. It is committed by default.
:::
...
```shell
dagger -m ./my-module api call build --src=.
```
...
dagger api call typescript-sdk mod deps list

# Add a dependency
dagger api call typescript-sdk mod deps add --source=github.com/dagger/dagger/modules/go

# Update one dependency, or all remote dependencies
dagger api call typescript-sdk mod deps update --help

# Remove a dependency by name or source
dagger api call typescript-sdk mod deps remove --help
```
...
```shell
dagger api call typescript-sdk mod generate
```
...
dagger api call typescript-sdk mod engine required

# Pin to a specific version
dagger api call typescript-sdk mod engine require --help

# Pin to the current engine, or the latest stable release
dagger api call typescript-sdk mod engine require-current
dagger api call typescript-sdk mod engine require-latest
```
