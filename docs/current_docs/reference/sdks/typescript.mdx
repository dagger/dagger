...
or a **service** (started by `dagger start`). Each is a regular `@func()` annotated
...
dependency. Mark a `@func()` with `@start()` and return a `Service`. `dagger start`
starts every service in the module. See [Services](../../using/services.mdx).

```typescript
import { Service, dag, object, func, start } from "@dagger.io/dagger"
...
The `@start()` decorator goes below `@func()`, and the function returns a `Service`
