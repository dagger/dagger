...
- [Generating code](../cli/generating.mdx) — how generators and tooling return diffs for you to apply
...
Like every Dagger tool that modifies your workspace, `dagger module init` returns a [changeset](../cli/generating.mdx) — a structured diff of the files to create — which Dagger shows you to review before anything is written to disk.
...
A useful, reusable module provides at least one of the three first-class function types — a **check**, a **generator**, or a **service** — so that the platform verbs (`dagger check`, `dagger generate`, `dagger up`) have something to run. These work the same in Go as in any SDK; see the [SDKs overview](./index.mdx) for the full treatment. In Go, you mark each one with a doc-comment **pragma** on the function:
...
- [SDKs overview](./index.mdx) — platform concepts that apply to every SDK
