...
Install a module into a workspace (`dagger.toml`) to make it part of a project's command surface; publish a reusable module when several projects need the same capability. See [Workspace Setup](../adopting/workspace-setup.mdx).
...
Round these out with the rest of the platform: settings and environments for team and per-context defaults, explicit `Secret` inputs for credentials, and [Dagger Cloud](../adopting/observability.mdx) for traces and remote execution.
