...
|---|---|
| `init` | Create a new Python SDK module. Returns a [changeset](../cli/changesets.mdx) of files to write. |
...
`init` returns a [changeset](../cli/changesets.mdx) — a diff of the files to write — which Dagger shows you to review before anything is written to disk.
...
See [Changesets](../cli/changesets.mdx).
