...
Generic modules can compose through workspace settings: if a module's constructor accepts an optional `Service`, you can wire another module's service into it with a **function reference** — no glue module required.
...
app = { from = "docusaurus:serve" }
```

The `from` value is a `<module>:<function>` path — the same string shown by `dagger up -l`. When `playwright` is constructed, Dagger resolves the reference and passes the service as the `app` argument.

This also works for `Container` constructor arguments: point `from` at any workspace module function that returns a `Container`, for example to share a common base image across modules.
