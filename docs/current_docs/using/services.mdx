...
```shell
dagger start -l                    # list available services
dagger start web                   # start only the 'web' service
dagger start web api redis         # start multiple services
```
...
The setting is a `<module>:<function>` string. The leading segment is a module's install name, the `[modules.X]` key in the same `dagger.toml`. The rest names a zero-arg function on it. Any function returning a matching type works, and Dagger injects the value it returns. `dagger start -l` is a convenient place to find and copy a service's path, but the path is valid because it resolves to a `Service`, not because the function is listed there. When Dagger constructs `playwright`, it resolves the reference and passes the service as the `app` argument.
