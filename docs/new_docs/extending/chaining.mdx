...
import Commands from '/snippets/commands.mdx';
...
Every `Container` object exposes a `Container.publish()` API method, which publishes the container as a new image to a specified container registry. The registry address is passed to the function using the `--address` argument, and the return title is a string referencing the container image address in the registry.
