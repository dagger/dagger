...
# A Dagger-Classic Plan orchestrates the Actions

## Plan structure

A config declared in Dagger-Classic starts with a plan, specifically `dagger.#Plan`
...
This Dagger-Classic property enables us to keep the entire CI/CD config in a single file, while keeping the integration execution separate from the deployment one.
...
This is one of the fundamental concepts that makes Dagger-Classic's CUE SDK a powerful language for building CI/CD pipelines.

If you want to learn more about packages in the context of CUE, the config language used by Dagger-Classic configs, check out the [Packages](1215-what-is-cue.md#packages) section on the **What is CUE?** page.

:::tip
Now that we understand the basics of a Dagger-Classic plan, we are ready to learn more about how to interact with the client environment.
