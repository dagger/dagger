...
Before we can build, test & deploy our example app with `dagger-cue`, we need to have Docker Engine running.
...
Before we can build & test our example app with `dagger-cue`, we need to have Docker running.
You most likely already have Docker set up.
If not, [Docker Desktop](https://www.docker.com/products/docker-desktop) makes this easy.
