...
`init` does **not** write files directly. Like every Dagger tool that modifies your workspace (for example `dagger call prettier write`, or a module's generators), it returns a [changeset](../../using-dagger/changesets.mdx) — a structured diff of files to create. You then choose how to apply it.
