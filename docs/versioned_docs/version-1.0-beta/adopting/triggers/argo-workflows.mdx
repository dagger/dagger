...
slug: /adopting/triggers/argo-workflows
description: "Learn how to use Dagger with Argo Workflows."
---
...
:::tip
For checks with no workflow YAML at all, enable [Cloud Checks](./index.mdx) with `dagger ws autocheck on`. The workflow below is the hybrid-mode alternative for when you want to drive Dagger from your existing Argo Workflows setup.
:::
...
- A running Kubernetes cluster [configured for use with Dagger](../scaling/kubernetes.mdx) and with a pre-configured `kubectl` profile
- Argo Workflows [installed](https://github.com/argoproj/argo-workflows/blob/master/docs/quick-start.md) in the cluster
- A repository with a Dagger workspace (`.dagger/config.toml`) — see [Workspace Setup](../workspace-setup.mdx)

## Example

The following example runs `dagger check` for a simple [Go application](https://github.com/kpenfound/greetings-api).
