...
:::tip
For checks with no buildspec at all, enable [Cloud Checks](/1.0-beta/adopting/triggers) with `dagger ws autocheck on`. The buildspec below is the hybrid-mode alternative for when you want to drive Dagger from your existing AWS CodeBuild setup.
:::
