...
:::tip
For checks with no workflow YAML at all, enable [Cloud Checks](/1.0-beta/adopting/triggers) with `dagger ws autocheck on`. The GitHub Action below is the hybrid-mode alternative for when you want to drive Dagger from your existing GitHub Actions setup.
:::
...
- A GitHub repository with a Dagger workspace (`.dagger/config.toml`) — see [Workspace Setup](/1.0-beta/adopting/workspace-setup).
...
To send traces to [Dagger Cloud](/1.0-beta/adopting/observability) and offload execution to a managed Dagger Engine, add your Cloud token:
