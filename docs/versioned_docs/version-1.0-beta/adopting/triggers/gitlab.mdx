...
:::tip
For checks with no pipeline YAML at all, enable [Cloud Checks](./index.mdx) with `dagger cloud check on`. The GitLab CI configuration below is the hybrid-mode alternative for when you want to drive Dagger from your existing GitLab CI setup.
:::
