...
:::tip
For checks with no `Jenkinsfile` changes at all, enable [Cloud Checks](/1.0-beta/adopting/triggers) with `dagger ws autocheck on`. The `Jenkinsfile` below is the hybrid-mode alternative for when you want to drive Dagger from your existing Jenkins setup.
:::
...
1. A repository with a Dagger workspace (`.dagger/config.toml`) — see [Workspace Setup](/1.0-beta/adopting/workspace-setup).
