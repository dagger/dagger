---
title: "Tekton"
slug: /adopting/triggers/tekton
description: "Learn how to integrate Dagger with Tekton for CI/CD pipelines."
---
...
:::tip
For checks with no pipeline YAML at all, enable [Cloud Checks](/1.0-beta/adopting/triggers) with `dagger ws autocheck on`. The Tekton Task below is the hybrid-mode alternative for when you want to drive Dagger from your existing Tekton setup.
:::
...
- A custom `dagger` Task, to run `dagger check` for the project (defined below).
