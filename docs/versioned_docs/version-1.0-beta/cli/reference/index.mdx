...
```
      --agent string                               Focus this restored agent (instance ID or name) instead of the top-level one
  ...
  -r, --resume trace[=picker]                      Resume agents from a retained trace (engine archive picker if no trace ID is given)
      --resume-timeout duration                    Maximum idle interval without bytes before a resume stream fails (background history retries nonfatally)
```
