...
This page is a standalone guide to the Go SDK. It assumes you already understand the platform concepts covered in the [Module Developer Guide](/1.0-beta/extending):

- [Type system](/1.0-beta/extending/type-system) — how SDK types map to the Dagger API
- [Checks](/1.0-beta/extending/how-dagger-works/checks) — functions that validate your code
- [Changesets](/1.0-beta/using-dagger/changesets) — how generators and tooling return diffs for you to apply
...
`init` does **not** write files directly. Like every Dagger tool that modifies your workspace (for example `dagger call prettier write`, or a module's generators), it returns a [changeset](/1.0-beta/using-dagger/changesets) — a structured diff of files to create. You then choose how to apply it.
...
`dagger generate` (the user-facing command described in [Generating Code](/1.0-beta/using-dagger/generating)) discovers and runs your module's generators, which for a Go module includes this binding regeneration. Both paths produce a [changeset](/1.0-beta/using-dagger/changesets) you review and apply.
...
A useful, reusable module provides at least one of the three first-class function types — a **check**, a **generator**, or a **service** — so that the platform verbs (`dagger check`, `dagger generate`, `dagger up`) have something to run. These work the same in Go as in any SDK; see the [Module Developer Guide](/1.0-beta/extending) for the full treatment. In Go, you mark each one with a doc-comment **pragma** on the function:
...
See [Generating Code](/1.0-beta/using-dagger/generating) and [Changesets](/1.0-beta/using-dagger/changesets).
...
- [Module Developer Guide](/1.0-beta/extending) — platform concepts that apply to every SDK
- [Type system](/1.0-beta/extending/type-system)
- [Checks](/1.0-beta/extending/how-dagger-works/checks)
- [Generating Code](/1.0-beta/using-dagger/generating) and [Changesets](/1.0-beta/using-dagger/changesets)
