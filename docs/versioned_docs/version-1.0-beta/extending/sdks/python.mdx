...
This page is the standalone reference for developing modules with Python. For platform concepts that apply to every SDK, start with the [Module Developer Guide](/1.0-beta/extending), and see the [Type System](/1.0-beta/extending/type-system), and [Checks](/1.0-beta/extending/how-dagger-works/checks).
...
|---|---|
| `init` | Create a new Python SDK module. Returns a [changeset](/1.0-beta/using-dagger/changesets) of files to write. |
| `mod` | Resolve the module at or above a workspace path; entry point for `deps`, `generate`, `engine`, `config`, `path`. |
| `mod deps` | Manage Dagger dependencies in `dagger.json` (`add`, `remove`, `update`, `list`). |
...
| `modules` | List every Dagger module whose `sdk.source` is `"python"`. |
...
`init` creates a new Python SDK module and returns a **changeset** — a diff of files to write — rather than writing directly to disk. This is the same review-then-apply flow as any Dagger generator (see [Changesets](/1.0-beta/using-dagger/changesets)).
...
Custom type names are namespaced in the schema by the module's main object (e.g. `MyModuleAccount`) to avoid conflicts when multiple modules are loaded together. See [User-defined object types](/1.0-beta/extending/type-system/user-defined-object-types).
...
See [Arguments and return values](/1.0-beta/extending/type-system/arguments-and-return-values) and [Scalars, lists, nullability, defaults](/1.0-beta/extending/type-system/scalars-lists-nullability-defaults).
...
Invalid values are rejected with an error listing the allowed choices. See [Enums and validation](/1.0-beta/extending/type-system/enums-and-validation).

## Core Dagger types

The generated client `dag` exposes the full Dagger API. The most common core types are `Container`, `Directory`, `File`, `Secret`, `Service`, and `CacheVolume`. They are immutable and lazy: each `with_*` method returns a new value, and nothing executes until you `await` a leaf operation. See [Core types](/1.0-beta/extending/type-system/core-types).
...
See [Designing for composability](/1.0-beta/extending/type-system/designing-for-composability).
...
See [Checks](/1.0-beta/extending/how-dagger-works/checks) and [Changesets](/1.0-beta/using-dagger/changesets).

### Directives

The annotations used above — `Name`, `Deprecated`, `Doc` — are the Python expression of Dagger's [directives](/1.0-beta/extending/how-dagger-works) and are applied via `typing.Annotated` (except `Doc`, which can be a plain annotation). The `@check`, `@generate`, and `@up` decorators mark function behavior.
...
Because all checks run in containers, local runs and CI runs are identical. See [Testing](/1.0-beta/extending/testing).
