...
This page is the standalone reference for developing modules with Python. For platform concepts that apply to every SDK, start with the [Module Developer Guide](../index.mdx), and see the [Type System](../type-system/index.mdx), and [Checks](../how-dagger-works/checks.mdx).

## The Python SDK is a module

Module development tooling lives in the **Python SDK module itself**: `github.com/dagger/python-sdk`, not the `dagger` CLI. You install it into your workspace once, then scaffold and maintain modules by calling its functions by name:
...
dagger api call python-sdk <function> ...
```

```shell
dagger api call python-sdk init --help
```
...
|---|---|
| `init` | Create a new Python SDK module. Returns a [changeset](../../using-dagger/changesets.mdx) of files to write. |
...
Install the Python SDK into your workspace, then create a new module with its `init` function:

```shell
dagger install github.com/dagger/python-sdk
dagger api call python-sdk init --name=my-module
```

`init` returns a [changeset](../../using-dagger/changesets.mdx) — a diff of the files to write — which Dagger shows you to review before anything is written to disk.
...
:::note
`init` writes only your source (`dagger-module.toml`, `pyproject.toml`, `src/`). The generated `sdk/` client is produced by the first [binding regeneration](#regenerate-bindings) — committed by default — not by `init` itself.
:::
...
The starter module is a single `@object_type` whose constructor reads the current workspace into a `source` field (see [Workspace inputs](#workspace-inputs)):
...
```shell
dagger api call --greeting=Hi --name=Dagger message
```
...
Custom type names are namespaced in the schema by the module's main object (e.g. `MyModuleAccount`) to avoid conflicts when multiple modules are loaded together. See [User-defined object types](../type-system/user-defined-object-types.mdx).
...
See [Arguments and return values](../type-system/arguments-and-return-values.mdx) and [Scalars, lists, nullability, defaults](../type-system/scalars-lists-nullability-defaults.mdx).
...
These descriptions surface in `dagger api functions` and `dagger api call --help`.
...
Invalid values are rejected with an error listing the allowed choices. See [Enums and validation](../type-system/enums-and-validation.mdx).
...
The `typing.Protocol` base is what marks this as a Dagger interface rather than a plain Python protocol. Any object whose functions match the protocol — including objects returned by other modules — can be passed where the interface is expected. See [Designing for composability](../type-system/designing-for-composability.mdx).

## Core Dagger types

The generated client `dag` exposes the full Dagger API. The most common core types are `Container`, `Directory`, `File`, `Secret`, `Service`, and `CacheVolume`. They are immutable and lazy: each `with_*` method returns a new value, and nothing executes until you `await` a leaf operation. See [Core types](../type-system/core-types.mdx).
...
```shell
dagger api call github-api --token=env:GITHUB_TOKEN
```
...
Add a dependency (returns a changeset of the `dagger-module.toml` edit to review and apply):
...
dagger api call python-sdk mod deps list

# Update one dependency by name, or all remote deps if --name is omitted
dagger api call python-sdk mod deps update --name=hello

# Remove a dependency by name or source
dagger api call python-sdk mod deps remove --name=hello
```
...
```shell
dagger api call python-sdk mod generate
```

To regenerate every Python SDK module discovered in the project at once:

```shell
dagger api call python-sdk generate-all
```
...
dagger api call python-sdk mod engine required

# Require a specific version
dagger api call python-sdk mod engine require --version=v{{ version }}

# Pin to the engine you're running now
dagger api call python-sdk mod engine require-current

# Pin to the latest stable release
dagger api call python-sdk mod engine require-latest
```
...
See [Checks](../how-dagger-works/checks.mdx) and [Changesets](../../using-dagger/changesets.mdx).

### Directives

The annotations used above — `Name`, `Deprecated`, `Doc` — are the Python expression of Dagger's [directives](../how-dagger-works/index.mdx) and are applied via `typing.Annotated` (except `Doc`, which can be a plain annotation). The `@check`, `@generate`, and `@up` decorators mark function behavior.
...
Because all checks run in containers, local runs and CI runs are identical. See [Testing](../testing.mdx).
...
dagger api call python-sdk mod config get
...
```shell
dagger install github.com/you/your-module@v0.1.0
```
...
- **`dagger init` / `dagger develop` not found.** Module development tooling lives in the Python SDK module, not the CLI. Install it with `dagger install github.com/dagger/python-sdk`, then use `dagger api call python-sdk init …` and `dagger api call python-sdk mod generate`.
...
- [Module Developer Guide](../index.mdx)
- [Type System](../type-system/index.mdx)
- [Checks](../how-dagger-works/checks.mdx)
- [Changesets](../../using-dagger/changesets.mdx)
