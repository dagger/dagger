...
- [Dagger CLI installed](../cli/install.mdx)
...
`dagger setup` walks through a few prompts. It logs you in to Dagger Cloud, then installs recommended modules based on a scan of your project. This project has no legacy Dagger configuration, so there is no migration step.

```shell
dagger setup
```

Log in to Dagger Cloud when prompted; you'll use it later to enable automatic checks. Then select `Install` to install the recommended modules.
...
All three modules run their checks concurrently. Some checks pass, but `prettier:check` fails because the example project has unformatted files, so `dagger check` exits non-zero.
...
This returns a [changeset](../cli/generating.mdx) with the reformatted files and offers to apply it to your working tree. Confirm to apply it. Run the checks again:
...
Now do it on your own project. See [Workspace setup](./workspace-setup.mdx).
