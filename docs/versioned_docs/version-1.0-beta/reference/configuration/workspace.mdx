...
Resolved lookups — module sources, container image references, and Git references — are recorded in `.dagger/lock` alongside the config. Refresh it with `dagger workspace update` or `dagger lock update`. See [Lockfiles](../cli/lockfiles.mdx) for lock modes and policies.
