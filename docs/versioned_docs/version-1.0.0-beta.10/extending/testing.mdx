...
slug: /extending/testing
description: "Patterns for testing Dagger modules."
---

# Testing Modules

Like any other piece of software, Dagger Functions and modules should be thoroughly tested. This page documents proven patterns for effectively testing reusable modules.
...
```go file=./snippets/testing/go/main.go
```
</TabItem>
<TabItem value="python" label="Python">

```python file=./snippets/testing/python/src/greeter/main.py
```

</TabItem>
<TabItem value="typescript" label="TypeScript">

```typescript file=./snippets/testing/typescript/src/index.ts
```
...
Example modules are special modules designed to showcase your own modules, offering better demonstrations than the automatically generated ones.
...
Since test functions are ordinary Dagger functions, you can return any value that's allowed. While this approach works fine when running a test with `dagger api call`, there are scenarios where a single return value isn't sufficient. For example, you might need to handle multiple output objects, wait for asynchronous operations, manage errors, or (in some languages) provide additional context.
...
In some situations, you may need to provide specific values to your test modules, such as when authenticating against an external service. In these cases, you can rely on [module constructors](./sdks/dang#arguments-and-return-values) to inject any required inputs.
...
You can now run all tests for the module using `dagger api call -m tests all`.
