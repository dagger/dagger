...
Dagger Modules run in a runtime container that's bootstrapped by the Dagger Engine, with the neccessary environment to run the Dagger Module's code. It's currently hardcoded to run in Go 1.21, but more configuration is planned in the future.
