...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "MyModule":
		...
		case "Build":
			...
			if inputArgs["architecture"] != nil {
				err = json.Unmarshal([]byte(inputArgs["architecture"]), &architecture)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg architecture", err))
				}
			}
			var os string
			if inputArgs["os"] != nil {
			...
			return (*MyModule).Build(&parent, ctx, source, architecture, os), nil
		default:
				...
				dag.TypeDef().WithObject("MyModule").
					WithFunction(
						dag.Function("Build",
							...
							WithArg("architecture", dag.TypeDef().WithKind(StringKind)).
							WithArg("os", dag.TypeDef().WithKind(StringKind)))), nil
	default:
