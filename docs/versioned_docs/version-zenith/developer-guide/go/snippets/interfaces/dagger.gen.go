...
func LoadFooerFromID(r *Client, id FooerID) Fooer {
	q := r.Query.Select("loadMyModuleFooerFromID")
	q = q.Arg("id", id)
	return &fooerImpl{
		Client: r.Client,
		Query:  q,
...
func (r *fooerImpl) XXX_GraphQLType() string {
	return "Fooer"
}

func (r *fooerImpl) XXX_GraphQLIDType() string {
	return "FooerID"
}

func (r *fooerImpl) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
...
func (r *fooerImpl) MarshalJSON() ([]byte, error) {
	if r == nil {
		return []byte("\"\""), nil
	}
...
func (r *fooerImpl) UnmarshalJSON(bs []byte) error {
	var id FooerID
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = *LoadFooerFromID(dag, id).(*fooerImpl)
	return nil
}

func (r *fooerImpl) toIface() Fooer {
	if r == nil {
		return nil
	}
	return r
...
func (r *fooerImpl) ID(ctx context.Context) (FooerID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.Query.Select("id")
	var response FooerID
	q = q.Bind(&response)
	return response, q.Execute(ctx, r.Client)
}

func (r *fooerImpl) Foo(ctx context.Context, bar int) (string, error) {
	if r.foo != nil {
		return *r.foo, nil
	}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "MyModule":
		...
		case "Foo":
			...
			var fooer *fooerImpl
			if inputArgs["fooer"] != nil {
				err = json.Unmarshal([]byte(inputArgs["fooer"]), &fooer)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg fooer", err))
				}
			}
			return (*MyModule).Foo(&parent, ctx, fooer.toIface())
		default:
				...
				dag.TypeDef().WithObject("MyModule").
					WithFunction(
						dag.Function("Foo",
							...
							WithArg("fooer", dag.TypeDef().WithInterface("Fooer")))).
			WithInterface(
				dag.TypeDef().WithInterface("Fooer").
					WithFunction(
						dag.Function("Foo",
							dag.TypeDef().WithKind(StringKind)).
							WithArg("bar", dag.TypeDef().WithKind(IntegerKind)))), nil
	default:
