...
There's a [dagger.field][dag-field] descriptor that is wrapper of
[dataclasses.field][data-field], but creates a getter function for the attribute
as well so that it's accessible from the API.
...
We can confirm with `dagger call --help` that only the `greeting` function
was created, while `name` remains only a constructor argument:
