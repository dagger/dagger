...
1. Update the generated `dagger/main.go` file with the following code.
    ...
    The `Build()` function takes care of building the application image.
      ...
      - It uses the `Container.WithDirectory()` method to return the container image with the application source code written at the `/src` path, and the `Container.WithWorkdir()` method to set the working directory in the container.
      - It chains the `Container.WithExec()` method to copy the contents of the working directory to the `/home/node` directory in the container and then uses the `Container.WithWorkdir()` method to change the working directory in the container to `/home/node`.
    ...
    The `Deploy()` function takes care of deploying the application to Google Cloud Run.
      ...
      - It creates a Google Cloud Run client and updates the Google Cloud Run service defined in Step 3 to use the published container image.
...
1. Update the generated `dagger/src/main.py` file with the following code.
    ...
    The `build()` function takes care of building the application image.
      ...
      - It uses the `Container.with_directory()` method to return the container image with the application source code written at the `/src` path, and the `Container.with_workdir()` method to set the working directory in the container.
      - It chains the `Container.with_exec()` method to copy the contents of the working directory to the `/home/node` directory in the container and then uses the `Container.with_workdir()` method to change the working directory in the container to `/home/node`.
...
Once credentials are configured, test the Dagger Functions by running the command below from the application directory. Remember to update the `service` and `registry` arguments if you specified different values when creating the various Google Cloud services in Step 3 and Step 4.

```shell
dagger -m ci call publish --source . --service projects/myproject/locations/us-central1/services/myapp --registry us-central1-docker.pkg.dev/myproject/myrepository/myapp --credential env:GOOGLE_CREDENTIALS
```

Dagger performs the operations defined in the `publish()` Dagger Function, logging each operation to the console. At the end of the process, the built container is deployed to Google Cloud Run and the function returns the deployment URL. Browse to the deployment URL to see the running application.
...
This guide walked you through the process of creating Dagger Functions to continuously build and deploy a Node.js application on Google Cloud Run. It explained key concepts, objects and methods available in the Dagger SDKs to create Dagger Functions.
