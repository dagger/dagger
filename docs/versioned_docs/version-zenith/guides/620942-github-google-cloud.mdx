...
1. Update the generated `dagger/main.go` file with the following code.
    ...
    The constructor accepts a single argument, a `Directory` representing the directory containing the application source code. By convention, constructor arguments are available to all the functions in the object. Since this Dagger Module has only one object, all the Dagger Functions in the Dagger Module have access to this constructor argument.
...
```shell
dagger call --source . deploy --project myproject --registry-location us-central1 --repository myregistry/myapp --service-location us-central1 --service myservice  --credential env:GOOGLE_CREDENTIALS
```
