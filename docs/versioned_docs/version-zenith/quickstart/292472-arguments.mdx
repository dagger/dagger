...
```shell
dagger -m github.com/shykes/daggerverse/hello call test --greeting bonjour --name daggernaut
...
Here we start exploring some of the features that make Dagger unique.
...
Let's try this with our `hello` function. In order to print giant letters, it dynamically executes the `figlet` utility in a container and returns the container's output. By default it builds a container with figlet installed; but it allows passing a custom container, with the `figletContainer` argument.

Let's try calling `hello` with a custom container, which we know has the `figlet` tool installed:
