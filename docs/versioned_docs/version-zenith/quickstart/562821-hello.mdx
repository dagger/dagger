...
If you [inspect the source code of the module](https://github.com/shykes/daggerverse/blob/0d8450fb2554d17854566028538aa6d55cdc1640/hello/main.go), you'll see a `Hello()` function, written in Go, which prepares a message and returns it as a string. if the `giant` argument is set, it dynamically installs a tool called `figlet` into a container, executes it to produce giant letters, and returns the result.
