...
Like `Directory`, `Container` is a Dagger core type which comes with a number of functions of its own - in particular, a `Shell()` function, which can be used to open an interactive shell session with the running container.
