...
title: "Building a container with a function"
---
...
## Building a container with a function
...
Like `Directory`, `Container` is a Dagger primitive which comes with a number of functions of its own - in particular, a `Shell()` function, which can be used to open an interactive shell session with the running container.
