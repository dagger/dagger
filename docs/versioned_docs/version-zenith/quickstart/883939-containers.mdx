...
The example above chains these various functions together and returns a special Dagger `Container` type representing the base image plus additional packages. The interesting thing here is that `Container` is a core type, which comes with a number of functions of its own - in particular, a `Shell()` function, which can be used to open an interactive shell session with the running container.
