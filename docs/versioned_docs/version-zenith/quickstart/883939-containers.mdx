...
title: "Chain container functions"
---
...
## Chain container function calls

Just as you can chain `Directory` function calls, you can also chain `Container` function calls. As an example, the Wolfi container builder module exposes a number of functions:
...
Chain these functions together to build a Wolfi container image with the `cowsay` package:

```shell
dagger call -m github.com/shykes/daggerverse/wolfi base with-package cowsay container
```

The function call above returns a Dagger `Container` representing the base image plus additional packages. You should see the container being built and the packages being added, as shown below:
...
The interesting thing here is that `Container` is a Dagger primitive, which comes with a number of functions of its own - in particular, a `Shell()` function, which can be used to open an interactive shell session with the running container.
...
The `Container` primitive has a `WithExec()` function, which returns the container after executing a specific command inside it. So, you could achieve the same output as before by chaining a function call to `WithExec()` on the `Container` returned previously, as shown in the following command:
...
The `Container` primitive also has a `Publish()` function, which publishes the container to a registry. To see this in action, call the previous function again, this time chaining an additional function call to `Publish()` on the returned `Container`:
