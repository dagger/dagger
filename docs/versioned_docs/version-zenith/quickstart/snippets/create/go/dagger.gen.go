...
// The `WolfiConfigID` scalar type represents an identifier for an object of type WolfiConfig.
type WolfiConfigID string
...
// Load a WolfiConfig from its ID.
func (r *Client) LoadWolfiConfigFromID(id WolfiConfigID) *WolfiConfig {
	q := r.q.Select("loadWolfiConfigFromID")
	q = q.Arg("id", id)
...
type WolfiConfig struct {
	q *querybuilder.Selection
	c graphql.Client

	id *WolfiConfigID
}
type WithWolfiConfigFunc func(r *WolfiConfig) *WolfiConfig

// With calls the provided function with current WolfiConfig.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *WolfiConfig) With(f WithWolfiConfigFunc) *WolfiConfig {
	return f(r)
}

// The container for this configuration
func (r *WolfiConfig) Container() *Container {
	q := r.q.Select("container")
...
// A unique identifier for this WolfiConfig.
func (r *WolfiConfig) ID(ctx context.Context) (WolfiConfigID, error) {
	if r.id != nil {
		return *r.id, nil
	}
	q := r.q.Select("id")

	var response WolfiConfigID
...
// XXX_GraphQLType is an internal function. It returns the native GraphQL type name
func (r *WolfiConfig) XXX_GraphQLType() string {
	return "WolfiConfig"
}

// XXX_GraphQLIDType is an internal function. It returns the native GraphQL type name for the ID of this object
func (r *WolfiConfig) XXX_GraphQLIDType() string {
	return "WolfiConfigID"
}

// XXX_GraphQLID is an internal function. It returns the underlying type ID
func (r *WolfiConfig) XXX_GraphQLID(ctx context.Context) (string, error) {
	id, err := r.ID(ctx)
	if err != nil {
		return "", err
	}
...
func (r *WolfiConfig) MarshalJSON() ([]byte, error) {
	id, err := r.ID(context.Background())
	if err != nil {
		return nil, err
	}
...
func (r *WolfiConfig) UnmarshalJSON(bs []byte) error {
	var id string
	err := json.Unmarshal(bs, &id)
	if err != nil {
		return err
	}
	*r = *dag.LoadWolfiConfigFromID(WolfiConfigID(id))
	return nil
}

func (r *WolfiConfig) Overlays(ctx context.Context) ([]Container, error) {
	q := r.q.Select("overlays")
	...
	convert := func(fields []overlays) []Container {
		out := []Container{}

		for i := range fields {
			val := Container{id: &fields[i].Id}
			val.q = querybuilder.Query().Select("loadContainerFromID").Arg("id", fields[i].Id)
			val.c = r.c
			out = append(out, val)
		}
...
func (r *WolfiConfig) Packages(ctx context.Context) ([]string, error) {
	q := r.q.Select("packages")
...
func (r *WolfiConfig) WithOverlay(image *Container) *WolfiConfig {
	assertNotNil("image", image)
	q := r.q.Select("withOverlay")
	q = q.Arg("image", image)
...
func (r *WolfiConfig) WithPackage(name string) *WolfiConfig {
...
func (r *WolfiConfig) WithPackages(packages []string) *WolfiConfig {
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "MyModule":
		...
		case "Test":
			...
			return (*MyModule).Test(&parent), nil
		case "GrepDir":
			var parent MyModule
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
