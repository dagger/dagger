...
- You have a source code repository and a Dagger Module that interacts with it. If not, fork and then locally clone the [Dagger source code repository](https://github.com/dagger/dagger), which you can use with the [Go builder module](https://github.com/kpenfound/dagger-modules/tree/main/golang) from the [Dagger Quickstart](../../quickstart/index.mdx).
...
To find your token, browse to the **Organization Settings** page, by clicking your profile photo in the left sidebar and then clicking **Organization Settings**. Select the **Configuration** tab. You can also use this URL pattern: `https://dagger.cloud/{Your Org Name}/settings?tab=configuration`
    ...
    ![Get token](/img/versioned_docs/version-zenith/user-guide/cloud/get-started/get-token.png)
...
To see this in action, commit a change to your locally-cloned example repository and build the Dagger CLI locally.
...
Because you ran this in a Git context, the Dagger Cloud UI shows the VCS Git metadata associated with the commit.

![Local run with metadata](/img/versioned_docs/version-zenith/user-guide/cloud/get-started/local-run-git-metadata.png)
...
![Local run, no metadata](/img/versioned_docs/version-zenith/user-guide/cloud/get-started/local-run-no-metadata.png)
...
To do this, trigger your CI workflow by pushing a commit or opening a pull request. If you are using the starter application and pipeline from Appendix A, use the following commands:

```shell
echo "test on Dagger Cloud" >> README.md
git commit -a -m "Updated README again"
git push
```
...
![View runs](/img/versioned_docs/version-zenith/user-guide/cloud/get-started/view-runs.png)
...
![View run details](/img/versioned_docs/version-zenith/user-guide/cloud/get-started/view-run-details-tree.png)
...
Dagger Cloud automatically detects and creates cache volumes when they are declared in your code. To see how this works, add a cache volume to your Dagger pipeline and then trigger a run.
...
![Manage volumes](/img/versioned_docs/version-zenith/user-guide/cloud/get-started/manage-volumes.png)
...
This guide introduced you to Dagger Cloud and walked you registering a new organization, integrating Dagger Cloud with your CI provider/tool, and using Dagger Cloud's visualization and caching features. For more information and technical support, contact Dagger via the support messenger in Dagger Cloud.
