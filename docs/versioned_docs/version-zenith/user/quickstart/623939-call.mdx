...
Modules don't need to be installed locally. Dagger lets you consume modules from GitHub as well. For example, you can `dagger call` a function from a module using its GitHub URL:

```shell
dagger call -m github.com/vikram-dagger/daggerverse/hello-dagger@main hello-dagger
```

### Use modules from the local filesystem
...
- [Publish a container to the `ttl.sh` registry](https://daggerverse.dev/mod/github.com/shykes/daggerverse/ttlsh): `dagger call -m github.com/shykes/daggerverse/ttlsh publish --ctr alpine:latest`
:::
