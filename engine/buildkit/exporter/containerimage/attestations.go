...
func supplementSBOM(ctx context.Context, s session.Group, target cache.ImmutableRef, targetRemote *cache.Remote, att exporter.Attestation) (exporter.Attestation, error) {
	if target == nil {
		return att, nil
	}
...
func newFileLayerFinder(target cache.ImmutableRef, remote *cache.Remote) (fileLayerFinder, error) {
