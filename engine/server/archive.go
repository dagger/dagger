...
//nolint:gocyclo // Verify one complete resume closure before writing any bootstrap data.
...
//nolint:gocyclo // Validate the authoritative roster and its ancestry as one contract.
...
func (srv *Server) archiveRequestRecord(metadataClientID, sessionID, token string) (*clientRecord, error) {
	...
	// Read the bootstrap credential under scopeMu: archive reads may precede
	// the first executable query, which seals metadata and fills secretToken.
	record.daggerSession.scopeMu.Lock()
	storedToken := ""
	if record.clientMetadata != nil {
		storedToken = record.clientMetadata.ClientSecretToken
	}
...
//nolint:gocyclo // Keep signal dispatch and finite-stream error handling at one HTTP boundary.
