...
func TestArchiveRequestRequiresMainClientSecret(t *testing.T) {
	...
	// Archive reads must work before an executable query seals the metadata.
	main := &clientRecord{daggerSession: sess, clientID: "main", clientMetadata: &engine.ClientMetadata{ClientSecretToken: "secret"}}
	nested := &clientRecord{daggerSession: sess, clientID: "nested", clientMetadata: &engine.ClientMetadata{ClientSecretToken: "nested-secret"}}
	...
	if _, err := srv.archiveRequestRecord("main", "session", ""); err == nil {
