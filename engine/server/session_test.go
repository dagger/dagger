...
func TestSessionClientStatsCountsRetainedDangClients(t *testing.T) {
	sess := &daggerSession{
		sessionID:          "session",
		mainClientCallerID: "main",
		clients: map[string]*daggerClient{
			"main": {clientID: "main", activeCount: 1},
			"module": {
				clientID:    "module",
				activeCount: 1,
			},
			"dang-idle": {
				clientID:                 "dang-idle",
				hostServiceProxyClientID: "main",
			},
			"dang-active": {
				clientID:                 "dang-active",
				hostServiceProxyClientID: "main",
				activeCount:              1,
			},
		},
	...
	require.Equal(t, SessionClientStats{
		Total:      4,
		Nested:     3,
		IdleNested: 1,
		Dang:       2,
		IdleDang:   1,
	}, srv.SessionClientStats())
}

func TestRetireClientRemovesNestedClient(t *testing.T) {
	mainClient := &daggerClient{clientID: "main", shutdownCh: make(chan struct{})}
	nestedClient := &daggerClient{
		clientID:                 "nested",
		hostServiceProxyClientID: "main",
	...
	require.NoError(t, srv.RetireClient(t.Context(), &engine.ClientMetadata{
		SessionID: "session",
		ClientID:  "nested",
	}))
	require.Equal(t, map[string]*daggerClient{"main": mainClient}, sess.clients)
	require.Zero(t, sess.dangClientCount)
	select {
	case <-nestedClient.shutdownCh:
	default:
		t.Fatal("retired client shutdown channel was not closed")
	}
}

func TestRetireClientWaitsForDescendants(t *testing.T) {
	mainClient := &daggerClient{clientID: "main", shutdownCh: make(chan struct{})}
	parent := &daggerClient{
		clientID:                 "parent",
		hostServiceProxyClientID: "main",
	...
	require.Contains(t, sess.clients, "parent")
...
func TestRetireClientWaitsForActiveRequest(t *testing.T) {
	nestedClient := &daggerClient{
		clientID:    "nested",
		activeCount: 1,
		shutdownCh:  make(chan struct{}),
	...
	sess := &daggerSession{
		...
		mainClientCallerID: "main",
		clients: map[string]*daggerClient{
			"main":   {clientID: "main", shutdownCh: make(chan struct{})},
			"nested": nestedClient,
		},
	}
	sess.state.Store(sessionStateInitialized)
	...
	require.Contains(t, sess.clients, "nested")
	require.NoError(t, srv.releaseClientConnection(t.Context(), sess, nestedClient))
	require.Eventually(t, func() bool {
		sess.clientMu.RLock()
		defer sess.clientMu.RUnlock()
		_, ok := sess.clients["nested"]
		return !ok
