...
func (srv *Server) detectAndLoadWorkspaceWithRootfs(
	...
	if wsConfig != nil && len(wsConfig.Include) > 0 {
		...
		wsConfig, err = workspace.MergeIncludedConfig(included, wsConfig, wsConfigKeys)
		if err != nil {
			return err
		}
	}
	if err := workspace.ValidateEffectiveConfig(wsConfig); err != nil {
		return err
	}
...
func (srv *Server) cloneGitTree(ctx context.Context, dag *dagql.Server, cloneRef, version string) (dagql.ObjectResult[*core.Directory], dagql.ObjectResult[*core.GitRef], error) {
	return core.CloneWorkspaceGitTree(ctx, dag, cloneRef, version)
}
