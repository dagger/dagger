...
func (w *watcher) update(server FSWatch_WatchServer) error {
	for {
		...
		case *WatchRequest_UpdateWatch:
			...
			newWatch := make([]string, 0, len(req.UpdateWatch.Paths))
			for _, p := range req.UpdateWatch.Paths {
				if strings.HasSuffix(p, string(filepath.Separator)) {
					...
					p = filepath.Clean(p)
					newWatch = append(newWatch, filepath.Dir(p))
					files[p] = struct{}{}
				}
			}
...
func (w *watcher) updateWatchers(newWatch []string) error {
	// TODO: optimize
	oldWatch := w.watcher.WatchList()

	// add all paths not currently being watched
	for _, path := range newWatch {
		if !slices.Contains(oldWatch, path) {
	...
	for _, path := range oldWatch {
		if !slices.Contains(newWatch, path) {
