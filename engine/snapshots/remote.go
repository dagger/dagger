...
// GetRemotes gets []*Remote from content store for this ref (potentially pulling lazily).
...
func (sr *immutableRef) GetRemotes(ctx context.Context, createIfNeeded bool, refCfg config.RefConfig, all bool, s session.Group) ([]*Remote, error) {
	...
	if !all || refCfg.Compression.Force || len(remote.Descriptors) == 0 {
		return []*Remote{remote}, nil // early return if compression variants aren't required
	}
	...
	if len(parentChain) == 0 {
		...
		parents, err := getAvailableBlobs(ctx, sr.cm.ContentStore, &Remote{
...
func appendRemote(parents []*Remote, desc ocispecs.Descriptor, p content.InfoReaderProvider) (res []*Remote) {
...
func getAvailableBlobs(ctx context.Context, cs content.Store, chain *Remote) ([]*Remote, error) {
	...
	parents, err := getAvailableBlobs(ctx, cs, &Remote{
		Descriptors: parentChain,
...
func (sr *immutableRef) getRemote(ctx context.Context, createIfNeeded bool, refCfg config.RefConfig, s session.Group) (*Remote, error) {
...
func (p lazyRefProvider) Info(ctx context.Context, dgst digest.Digest) (content.Info, error) {
	...
	// If this digest can be fetched via desc handler, return synthetic metadata and
	// defer fetching until ReaderAt/Unlazy.
	if p.dh == nil {
		return content.Info{}, err
	}
	return content.Info{
...
func (p lazyRefProvider) Unlazy(ctx context.Context) error {
	_, err := p.ref.cm.unlazyG.Do(ctx, string(p.desc.Digest), func(ctx context.Context) (_ struct{}, rerr error) {
		if _, err := p.ref.cm.ContentStore.Info(ctx, p.desc.Digest); err == nil {
			return struct{}{}, nil
		...
		if p.dh == nil {
			// No provider available and content is missing.
		...
		if err := p.ref.linkBlob(ctx, p.desc); err != nil {
			return struct{}{}, err
		}
