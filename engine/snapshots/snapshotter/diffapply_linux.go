...
func (sn *mergeSnapshotter) diffApply(ctx context.Context, dest Mountable, diffs ...Diff) (_ snapshots.Usage, rerr error) {
	...
	for _, diff := range diffs {
		...
		d, err := differFor(lowerMntable, upperMntable, diff.Comparison)
		if err != nil {
...
func applierFor(dest Mountable, tryCrossSnapshotLink, userxattr bool) (_ *applier, rerr error) {
	...
	if fsdiff.IsOverlayMountType(mnt) {
...
func (a *applier) applyDelete(ca *changeApply) (bool, error) {
	deleteOnly := ca.kind == continuityfs.ChangeKindDelete
...
func (a *applier) applyCopy(ctx context.Context, ca *changeApply) error {
	switch ca.srcStat.Mode & unix.S_IFMT {
	case unix.S_IFREG:
		if err := continuityfs.CopyFile(ca.dstPath, ca.srcPath); err != nil {
...
func differFor(lowerMntable, upperMntable Mountable, comparison fsdiff.Comparison) (_ *differ, rerr error) {
	...
	if len(upperMnts) == 1 {
		...
		} else if fsdiff.IsOverlayMountType(upperMnts[0]) {
			overlayDirs, err := fsdiff.GetOverlayLayers(upperMnts[0])
			if err != nil {
...
func (d *differ) overlayChanges(ctx context.Context, handle func(context.Context, *change) error) error {
	return fsdiff.WalkUpperdirChanges(ctx, func(kind continuityfs.ChangeKind, subPath string, srcfi os.FileInfo, prevErr error) error {
		if prevErr != nil {
			return prevErr
		}
		...
		if srcfi != nil {
			...
			if !srcfi.IsDir() && c.srcStat.Nlink > 1 && c.kind != continuityfs.ChangeKindDelete {
				if linkSubPath, ok := d.inodes[statInode(c.srcStat)]; ok {
					c.linkSubPath = linkSubPath
				} else {
					d.inodes[statInode(c.srcStat)] = c.subPath
		...
		return handle(ctx, c)
	}, d.upperdir, d.upperRoot, d.lowerRoot, d.comparison)
}
...
func safeJoin(root, path string) (string, error) {
	...
	parent, err := continuityfs.RootPath(root, dir)
	if err != nil {
		return "", err
	}
