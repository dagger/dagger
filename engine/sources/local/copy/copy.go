...
func newCopier(destRoot string, chown Chowner, tm *time.Time, mode *int, xeh XAttrErrorHandler, only []string, includePatterns, excludePatterns []string, useGitignore bool, alwaysReplaceExistingDestPaths bool, changeFunc fsutil.ChangeFunc) (*copier, error) {
	if xeh == nil {
	...
	if only != nil {
		...
		for _, p := range only {
			onlySet[filepath.Clean(p)] = struct{}{}
		}
	}
