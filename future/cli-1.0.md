...
The optional `--sdk` flag selects one installed SDK module. Without it, the engine uses the current-scope lookup for each installed SDK. The engine selects the unique deepest scope. If several SDKs have that deepest scope, the command reports an error with their names, paths, and `--sdk` guidance.

The command reports an error if no scope exists or an inspected SDK fails. It changes at most one scope. The command accepts no SDK setting flags and uses saved settings.
...
Without `--all`, the command lists clients in scopes that contain the current directory.
...
`TARGET` contains the complete stored removal key. `SCOPE` is relative to the workspace root. To remove a specific row, run `dagger module client rm <TARGET> --sdk=<SDK>` from its listed scope.
...
| `module client add <module> [--sdk=SDK]` | The selected SDK, or all installed SDKs | Persist the unique deepest client scope |
| `module client rm <module> [--sdk=SDK]` | None | Use the deepest matching recorded scope |
...
The CLI must expose these settings as flags for module initialization.
...
`dagger module init --help` must list one subcommand for each installed SDK. Each SDK subcommand's help must list that SDK's settings.

`dagger module client add` and `rm` accept optional `--sdk`. Their flag help must list installed SDK names in sorted order, or state that none are installed. Client commands have no SDK subcommands or SDK setting flags.

The `Workspace.withClient` API accepts explicit settings only with an explicit SDK. It preserves saved settings. The `sdk` argument is optional in both `Workspace.withClient` and `Workspace.withoutClient`.

Help must work for local and remote workspaces selected with `-W`.

Module initialization must reject a setting flag that does not belong to the selected SDK module.
