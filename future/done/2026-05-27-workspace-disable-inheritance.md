...
The mechanism below originally gated on `CurrentModule` **or** `CurrentFunctionCall`
(and the server twin on `client.mod` or `client.fnCall`). That over-matched:
`client.mod` / `CurrentModule` is set for any module-scoped client, including
schema-walking flows like `dagger generate`, `dagger check`, and `dagger up`,
which load a module but run no function. Those flows were wrongly denied their
auto-injected `Workspace`.

Final rule: gate on the **active function call only** (`CurrentFunctionCall` /
`client.fnCall`). It is set exclusively when a module function body is executing,
