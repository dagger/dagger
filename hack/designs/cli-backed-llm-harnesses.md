...
> Extraction note (2026-09-17): this design is retained from the source branch.
> The implementation uses the current `LLM.spawn(handle:)` restore API and
> current nested-client transport manager. Codex token refresh uses the existing
> rejected-token fingerprint mechanism. The Claude/Codex examples are registered
> in the root `dagger.toml`; the legacy example `dagger.json` files are replaced.
