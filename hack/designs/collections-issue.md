...
### 8. Port checks, up, generators, and shells to artifacts

Today `core/modtree.go` has six walks over the same tree: `RollupChecks`, `RollupUp`, `RollupGenerator`, `RollupAgents`, `RollupTerminals`, and `ModuleArtifactNodes`. `dagger check`, `dagger up`, `dagger generate`, and `dagger shell` each have their own walk, filter, and runner. Port them to one walk and one selection API, in this order. Land each step on its own, with the integration suite green after each one.

1. **One walk.** The walk predicate returns two values: collect and descend. A node with a directive returns no descent. Section 7 gives the collect rule. The six walks become filters over this walk. `RollupTerminals` is `filterTypes(["Container", "Directory"])`.
...
3. **Select through artifacts.** `Workspace.checks`, `up`, `generators`, and `terminals` select their targets with `artifacts` and `filterDirectives`. Wire types and behavior do not change.
4. **One evaluation.** `Artifacts.values(failFast:)` evaluates every artifact in parallel, with one span per artifact, and records each error in an `ArtifactResult`. The wrapper is necessary: a raised error in a non-null field ends the whole list. After evaluation, the directive selects the last step. A check needs nothing. An up calls `up` with the port mappings. A generate takes the `Changeset`. This step removes `CheckGroup`, `GeneratorGroup`, `TerminalGroup`, and the per-command runners in `modtree.go`.
...
Keep these behaviors through all four steps:
...
Ship the `Check` projection and `Changeset.stale` with the walk, and all four steps of the port, `dagger shell` first.
...
- Batch replacement, per-item fallback, and unchanged behavior for static artifacts and older API versions.
