...
No TLA source or configuration changed, and no TLC run was made for this change. No broad Go suite was run. **Verified** from the source diff and recorded commands. The preexisting raw `Nullable[Int]` JSON-null limitation remains unchanged. Its initial compatibility fixture was replaced with the supported null-envelope form. Separate fixture corrections supplied a live consumer span, the missing Platform and the container schema selector. Their failed logs remain in the evidence; none is counted as a successful check.
