...
- `fix(idtui): auto-hide completed transfers from the live progress roll-up` — rolled-up progress rows previously rendered every descendant source unconditionally and the TUI's default verbosity (`dagger call` = ShowCompletedVerbosity) never GCs completed spans, so finished transfers pierced their collapsed ancestors forever, accumulating without bound on large traces. The roll-up filter is activity-based, NOT ShouldShow/GC-based (a first attempt with `ShouldShow` was a no-op at default verbosity — verified live): live rendering skips sources that aren't `IsRunningOrEffectsRunning`, while the final render (`r.final`, also `Debug`) keeps all of them as the run's transfer summary — the PR's marquee output is final-render roll-up, and goldens run `--progress=report` (final-only), so neither changes. Completed transfers stay reachable live by expanding the parent (progress still reveals encapsulated spans in natural position). Covered by `TestRenderProgressSpanRowsAutoHide` (live hides completed/keeps in-flight; final keeps both); `TestRenderProgressSpanRows`' collapsed case now asserts via `FinalRender`. Live invalidation needs no new plumbing: an in-flight roll-up row renders a spinner child in the host SpanTreeView, tuist Update() propagation re-renders the host every tick, and the render that drops the row also dismounts the spinner. Verified e2e with the `transient-progress` viztest (see below) against a dev engine: live frames show only the in-flight transfer; the final frame shows both.
- viztest `TransientProgress` (dagql/idtui/viztest/main.go) — a long-lived collapsed "syncing layers" span containing one instantly-completing transfer and one ~8s in-flight transfer; built for live QA of the auto-hide (not a golden: wall-clock pacing). `PartialProgress`'s emit helper is extracted to package-level `emitProgress`.
