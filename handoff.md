...
- `fix(idtui): fold quick transfers into one roll-up line` — only transfers active ≥ `quickTransferThreshold` (1s) earn their own roll-up row. Live: an in-flight transfer's row appears once it crosses the threshold (sub-second storms — e.g. a module fetching dozens of apk packages — never spam the view; completed ones still drop out per the previous commit). Final render: slow transfers render individually as the run's summary; quick ones merge into a single line like `✔ 1 upload, 38 fetches 0.8s` — counts by kind (`transferSummary` maps the emitters' leading verb pulling/unpacking/fetching/uploading/downloading to nouns, unknown → "transfers", first-appearance order) plus the wall-clock union of their `dagui.Activity` (parallelism-aware; `Activity.Add` + `Duration`). No byte total on the merged line — fetch and unpack read the same bytes, same stance as never merging bars into parent titles. A merged group of one renders as the real row. Debug or verbosity ≥ ShowSpammyVerbosity shows every transfer individually. Goldens run `--progress=report` with all-1.0s spans (exactly at threshold → individual), so no churn. Tests: `TestRenderProgressSpanRowsAutoHide` (live hides completed / keeps in-flight; final keeps slow), `TestRenderProgressMergedRollup` (merged line text + union duration, quick names absent), `TestRenderProgressLiveHidesQuick` (young in-flight row suppressed). Known imprecision: the threshold compares engine-stamped span times against the local clock, so remote-engine clock skew shifts when a row first appears — the same skew every displayed running duration already has.
