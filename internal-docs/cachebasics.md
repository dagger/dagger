...
it as `parent.field`. For example, a module object returned as an SDK map owns
its nested object fields, but each field needs the containing object's recipe
when exported.
