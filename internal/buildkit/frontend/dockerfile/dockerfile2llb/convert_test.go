...
import (
	...
	"github.com/dagger/dagger/buildkit/frontend/dockerfile/instructions"
	"github.com/dagger/dagger/buildkit/frontend/dockerfile/shell"
...
COPY --from=0 f2 /
	...
	df = `FROM scratch
	ADD http://github.com/dagger/dagger/buildkit/blob/master/README.md /
		`
	_, _, _, _, err = Dockerfile2LLB(appcontext.Context(), []byte(df), ConvertOpt{})
	require.NoError(t, err)

	df = `FROM scratch
	COPY http://github.com/dagger/dagger/buildkit/blob/master/README.md /
		`
	_, _, _, _, err = Dockerfile2LLB(appcontext.Context(), []byte(df), ConvertOpt{})
