...
import (
	...
	v1 "github.com/dagger/dagger/buildkit/cache/remotecache/v1"
	...
	"github.com/dagger/dagger/buildkit/frontend/dockerfile/builder"
	...
	"github.com/dagger/dagger/buildkit/session/filesync"
	"github.com/dagger/dagger/buildkit/session/upload/uploadprovider"
...
`, server.URL+"/t.tar.gz"))
	...
	// https://github.com/dagger/dagger/buildkit/issues/386
	dockerfile = []byte(fmt.Sprintf(`
FROM scratch
...
# COPY foo /test3/ # TODO -> https://github.com/dagger/dagger/buildkit/issues/5249
COPY foo /test3/foo
...
func testReproSourceDateEpoch(t *testing.T, sb integration.Sandbox) {
	...
	if sb.Snapshotter() == "native" {
		t.Skip("the digest is not reproducible with the \"native\" snapshotter because hardlinks are processed in a different way: https://github.com/dagger/dagger/buildkit/pull/3456#discussion_r1062650263")
	}
	...
	testCases := []testCase{
		{
			name: "Basic",
			dockerfile: `# The base image could not be busybox, due to https://github.com/dagger/dagger/buildkit/issues/3455
...
`,
		...
		{
			// https://github.com/dagger/dagger/buildkit/issues/4746
			name: "CopyLink",
...
`,
		...
		{
			// https://github.com/dagger/dagger/buildkit/issues/4793
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// For testing https://github.com/dagger/dagger/buildkit/issues/4746
			ensurePruneAll(t, c, sb)
			_, err = f.Solve(ctx, c, solveOpt, nil)
			require.NoError(t, err)
