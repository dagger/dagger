...
func artifactCommandFlags(cmd *cobra.Command) string {
	...
	flags.VisitAll(func(flag *pflag.Flag) {
		...
		copy := *flag
		group, usage := artifactFlagGroup(flag)
		copy.Usage = usage
...
func artifactFlagGroup(flag *pflag.Flag) (string, string) {
	group, usage := "Options", flag.Usage
	switch flag.Name {
	case "generated", "skip":
		group = "Filters"
	case "list", "all", "absolute", "format":
		group = "List options"
	case "workspace":
		group, usage = "Workspace options", "Select a workspace at `location` (local path or Git ref)"
	case "env":
		group, usage = "Workspace options", "Apply workspace environment `name`"
	case "load-module":
	...
	case "shell-on-error":
		group, usage = "Execution options", "Open a shell when a container command fails"
	case "progress":
		group, usage = "Tracing options", "Select progress `mode`"
	case "quiet":
		group, usage = "Tracing options", "Reduce progress output"
	case "silent":
		group, usage = "Tracing options", "Hide progress output"
	case "verbose":
		group, usage = "Tracing options", "Show more detail; repeat for more"
	case "debug":
		group, usage = "Tracing options", "Show engine diagnostics"
	case "web":
		group, usage = "Tracing options", "Open the trace in a browser"
	case "no-exit":
		group, usage = "Tracing options", "Keep the terminal UI open"
	}
	return group, usage
}
