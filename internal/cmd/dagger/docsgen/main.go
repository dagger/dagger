...
// (docs/current_docs/reference/cli), which runs it from the repo root so the
// output lands inside that module's tree.
package main
...
// fenced code blocks, so command-usage like `dagger module install <module-ref>` and the
