...
func (DaggerCMDSuite) TestAgentWorkspaceSwitched(ctx context.Context, t *testctx.T) {
	...
	otherRepo := core.NewQuery(dag).Host().Directory(other).AsGit().Head().AsWorkspace().WithNewFile("edit.txt", "pending\n")
	...
	s.Target().llm = start.WithWorkspace(core.NewQuery(dag).Directory().WithNewFile("loose.txt", "x\n").AsWorkspace())
	require.NoError(t, s.Target().updateChangesPreview(s.Target().llm))
	text = ansi.Strip(changes.Body(80))
