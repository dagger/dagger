...
func (cloudTraceRestoreSource) Select(context.Context) (string, error) {
	return "", errors.New("engine archive picker unavailable; resume an explicit trace with dagger agent --trace=<trace-id>")
}
...
func restoreFromTrace(ctx context.Context, handler *shellCallHandler, req traceRestore) (rerr error) {
	source := req.source
	if source == nil {
		source = traceRestoreSourceFromContext(ctx)
	}
	if source == nil {
		source = cloudTraceRestoreSource{}
	}
	if req.traceID == "" {
		selected, err := source.Select(ctx)
		if err != nil {
			return err
		}
		if selected == "" {
...
func fetchTraceIntoFrontend(ctx context.Context, traceID string, timeout time.Duration) error {
	...
	if timeout > 0 {
		client = client.WithStallTimeout(timeout)
	}
...
func executeRestorePlan(ctx context.Context, src agentRestoreSource, dst restoreTarget, req traceRestore) error {
	...
	for _, restored := range restoring {
		...
		if err != nil {
			if !req.partial {
				return fmt.Errorf("rehydrate agent %q: %w", restored.entry.Name, err)
			}
	...
	// Phase 4: point the prompt at an attached conversation. An explicit --agent
	// must resolve to a restored conversation; otherwise use the default focus.
...
const rehydrateQuery = `query Rehydrate($llm: ID!, $id: String!, $name: String!, $parentAgentID: String!, $state: AgentState!, $error: String!) {
  node(id: $llm) {
    ... on LLM {
      spawn(handle: $id, name: $name, parentHandle: $parentAgentID, state: $state, error: $error)
    }
  }
}`
...
func (r *sessionRestore) Adopt(ctx context.Context, entry dagui.AgentRestore, agentID string) error {
	conv, err := r.session.Attach(ctx, entry.ID, entry.Name, agentID)
	if err != nil {
		return err
	}
