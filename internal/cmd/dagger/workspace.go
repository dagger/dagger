...
When the workspace includes another config, reads show the merged view — the
included configuration with this workspace's own on top — while writes still
only touch this workspace's dagger.toml. Reading the include key itself reports
this workspace's own value.
