...
func Vendor(ctx context.Context, p string) error {
	...
	if err := os.MkdirAll(cuePkgDir, 0750); err != nil {
		return err
	}
	...
	if contents, err := ioutil.ReadFile(filepath.Clean(gitignorePath)); err == nil {
...
func extractModules(dest string) error {
	return fs.WalkDir(FS, ".", func(p string, entry fs.DirEntry, err error) error {
		...
		if err := os.MkdirAll(filepath.Dir(overlayPath), 0750); err != nil {
			return err
		}
		...
		//nolint
		return os.WriteFile(overlayPath, contents, 0700) //#nosec
	})
}
...
func CueModInit(ctx context.Context, parentDir, module string) error {
	...
	if err := os.Mkdir(path.Join(modDir, "usr"), 0750); err != nil {
		if !errors.Is(err, os.ErrExist) {
			return err
		}
	}
	if err := os.Mkdir(path.Join(modDir, "pkg"), 0750); err != nil {
		if !errors.Is(err, os.ErrExist) {
			return err
		}
	}
