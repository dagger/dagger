...
public class ConsumerExample
{
    ...
    public async Task<string> DemoConstructor()
    {
        ...
        var result1 = await Dag.ConstructorExample().Greet("World");

        // Call with custom constructor values
        var result2 = await Dag.ConstructorExample("Hi", 9000, false).Greet("Alice");
    ...
    {
        var dirId = await Dag.DefaultsExample().CreateFile().Id();
    ...
    public async Task<string> DemoAttributes()
    {
        ...
        var result = await Dag.AttributesExample().AnalyzeSource(dir);
        return result;
    }
    ...
    public async Task<string> DemoMultiFile()
    {
        ...
        var formatted = await Dag.MultiFileExample().FormatUser(user);
        return formatted;
    }
    ...
    public async Task<string> DemoInterface()
    {
        ...
        string result = await interfaceModule.ProcessText(
            implementationModule.AsInterfaceExampleProcessor(),
            text
        );
