...
using VerifyCS = CSharpCodeFixVerifier<DaggerObjectAnalyzer, AddFunctionAttributeCodeFixProvider>;
...
public class AddFunctionAttributeCodeFixTests
{
    ...
    public async Task AddFunctionAttribute_PreservesExistingAttributes()
    {
        ...
        {
            VerifyCS.Diagnostic(ObjectClassMissingXmlDocumentation).WithSpan(4, 14, 4, 22).WithArguments("MyModule"),
            VerifyCS.Diagnostic(FunctionMissingXmlDocumentation).WithSpan(8, 19, 8, 24).WithArguments("Hello"),
        };
    ...
    public async Task AddFunctionAttribute_WithParameters()
    {
        ...
        var expected = new[]
        {
            VerifyCS.Diagnostic(PublicMethodInObjectMissingFunctionAttribute).WithLocation(0).WithArguments("Greet"),
            VerifyCS.Diagnostic(ObjectClassMissingXmlDocumentation).WithSpan(4, 14, 4, 22).WithArguments("MyModule"),
        };
        ...
        {
            VerifyCS.Diagnostic(ObjectClassMissingXmlDocumentation).WithSpan(4, 14, 4, 22).WithArguments("MyModule"),
            VerifyCS.Diagnostic(FunctionMissingXmlDocumentation).WithSpan(7, 19, 7, 24).WithArguments("Greet"),
            VerifyCS.Diagnostic(ParameterMissingXmlDocumentation).WithSpan(7, 32, 7, 36).WithArguments("name"),
        };
