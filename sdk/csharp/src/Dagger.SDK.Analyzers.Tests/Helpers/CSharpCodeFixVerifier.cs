    ...
    where TCodeFix : CodeFixProvider, new()
{
    ...
    public static async Task VerifyCodeFixAsync(string source, string fixedSource, int? codeFixIndex = null, Action<CSharpCodeFixTest<TAnalyzer, TCodeFix, DefaultVerifier>>? configureTest = null)
        => await VerifyCodeFixAsync(source, DiagnosticResult.EmptyDiagnosticResults, DiagnosticResult.EmptyDiagnosticResults, fixedSource, codeFixIndex, configureTest: configureTest);

    /// <inheritdoc cref="CodeFixVerifier{TAnalyzer, TCodeFix, TTest, TVerifier}.VerifyCodeFixAsync(string, DiagnosticResult, string)"/>
    public static async Task VerifyCodeFixAsync(string source, DiagnosticResult expected, string fixedSource, int? codeFixIndex = null, Action<CSharpCodeFixTest<TAnalyzer, TCodeFix, DefaultVerifier>>? configureTest = null)
        => await VerifyCodeFixAsync(source, [expected], DiagnosticResult.EmptyDiagnosticResults, fixedSource, codeFixIndex, configureTest: configureTest);
    ...
    /// </summary>
    public static async Task VerifyCodeFixAsync(string source, DiagnosticResult[] expected, DiagnosticResult[] fixedExpected, string fixedSource, int? codeFixIndex = null, Action<CSharpCodeFixTest<TAnalyzer, TCodeFix, DefaultVerifier>>? configureTest = null)
    {
        var test = new Test
        {
            TestCode = source,
    ...
    public static async Task VerifyCodeFixAsync(
        ...
        => await VerifyCodeFixAsync(source, additionalFiles, expected, DiagnosticResult.EmptyDiagnosticResults, fixedSource, codeFixIndex, assemblyName, configureTest);
        ...
        Action<CSharpCodeFixTest<TAnalyzer, TCodeFix, DefaultVerifier>>? configureTest = null)
    {
        ...
        {
            if (path.EndsWith(".editorconfig", StringComparison.OrdinalIgnoreCase)
