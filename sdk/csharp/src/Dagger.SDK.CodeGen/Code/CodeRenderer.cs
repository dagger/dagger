...
public class CodeRenderer : ICodeRenderer
{
    ...
    public string RenderObject(Type type)
    {
        ...
        var interfaceConversionMethods = type.Interfaces?.Select(iface =>
        {
            ...
            var graphqlFieldName = char.ToLowerInvariant(methodName[0]) + methodName.Substring(1);
    ...
    private static string RenderDocComment(Field field)
    {
        ...
        if (isAsync)
        {
            ...
            builder.AppendLine("/// A cancellation token that can be used to cancel the operation.");
