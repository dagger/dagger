...
";
    ...
    public async Task TestDirectoryEntries()
    {
        ...
        var entries = await dir.EntriesAsync();
    ...
    public async Task TestContainerEnvVariables()
    {
        ...
        var envs = await container.EnvVariablesAsync();

        Assert.IsTrue(envs.Length >= 2); // At least our 2 vars, plus PATH from base image

        // Find our custom env vars
        var fooVar = envs.FirstOrDefault(e => e.NameAsync().Result == "FOO");
        var bazVar = envs.FirstOrDefault(e => e.NameAsync().Result == "BAZ");

        Assert.IsNotNull(fooVar);
        Assert.AreEqual("BAR", await fooVar.ValueAsync());

        Assert.IsNotNull(bazVar);
        Assert.AreEqual("QUX", await bazVar.ValueAsync());
    }
