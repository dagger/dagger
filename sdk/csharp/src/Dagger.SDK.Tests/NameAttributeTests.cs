...
public class NameAttributeTests
{
    ...
    /// <summary>
    /// Verifies that [Name] can be applied to properties.
    /// </summary>
    [TestMethod]
    public void NameAttribute_AppliedToProperty_HasCorrectAttributeUsage()
    {
        var attributeUsage = typeof(NameAttribute)
            .GetCustomAttributes(typeof(AttributeUsageAttribute), false)
            .Cast<AttributeUsageAttribute>()
    ...
    /// <summary>
    /// Verifies that [Name] can be applied to methods.
    /// </summary>
    [TestMethod]
    public void NameAttribute_AppliedToMethod_HasCorrectAttributeUsage()
    {
        var attributeUsage = typeof(NameAttribute)
            .GetCustomAttributes(typeof(AttributeUsageAttribute), false)
            .Cast<AttributeUsageAttribute>()
    ...
    /// <summary>
    /// Verifies that Name attribute can be applied to properties (fields).
    /// </summary>
    [TestMethod]
    public void Integration_Property_CanHaveNameAttribute()
    {
        ...
        Assert.IsNotNull(nameAttr);
        Assert.AreEqual("customField", nameAttr.ApiName);
    }

    /// <summary>
    /// Verifies that Name attribute can be applied to methods (functions).
    /// </summary>
    [TestMethod]
    public void Integration_Method_CanHaveNameAttribute()
    {
        var method = typeof(SampleModuleWithNames).GetMethod("Import_");
        var nameAttr = method!.GetCustomAttribute<NameAttribute>();

        Assert.IsNotNull(nameAttr);
        Assert.AreEqual("import", nameAttr.ApiName);
    }
