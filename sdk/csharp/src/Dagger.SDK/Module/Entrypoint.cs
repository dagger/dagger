...
public static partial class Entrypoint
{
                ...
                invocationResult = await DaggerTracer.StartActiveSpanAsync(
                    ...
                    async (activity) =>
                    {
                        ...
                        if (activity != null)
                        {
                            ...
                            {
                                var argName = await arg.NameAsync();
                                var argValue = await arg.ValueAsync();
    ...
    private static async Task<object?> NormalizeResultAsync(object? value)
    {
        ...
        if (value is DaggerObject daggerObject)
        {
            ...
            {
                idAsyncMethod =
                    objectType.GetMethod("IdAsync", new[] { typeof(CancellationToken) })
                    ?? throw new InvalidOperationException(
                        $"Type '{objectType.Name}' does not have IdAsync method."
                    );
            ...
            var idTask = (Task)
                idAsyncMethod.Invoke(daggerObject, new object[] { CancellationToken.None })!;
