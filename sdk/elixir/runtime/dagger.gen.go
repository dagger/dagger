...
import (
	...
	"elixir-sdk/internal/querybuilder"
	"elixir-sdk/internal/telemetry"
)
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "ElixirSdk":
		...
		case "Codegen":
			...
			return (*ElixirSdk).Codegen(&parent, ctx, modSource, introspectionJson)
		case "Common":
			var parent ElixirSdk
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		case "WithNewElixirPackage":
			...
			var modName string
			if inputArgs["modName"] != nil {
				err = json.Unmarshal([]byte(inputArgs["modName"]), &modName)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg modName", err))
				}
			}
			return (*ElixirSdk).WithNewElixirPackage(&parent, ctx, modName), nil
		case "WithSDK":
			var parent ElixirSdk
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
