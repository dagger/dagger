...
// ExecError is an API error from an exec operation. It lives in
// dagger.io/dagger, not here, because classifying transport-level errors
// doesn't need any generated type, and dagger.io/dagger can't import this
// package (this package imports dagger.io/dagger). Aliased here so existing
// code can keep writing core.ExecError.
...
func Ref[T Loadable[T]](root *Query, id ID) T {
	var zero T
	return zero.WithGraphQLQuery(selectNode(root.query, id, zero.XXX_GraphQLType()))
}
...
func Load[T Loadable[T]](ctx context.Context, root *Query, id ID) (T, error) {
	var zero T
	...
	q := selectNode(root.query, id, expectedType)
	var typeName string
...
// Creates a file from arbitrary binary contents.
func Blob(name string, contents Bytes, opts ...BlobOpts) *File {
	root := initRoot()
	return root.Blob(name, contents, opts...)
}

// Constructs a cache volume for a given cache key.
func NewCacheVolume(key string, opts ...CacheVolumeOpts) *CacheVolume {
	root := initRoot()
	return root.CacheVolume(key, opts...)
}
...
func NewContainer(opts ...ContainerOpts) *Container {
	root := initRoot()
	return root.Container(opts...)
}

// The FunctionCall context that the SDK caller is currently executing in.
//
// If the caller is not currently executing in a function, this will return an error.
func CurrentFunctionCall() *FunctionCall {
	root := initRoot()
	return root.CurrentFunctionCall()
}

// The module currently being served in the session, if any.
func NewCurrentModule() *CurrentModule {
	root := initRoot()
	return root.CurrentModule()
}
...
func CurrentTimestamp(ctx context.Context) (string, error) {
	root := initRoot()
	return root.CurrentTimestamp(ctx)
}

// The TypeDef representations of the objects currently being served in the session.
func CurrentTypeDefs(ctx context.Context, opts ...CurrentTypeDefsOpts) ([]TypeDef, error) {
	root := initRoot()
	return root.CurrentTypeDefs(ctx, opts...)
}
...
// The default platform of the engine.
func DefaultPlatform(ctx context.Context) (Platform, error) {
	root := initRoot()
	return root.DefaultPlatform(ctx)
}
...
// Constructs an engine-managed volume backed by operator-provided storage beneath the configured engine state root.
func EngineVolume(name string, opts ...EngineVolumeOpts) *Volume {
	root := initRoot()
	return root.EngineVolume(name, opts...)
}

// Initialize an environment file
func NewEnvFile(opts ...EnvFileOpts) *EnvFile {
	root := initRoot()
	return root.EnvFile(opts...)
}
...
// Creates a file with the specified contents.
func NewFile(name string, contents string, opts ...FileOpts) *File {
	root := initRoot()
...
func NewFunction(name string, returnType *TypeDef) *Function {
	root := initRoot()
	return root.Function(name, returnType)
}

// Create a code generation result, given a directory containing the generated code.
func NewGeneratedCode(code *Directory) *GeneratedCode {
	root := initRoot()
	return root.GeneratedCode(code)
}

// Queries a Git repository.
func Git(url string, opts ...GitOpts) *GitRepository {
	root := initRoot()
	return root.Git(url, opts...)
}
...
// Returns a file containing an http remote url content.
func HTTP(url string, opts ...HTTPOpts) *File {
	root := initRoot()
	return root.HTTP(url, opts...)
}

// A unique identifier for this Query.
func NewID(ctx context.Context) (ID, error) {
	root := initRoot()
	return root.ID(ctx)
}
...
// Load any object by its ID.
func NewNode(ctx context.Context, id ID) (Node, error) {
	root := initRoot()
	return root.Node(ctx, id)
}

// Load a GraphQL introspection schema for merging.
func NewSchema(json JSON) *Schema {
	root := initRoot()
	return root.Schema(json)
}

// Creates a new secret.
func NewSecret(uri string, opts ...SecretOpts) *Secret {
	root := initRoot()
	return root.Secret(uri, opts...)
}

// Load the module at the given address and serve its API in the current session.
//
// A local address resolves against the caller's workspace, so a generated client can serve the module it is bound to without reaching for the workspace itself.
func ServeModule(ctx context.Context, address string, opts ...ServeModuleOpts) error {
	root := initRoot()
	return root.ServeModule(ctx, address, opts...)
}
...
// Creates source map metadata.
func NewSourceMap(filename string, line int, column int) *SourceMap {
	root := initRoot()
	return root.SourceMap(filename, line, column)
}
