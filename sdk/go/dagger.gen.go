...
// JSONValueBytesOpts contains options for JSONValue.Bytes
type JSONValueBytesOpts struct {
	// Pretty-print
	Pretty bool
...
func (r *JSONValue) Bytes(ctx context.Context, opts ...JSONValueBytesOpts) (JSON, error) {
	if r.bytes != nil {
		return *r.bytes, nil
	}
	q := r.query.Select("bytes")
	for i := len(opts) - 1; i >= 0; i-- {
...
func (r *JSONValue) WithBytes(bytes JSON) *JSONValue {
	q := r.query.Select("withBytes")
	q = q.Arg("bytes", bytes)
