...
func (r *Changeset) WithChangeset(changes *Changeset, opts ...ChangesetWithChangesetOpts) *Changeset {
	...
	for i := len(opts) - 1; i >= 0; i-- {
		// `continueOnConflicts` optional argument
		if !querybuilder.IsZeroValue(opts[i].ContinueOnConflicts) {
			q = q.Arg("continueOnConflicts", opts[i].ContinueOnConflicts)
		}
	}
