...
func (r *Environment) WithShell_(in any) *Environment {
	...
	if fn.returns[0].typ == reflect.TypeOf((*EnvironmentShell)(nil)) {
		// this returns a shell, so call it now to register it
		...
		var ok bool
		shell, ok = ret.(*EnvironmentShell)
		if !ok {
			writeErrorf(fmt.Errorf("expected *EnvironmentShell, got %T", ret))
		}
		/* TODO:?
		// force eval
		_, err = shell.ID(context.Background())
		if err != nil {
			writeErrorf(err)
		}
		*/
