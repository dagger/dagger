...
class ObjectVisitor extends AbstractVisitor {
    ...
    TypeSpec generateType(Type type) {
        ...
        if ("Query".equals(type.getName())) {
            MethodSpec constructor = MethodSpec.constructorBuilder()
                    .addParameter(ClassName.bestGuess("io.dagger.client.engineconn.Connection"), "connection")
    ...
    private void buildFieldMethod(TypeSpec.Builder classBuilder, Field field, boolean withOptionalArgs) {
        MethodSpec.Builder fieldMethodBuilder = MethodSpec.methodBuilder(Helpers.formatName(field)).addModifiers(Modifier.PUBLIC);
        ...
        if (withOptionalArgs && field.hasOptionalArgs()) {
            fieldMethodBuilder.addParameter(ParameterSpec.builder(ClassName.bestGuess(capitalize(Helpers.formatName(field)) + "Arguments"), "optArgs")
