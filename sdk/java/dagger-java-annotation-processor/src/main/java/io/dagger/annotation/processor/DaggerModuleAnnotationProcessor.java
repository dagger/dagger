...
public class DaggerModuleAnnotationProcessor extends AbstractProcessor {
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    if (!annotatedObjects.isEmpty()) {
      try {
        var rm = MethodSpec.methodBuilder("register")
        ...
        for (var objectInfo : annotatedObjects) {
          ...
          if (isNotBlank(objectInfo.description())) {
            rm.addCode(", new $T.WithObjectArguments().withDescription($S)", TypeDef.class, objectInfo.description());
          }
        ...
        var im = MethodSpec.methodBuilder("invoke")
            .addModifiers(Modifier.PRIVATE)
            .returns(JSON.class)
            ...
            .addParameter(ParameterizedTypeName.get(Map.class, String.class, JSON.class), "inputArgs")
        ...
        for (var objectInfo : annotatedObjects) {
          ...
          im
              .addStatement("$T clazz = Class.forName($S)", Class.class, objectInfo.qualifiedName())
              .addStatement("var obj = $T.fromJSON(dag, parentJson, clazz)", Convert.class)
              .addStatement("clazz.getMethod(\"setClient\", $T.class).invoke(obj, dag)", Client.class);
          var firstFn = true;
          for (var fnInfo : objectInfo.functions()) {
            ...
            var fnBlock = CodeBlock.builder()
                .add("$T fn = clazz.getMethod($S", Method.class, fnInfo.name());
            var invokeBlock = CodeBlock.builder()
            ...
            for (var parameterInfo : fnInfo.parameters()) {
              ...
              im
                  .addStatement("$T $L = ($T) $T.fromJSON(dag, inputArgs.get($S), Class.forName($S))",
                      paramClazz, parameterInfo.name(), paramClazz, Convert.class, parameterInfo.name(), parameterInfo.type());
            }
            fnBlock.add(")");
            invokeBlock.add(")");
            im
                .addStatement(fnBlock.build())
                .addStatement(invokeBlock.build())
            ;
        ...
        im
            .endControlFlow() // try json
            .addStatement("return null");

        var f = JavaFile.builder("io.dagger.gen.entrypoint",
                TypeSpec.classBuilder("Entrypoint")
                    .addField(FieldSpec.builder(Client.class, "dag", Modifier.PRIVATE, Modifier.FINAL)
                        .build())
                    ...
                    .addMethod(MethodSpec.methodBuilder("dispatch")
                        ...
                        .returns(void.class)
                        .addException(Exception.class)
                        .addStatement("$T fnCall = dag.currentFunctionCall()", FunctionCall.class)
                        ...
                        .addStatement("$T fnArgs = fnCall.inputArgs()", ParameterizedTypeName.get(List.class, FunctionCallArgValue.class))
                        .addStatement("$T<$T, $T> inputArgs = new $T<>()", Map.class, String.class, JSON.class, HashMap.class)
                        ...
                        .addStatement("result = invoke(parentJson, parentName, fnName, inputArgs)")
                        .endControlFlow()
                        .addStatement("fnCall.returnValue(result)")
                        .nextControlFlow("catch ($T e)", InvocationTargetException.class)
                        .addStatement("fnCall.returnError(dag.error(e.getTargetException().getMessage()))")
