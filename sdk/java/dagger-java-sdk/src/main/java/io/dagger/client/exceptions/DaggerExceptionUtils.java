...
public class DaggerExceptionUtils {
    ...
    public static String getCmd(GraphQLError error) {
        ...
        if (cmdList != null && cmdList instanceof JsonArray array) {
            cmd = array.stream().map(JsonValue::toString).collect(Collectors.joining(" "))
                    .replace("\"", "");
        }
        return cmd;
    }
    ...
    public static String toEnhancedMessage(GraphQLError... errors) {
        return Arrays.stream(errors).map(e -> String.format(ENHANCED_MESSAGE, e.getMessage(),
                getPath(e), getType(e), getExitCode(e), getCmd(e)))
                .collect(Collectors.joining("\n"));
    }

    public static String toFullMessage(GraphQLError... errors) {
        return Arrays.stream(errors)
                .map(e -> String.format(FULL_MESSAGE, e.getMessage(), getPath(e), getType(e),
                        getExitCode(e), getCmd(e), getExtensionValueByKey(e, STDERR_KEY)))
                .collect(Collectors.joining("\n"));
    }
}
