...
class JsonValue extends Client\AbstractObject implements Client\IdAble
{
     ...
     */
    public function bytes(?bool $pretty = null, ?int $indent = 1): Json
    {
        $leafQueryBuilder = new \Dagger\Client\QueryBuilder('bytes');
        if (null !== $pretty) {
        ...
        return new \Dagger\Json((string)$this->queryLeaf($leafQueryBuilder, 'bytes'));
    }
     ...
     */
    public function withBytes(Json $bytes): JsonValue
    {
        $innerQueryBuilder = new \Dagger\Client\QueryBuilder('withBytes');
        $innerQueryBuilder->setArgument('bytes', $bytes);
        return new \Dagger\JsonValue($this->client, $this->queryBuilderChain->chain($innerQueryBuilder));
    }
