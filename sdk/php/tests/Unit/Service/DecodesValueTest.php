...
class DecodesValueTest extends TestCase
{
    ...
    public static function provideEnums(): Generator
    {
        ...
        foreach (Fixture\StringBackedEnum::cases() as $case) {
            yield sprintf('nonsense word: %s', $case->name) => [
                $case,
                sprintf('"%s"', $case->name),
                $stringBackedEnum,
            ];
        ...
        yield 'pure enum' => [
            Fixture\PureEnum::High,
            '"High"',
            new Type(Fixture\PureEnum::class),
        ];

        yield '[StringBackedEnum!]!' => [
            [Fixture\StringBackedEnum::Foo, Fixture\StringBackedEnum::Baz],
            '["Foo","Baz"]',
            new ListOfType($stringBackedEnum, false),
        ];
    }
}
