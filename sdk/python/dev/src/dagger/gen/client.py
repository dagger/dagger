...
class DockerdID(Scalar):
    """The `DockerdID` scalar type represents an identifier for an object
    of type Dockerd."""
...
class WolfiID(Scalar):
    """The `WolfiID` scalar type represents an identifier for an object of
    type Wolfi."""
...
class Dockerd(Type):
    ...
    def attach(self, container: Container, *, docker_version: str | None = "24.0",) -> Container:
        """
        Attach a dockerd service to a container
        """
        _args = [
            Arg("container", container),
            Arg("dockerVersion", docker_version, "24.0"),
        ]
        _ctx = self._select("attach", _args)
        return Container(_ctx)

    async def id(self) -> DockerdID:
        ...
        _args: list[Arg] = []
        _ctx = self._select("id", _args)
        return await _ctx.execute(DockerdID)

    def service(self, *, docker_version: str | None = "24.0",) -> "Service":
        """
...
class GitRef(Type):
    ...
    def tree(self, *, discard_git_dir: bool | None = False,) -> Directory:
...
class Client(Root):
    ...
    def dockerd(self) -> Dockerd:
        _args: list[Arg] = []
        _ctx = self._select("dockerd", _args)
        return Dockerd(_ctx)
    ...
    def load_dockerd_from_id(self, id: DockerdID) -> Dockerd:
        """Load a Dockerd from its ID."""
        _args = [
            Arg("id", id),
        ]
        _ctx = self._select("loadDockerdFromID", _args)
        return Dockerd(_ctx)
    ...
    def load_secret_from_name(self, name: str, *, accessor: str | None = None,) -> "Secret":
    ...
    def load_wolfi_from_id(self, id: WolfiID) -> "Wolfi":
        """Load a Wolfi from its ID."""
        _args = [
            Arg("id", id),
        ]
        _ctx = self._select("loadWolfiFromID", _args)
        return Wolfi(_ctx)
...
class Wolfi(Type):
    ...
    def container(self, *, packages: list[str] | None = None, arch: str | None = None, overlays: list[Container] | None = None,) -> Container:
        """
        ...
        _args = [
            ...
            Arg("arch", arch, None),
            Arg("overlays", overlays, None),
        ]
        _ctx = self._select("container", _args)
        return Container(_ctx)

    async def id(self) -> WolfiID:
        ...
        -------
        WolfiID
            The `WolfiID` scalar type represents an identifier for an object
            of type Wolfi.
        ...
        _args: list[Arg] = []
        _ctx = self._select("id", _args)
        return await _ctx.execute(WolfiID)
