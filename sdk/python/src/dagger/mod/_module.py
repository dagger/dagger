...
def _import_static_entrypoint(import_pkg: str) -> Any | None:
    """Import the committed ``<import_pkg>/_dagger_main.py`` if present.

    Returns the imported module (exposing ``typedefs()``), or ``None`` when it
    hasn't been generated yet — e.g. the first ``generate`` of a module, before
    the static entrypoint is committed, where the def phase falls back to live
    introspection.
    """
    import importlib
    import importlib.util

    name = f"{import_pkg}._dagger_main"
    try:
...
class Module:
    ...
    async def _typedefs(self) -> dagger.ModuleID:
        """Return the module's TypeDefs for the def phase.

        Prefers the committed static entrypoint (a pure replay of the values
        resolved at generate time); otherwise introspects the live module that
        was just imported. Either path executes real code — never static source
        analysis — so runtime-resolved defaults (e.g. ``logging.INFO`` -> 20)
        ...
        entrypoint = _import_static_entrypoint(IMPORT_PKG)
        try:
            if entrypoint is not None:
                return await entrypoint.typedefs()
            module_types = live_to_introspection_json(
                self,
