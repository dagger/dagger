...
def _doc_meta() -> st.SearchStrategy[str]:
    return st.text(
        min_size=1, max_size=20, alphabet="abcdefghijk "
...
def param_name_strategy() -> st.SearchStrategy[str]:
    return (
        st.text(min_size=1, max_size=6, alphabet=_PARAM_NAME_ALPHABET)
        .filter(lambda s: not keyword.iskeyword(s) and s not in _RESERVED)
    )
...
def _function_name_strategy() -> st.SearchStrategy[str]:
    return (
        st.text(min_size=1, max_size=8, alphabet="abcdefghijk_")
        .filter(
