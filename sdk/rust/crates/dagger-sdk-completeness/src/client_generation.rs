...
pub enum ClientImplementationSubject {
    ...
    DiagnosticSecurity,
    /// Engine-free checkpoint and verification boundary.
    EvidenceBoundary,
}
...
pub enum ClientEvidenceDomain {
    ...
    /// Completed-engine external-consumer verification.
    ExternalConsumer,
...
/// Admitted engine-free implementation closure.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ClientGenerationClosureEvidence {
    ...
    pub status_changes: BTreeMap<CapabilityId, Status>,
    /// Engine-dependent blockers intentionally retained after local closure.
...
pub fn client_implementation_closure_claims(
    ...
    for mapping in scope.mappings.values() {
        for domain in &mapping.evidence_domains {
            if !matches!(
                domain,
                ClientEvidenceDomain::ExternalConsumer | ClientEvidenceDomain::EngineHook
            ) {
                claims
                    .entry(*domain)
                    .or_default()
...
pub fn admit_client_generation_closure(
    ...
    if observation.claims != expected_claims
        || observation
            .claims
            .contains_key(&ClientEvidenceDomain::ExternalConsumer)
        || observation
            .claims
...
/// Derives the honest standalone-client report from admitted local closure.
pub fn derive_client_generation_report(
    ...
    let expected_local = expected_client_status_changes(scope, |mapping| {
        !mapping
            .evidence_domains
            .contains(&ClientEvidenceDomain::ExternalConsumer)
            && !mapping
                .evidence_domains
                .contains(&ClientEvidenceDomain::EngineHook)
    });
    let mut status_changes = BTreeMap::new();
    let implementation_closure = if let Some(closure) = closure {
        if closure.target_digest != *scope.target_digest()
            ...
            || closure.remaining_blockers != remaining_client_claims(scope, &closure.status_changes)
        {
            return Err(rejected());
        }
...
const fn client_evidence_domain_slug(domain: ClientEvidenceDomain) -> &'static str {
    match domain {
        ...
        ClientEvidenceDomain::ExternalConsumer => "external-consumer",
...
fn initialization_mapping(target_digest: &TargetDigest) -> ClientGenerationMapping {
    ClientGenerationMapping {
        ...
        rationale: text(
            "The definitive lifecycle requires completed-engine consumer verification.",
        ),
        evidence_domains: BTreeSet::from([ClientEvidenceDomain::ExternalConsumer]),
...
const POLICY_SPECS: &[PolicySpec] = &[
    ...
    policy(
        "policy/rust-policy/client-completed-engine-external-consumer",
        "10.12-10.19",
        ClientAuthority::Engine,
        ClientImplementationSubject::EvidenceBoundary,
        ClientEvidenceDomain::ExternalConsumer,
        ClientReportSection::Initialization,
        "One external Rust consumer verifies the packaged SDK against the completed engine.",
    ),
];
