...
impl Query {
    ...
    pub fn load_toml_value_from_id(&self, id: impl IntoID<TomlValueId>) -> TomlValue {
        let mut query = self.selection.select("loadTOMLValueFromID");
        query = query.arg_lazy(
            "id",
            Box::new(move || {
                let id = id.clone();
