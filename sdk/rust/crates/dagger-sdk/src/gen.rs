...
#[derive(Builder, Debug, PartialEq)]
pub struct JsonValueBytesOpts {
    /// Indent each line by the given number of whitespaces
    #[builder(setter(into, strip_option), default)]
    pub indent: Option<isize>,
    /// Pretty-print
    #[builder(setter(into, strip_option), default)]
...
impl JsonValue {
    ...
    ///
    /// * `opt` - optional argument, see inner type for documentation, use <func>_opts to use
    pub async fn bytes(&self) -> Result<Json, DaggerError> {
        let query = self.selection.select("bytes");
        query.execute(self.graphql_client.clone()).await
    }
    /// Return the value encoded as json
    ///
    ...
    ///
    /// * `opt` - optional argument, see inner type for documentation, use <func>_opts to use
    pub async fn bytes_opts(&self, opts: JsonValueBytesOpts) -> Result<Json, DaggerError> {
        let mut query = self.selection.select("bytes");
    ...
    pub fn with_bytes(&self, bytes: Json) -> JsonValue {
        let mut query = self.selection.select("withBytes");
        query = query.arg("bytes", bytes);
        JsonValue {
            proc: self.proc.clone(),
            selection: query,
            graphql_client: self.graphql_client.clone(),
