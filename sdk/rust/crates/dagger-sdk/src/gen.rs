...
pub struct ChangesetWithChangesetOpts {
    /// Continue to merge changes even if conflict exists, like the same file modified in both changesets
    #[builder(setter(into, strip_option), default)]
    pub continue_on_conflicts: Option<bool>,
}
impl Changeset {
    ...
    pub fn with_changeset_opts(
        ...
        if let Some(continue_on_conflicts) = opts.continue_on_conflicts {
            query = query.arg("continueOnConflicts", continue_on_conflicts);
        }
        Changeset {
            proc: self.proc.clone(),
            selection: query,
...
pub struct GeneratorGroupChangesOpts {
    /// Continue to merge changes even if conflict exists, like the same file modified by multiple generators
    #[builder(setter(into, strip_option), default)]
    pub continue_on_conflicts: Option<bool>,
}
#[derive(Builder, Debug, PartialEq)]
