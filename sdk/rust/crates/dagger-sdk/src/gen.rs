...
impl Agent {
    ...
    pub async fn instance_id(&self) -> Result<String, DaggerError> {
        let query = self.selection.select("instanceID");
        query.execute(self.graphql_client.clone()).await
    }
    ...
    /// # Arguments
    ///
    /// * `id` - The message ID, as generated by the send that enqueued the message.
    pub fn message(&self, id: impl Into<String>) -> AgentMessage {
    ...
    ///
    /// * `opt` - optional argument, see inner type for documentation, use <func>_opts to use
    pub async fn wait_for(&self) -> Result<Agent, DaggerError> {
        let query = self.selection.select("waitFor");
        let id: Id = query.execute(self.graphql_client.clone()).await?;
        Ok(Agent {
            proc: self.proc.clone(),
            selection: query
    ...
    ///
    /// * `opt` - optional argument, see inner type for documentation, use <func>_opts to use
    pub async fn wait_for_opts(&self, opts: AgentWaitForOpts) -> Result<Agent, DaggerError> {
        let mut query = self.selection.select("waitFor");
        if let Some(state) = opts.state {
            query = query.arg("state", state);
        }
        let id: Id = query.execute(self.graphql_client.clone()).await?;
    ...
    pub async fn wait_settled(&self) -> Result<Agent, DaggerError> {
        let query = self.selection.select("waitSettled");
        let id: Id = query.execute(self.graphql_client.clone()).await?;
        Ok(Agent {
            proc: self.proc.clone(),
            selection: query
...
impl AgentMessage {
    ...
    pub async fn r#await(&self) -> Result<String, DaggerError> {
        let query = self.selection.select("await");
        query.execute(self.graphql_client.clone()).await
    }
...
impl Llm {
    ...
    /// # Arguments
    ///
    /// * `id` - The agent instance ID, as minted by the spawn that created the agent.
    /// * `name` - The agent's display name, as recorded by the spawn.
    pub fn agent(&self, id: impl Into<String>, name: impl Into<String>) -> Agent {
...
impl LlmMessageOrigin {
    ...
    pub async fn agent_id(&self) -> Result<String, DaggerError> {
        let query = self.selection.select("agentId");
        query.execute(self.graphql_client.clone()).await
    }
