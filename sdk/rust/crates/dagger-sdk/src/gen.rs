...
impl Check {
    ...
    pub async fn context(&self) -> Result<String, DaggerError> {
        let query = self.selection.select("context");
        query.execute(self.graphql_client.clone()).await
    }
    ...
    pub async fn full_name(&self) -> Result<String, DaggerError> {
        let query = self.selection.select("fullName");
        query.execute(self.graphql_client.clone()).await
    }
    pub async fn function_name(&self) -> Result<String, DaggerError> {
        let query = self.selection.select("functionName");
        query.execute(self.graphql_client.clone()).await
    }
...
impl Query {
    ...
    ///
    /// * `opt` - optional argument, see inner type for documentation, use <func>_opts to use
    pub fn checks_opts<'a>(&self, opts: QueryChecksOpts<'a>) -> CheckGroup {
