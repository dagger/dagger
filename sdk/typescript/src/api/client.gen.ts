...
export class WorkspaceModule extends BaseClient {
  ...
  settings = async (): Promise<WorkspaceModuleSetting[]> => {
    ...
    return response.map((r) =>
      new Client(ctx.copy()).loadWorkspaceModuleSettingFromID(r.id),
    )
  }
...
export class WorkspaceModuleSetting extends BaseClient {
  private readonly _id?: WorkspaceModuleSettingID = undefined
  private readonly _description?: string = undefined
   ...
   */
  id = async (): Promise<WorkspaceModuleSettingID> => {
    if (this._id) {
      return this._id
    }
    ...
    const response: Awaited<WorkspaceModuleSettingID> = await ctx.execute()
