...
export type DirectoryAsModuleOpts = {
   ...
   *
   * This is needed when the module code is in a subdirectory but requires parent directories to be loaded in order to execute. For example, the module source code may need a go.mod, project.toml, packageon, etc. file from a parent directory.
   *
...
export class ModuleSource extends BaseClient {
  ...
  /**
   * The path relative to context of the root of the module source, which contains daggeron. It also contains the module implementation source code, but that may or may not being a subdir of this root.
   */
