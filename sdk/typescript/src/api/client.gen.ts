...
export class Changeset extends BaseClient {
   ...
   * @param opts.continueOnConflicts Continue to merge changes even if conflict exists, like the same file modified in both changesets
   */
  withChangeset = (
    changes: Changeset,
    opts?: ChangesetWithChangesetOpts,
  ): Changeset => {
    const ctx = this._ctx.select("withChangeset", { changes, ...opts })
    return new Changeset(ctx)
  }
...
export class GeneratorGroup extends BaseClient {
   ...
   *
   * Set 'continueOnConflicts' flag to force to merge the changes in a 'last write wins' strategy.
   * @param opts.continueOnConflicts Continue to merge changes even if conflict exists, like the same file modified by multiple generators
   */
  ...
  isEmpty = async (opts?: GeneratorGroupIsEmptyOpts): Promise<boolean> => {
    ...
    const ctx = this._ctx.select("isEmpty", { ...opts })
