...
export class Workspace extends BaseClient {
   ...
   */
  envCreate = async (name: string): Promise<string> => {
    if (this._envCreate) {
      return this._envCreate
    }

    const ctx = this._ctx.select("envCreate", { name })
   ...
   * @param name Environment name.
   */
  envRemove = async (name: string): Promise<string> => {
    if (this._envRemove) {
      return this._envRemove
    }

    const ctx = this._ctx.select("envRemove", { name })
