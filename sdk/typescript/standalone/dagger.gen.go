...
func (r Ts) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *Ts) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Ts":
		...
		case "ModuleRuntime":
			...
			return (*Ts).ModuleRuntime(&parent, modSource, introspectionJson), nil
		case "Codegen":
			var parent Ts
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Ts).Codegen(&parent, modSource, introspectionJson), nil
		case "RequiredPaths":
			var parent Ts
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Ts).RequiredPaths(&parent), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("This is a thin wrapper around the TypeScript SDK to workaround https://github.com/dagger/dagger/issues/7583 until the next engine release.\n").
			WithObject(
				dag.TypeDef().WithObject("Ts").
					WithFunction(
						dag.Function("ModuleRuntime",
							dag.TypeDef().WithObject("Container")).
							WithArg("modSource", dag.TypeDef().WithObject("ModuleSource")).
							WithArg("introspectionJSON", dag.TypeDef().WithKind(StringKind))).
					WithFunction(
					...
					WithFunction(
						dag.Function("RequiredPaths",
							dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind))))), nil
	default:
