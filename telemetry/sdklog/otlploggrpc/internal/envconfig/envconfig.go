...
func WithDuration(n string, fn func(time.Duration)) func(e *EnvOptionsReader) {
	return func(e *EnvOptionsReader) {
		if v, ok := e.GetEnvValue(n); ok {
			...
			if err != nil {
				slog.Error("parse duration", "error", err, "input", v)
				return
			}
...
func WithCertPool(n string, fn func(*x509.CertPool)) ConfigFn {
	return func(e *EnvOptionsReader) {
		if v, ok := e.GetEnvValue(n); ok {
			b, err := e.ReadFile(v)
			if err != nil {
				slog.Error("read tls ca cert file", "error", err, "file", v)
				return
			}
			c, err := createCertPool(b)
			if err != nil {
				slog.Error("create tls cert pool", "error", err)
				return
			}
			fn(c)
		}
...
func WithClientCert(nc, nk string, fn func(tls.Certificate)) ConfigFn {
	return func(e *EnvOptionsReader) {
		...
		if err != nil {
			slog.Error("read tls client cert", "error", err, "file", vc)
			return
		}
		key, err := e.ReadFile(vk)
		if err != nil {
			slog.Error("read tls client key", "error", err, "file", vk)
			return
		}
		crt, err := tls.X509KeyPair(cert, key)
		if err != nil {
			slog.Error("create tls client key pair", "error", err)
			return
		}
		fn(crt)
	}
...
func stringToHeader(value string) map[string]string {
	...
	for _, header := range headersPairs {
		...
		if !found {
			slog.Error("parse headers", "error", errors.New("missing '="), "input", header)
			continue
		}
		name, err := url.PathUnescape(n)
		if err != nil {
			slog.Error("escape header key", "error", err, "key", n)
			continue
		}
		trimmedName := strings.TrimSpace(name)
		value, err := url.PathUnescape(v)
		if err != nil {
			slog.Error("escape header value", "error", err, "value", v)
			continue
		}
