...
func newPublishCheckEnv(ctx context.Context, source *dagger.Directory) (*publishCheckEnv, error) {
	...
	releaseTag, releaseVersion, source, err := publishCheckRelease(ctx, source)
	if err != nil {
		return nil, err
	}
...
// publishCheckRelease derives the release tag under test from the tree's
// target version (helm/dagger/Chart.yaml) and returns the source with its
// target-version files restamped to match.
//
// The tree may carry a prerelease target version while a release is being
// cut (e.g. 1.0.0-beta.7 between betas), but Release.publish deliberately
// skips docs, SDK and Helm publishing for prerelease tags — which would
// defeat most of this check's assertions. Strip any prerelease/build suffix
// so the check always exercises the full stable-release path, and restamp
// the version files that must agree with the tag: helm packaging names the
// chart archive after Chart.yaml's version, the Go/PHP SDK mirrors publish
// their generated version files verbatim (asserted against the release
// version), and the released CLI self-reports internal/version/VERSION.
func publishCheckRelease(ctx context.Context, source *dagger.Directory) (tag, version string, stamped *dagger.Directory, rerr error) {
	chartYaml, err := source.File("helm/dagger/Chart.yaml").Contents(ctx)
	if err != nil {
		return "", "", nil, fmt.Errorf("read Helm chart metadata: %w", err)
	}
	var chart struct {
		Version string `json:"version"`
	}
	if err := yaml.Unmarshal([]byte(chartYaml), &chart); err != nil {
		return "", "", nil, fmt.Errorf("parse Helm chart metadata: %w", err)
	}

	version = strings.TrimSpace(chart.Version)
	if version == "" {
		return "", "", nil, fmt.Errorf("helm chart version is empty")
	}
	tag = version
	...
	if !semver.IsValid(tag) {
		return "", "", nil, fmt.Errorf("helm chart version %q does not produce a valid release tag", version)
	}
	// Stabilize: strip any prerelease/build suffix (the version core cannot
	// contain "-" or "+").
	...
	// Restamp Chart.yaml's version line in place, preserving everything else.
	chartLines := strings.Split(chartYaml, "\n")
	restamped := false
	for i, line := range chartLines {
		if strings.HasPrefix(line, "version:") {
			chartLines[i] = "version: " + version
			restamped = true
		}
	}
	if !restamped {
		return "", "", nil, fmt.Errorf("could not find version line in Helm chart metadata:\n%s", chartYaml)
	}

	// These file contents mirror the release target-version generators (see
	// generate.go); assertSDKTags greps for the Go/PHP contents verbatim.
	source = source.
		WithNewFile("helm/dagger/Chart.yaml", strings.Join(chartLines, "\n")).
		WithNewFile("internal/version/VERSION", tag+"\n").
		WithNewFile("sdk/go/engineconn/version.gen.go", fmt.Sprintf(`// Code generated by dagger. DO NOT EDIT.

package engineconn

const CLIVersion = %q
`, version)).
		WithNewFile("sdk/php/src/Connection/version.php", fmt.Sprintf("<?php\n\n/* Code generated by dagger. DO NOT EDIT. */\nreturn '%s';\n", version))

	return tag, version, source, nil
}
