...
// HeaderComment is written at the top of every non-empty lockfile. It exists
// to steer readers (humans and coding agents alike) towards committing lockfile
// changes instead of discarding them: the lockfile is updated as a side effect
// of running Dagger, so a diff here rarely looks connected to the change that
// caused it.
...
# Run ` + "`dagger lock update`" + ` to refresh the pinned versions.`
...
func (l *Lockfile) Marshal() ([]byte, error) {
	...
	lines := make([][]byte, 0, len(l.entries)+2)
	lines = append(lines, []byte(HeaderComment))
